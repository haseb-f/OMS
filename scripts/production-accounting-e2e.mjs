...
async function api(token, method, path, body, extraHeaders = {}) {
  ...
  if (!res.ok) {
    const message = json?.message || json?.error || json?.code || text.slice(0, 400);
    ...
    for (const persona of PERSONAS) {
      ...
      try {
        ...
        for (const route of routes[persona.key] ?? [persona.home]) {
          ...
          const financeDeniedExpected = persona.key === "agent" && route.startsWith("/finance/");
...
async function accountingFlow(token) {
  ...
  assert(
    "Posting Settings mapped",
    missingMaps.length === 0,
    missingMaps.join(",") || "all required roles filled",
  );
  assert(
    "Fiscal year exists",
    unwrap(fy).length > 0,
  ...
  const fiscalYear = unwrap(fy).find((y) => y.name === "FY 2026") || unwrap(fy)[0];
  try {
    const opening = await authed("POST", "/accounting/opening-balances", {
      ...
      lines: [
        ...
        {
          accountId: settings.retainedEarningsAccountId || settings.suspenseAccountId,
          credit: 1,
  ...
  assert(
    ...
    unwrap(receiving).length > 0,
    unwrap(receiving)
      .map((r) => r.code || r.name)
      .join(","),
  );
  assert(
    "Payment sources exist",
    unwrap(sources).length > 0,
    unwrap(sources)
      .map((s) => s.code || s.name)
      .join(","),
  );
  ...
  const receivingAccount = pick(receiving, (r) => r.isDefault) ?? pick(receiving, () => true);
  const paymentSource =
  ...
  record(
    "Lead follow-up",
    Boolean(followUps?.id || followUps?.leadId || Array.isArray(followUps)),
    "follow-up saved",
  );
  const actions = await authed("GET", `/workflow/LEAD/${lead.id}/available-actions`);
  record(
    "Lead available-actions",
    Array.isArray(actions) && actions.length > 0,
    `${unwrap(actions).length} actions`,
  );
  ...
  if (invoiceJe) {
    ...
    const hasCogs = lines.some((line) => line.accountId === cogsId || line.account?.id === cogsId);
    ...
    try {
      ...
      if (vatSaleJe) {
        ...
        const vatCodes = (full.lines ?? [])
          ...
          .filter(Boolean)
          .join(",");
        record("VAT sales JE includes tax/AR/revenue", vatCodes.length > 0, vatCodes);
      }
      const vatReceipt = await authed("POST", "/financial-transactions/receipts", {
        ...
        allocations: [{ invoiceId: vatInvoice.id, allocatedAmount: Number(vatDoc.grandTotal) }],
      });
  ...
  try {
    ...
    if ((depRun.periodIds ?? [])[0]) {
      const depJe = await waitForJe(token, "FIXED_ASSET_DEPRECIATION", depRun.periodIds[0]);
  ...
  try {
    ...
    if ((recognized.recognitionIds ?? [])[0]) {
      const recJe = await waitForJe(token, "PREPAID_RECOGNITION", recognized.recognitionIds[0]);
  ...
  const tbBalanced =
    Math.abs(Number(tb.totals?.debitTotal) - Number(tb.totals?.creditTotal)) < 0.05;
  assert(
  ...
  try {
    ...
    const debitAccount =
      postingAccounts.find((account) => account.accountType === "EXPENSE") ?? postingAccounts[0];
    const creditAccount =
