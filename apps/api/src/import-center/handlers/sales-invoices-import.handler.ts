...
/**
 * Sales Invoices Import (TASK-059) — direct/standalone invoices (no
 * originating Sales Order), one `SalesInvoicesService.create()` call per
 * `documentNumber` group. Creates a Draft only — Confirm (inventory +
 * Posting Engine) stays a deliberate manual action, exactly like a manually
 * created invoice.
 */
@Injectable()
export class SalesInvoicesImportHandler
  implements ImportTypeHandler, OnModuleInit
{
  readonly type = 'SALES_INVOICES';
  readonly labelKey = 'importCenter.types.salesInvoices.label';
  readonly descriptionKey = 'importCenter.types.salesInvoices.description';
