...
/**
 * Public Investor Portal screens (login/forgot-password/activate) — reuses
 * the visual pattern (not the component) of the internal `(auth)/layout.tsx`
 * brand panel, since it's this repo's established public-auth-screen look.
 * A standalone copy, not a shared import, so the two auth systems never
 * couple through a shared layout component (mission Part 18/22/52).
 */
export default function InvestorPortalPublicLayout({ children }: { children: ReactNode }) {
  return (
