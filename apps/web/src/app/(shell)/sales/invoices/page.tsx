...
function SalesInvoicesPageContent() {
  ...
  return (
    <div className="flex flex-col gap-6">
      <PageHeader
        title={t("sales.invoices.title")}
        subtitle={t("sales.invoices.description")}
        actions={
          <>
          ...
          <>
            ...
            >
              <SelectTrigger size="sm" className="w-40">
                <SelectValue placeholder={t("sales.invoices.filters.status")} />
              </SelectTrigger>
              <SelectContent>
                <SelectItem value="__all__">{t("sales.invoices.filters.allStatuses")}</SelectItem>
                {INVOICE_FILTERABLE_STATUSES.map((status) => (
                  <SelectItem key={status} value={status}>
                    {t(INVOICE_STATUS_LABEL_KEY[status])}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            <div className="flex items-center gap-1">
              ...
              {customerFilter && (
                <EnterpriseButton
                  ...
                  variant="ghost"
                  size="icon-sm"
                  aria-label={t("sales.invoices.filters.allCustomers")}
                  onClick={() => {
