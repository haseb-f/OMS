const en = {
  common: {
    none: "— None —",
    yes: "Yes",
    no: "No",
    select: "Select…",
    clearSelection: "Clear selection",
    search: "Search",
    loading: "Loading…",
    noResults: "No results found.",
    noDataAvailable: "No data available.",
    addNew: "Add new",
    loadListFailed: 'Couldn\'t load "{name}". Refresh the page or try again later.',
    comingSoon: "Coming Soon",
    comingSoonDescription: "This page is not built yet.",
    offline: "You're offline",
    offlineDescription: "Check your connection and try again.",
    retry: "Retry",
    loadFailed: "Couldn't load this data.",
    failedToSave: "Couldn't save",
    confirming: "Working…",
    noPermission: "No Permission",
    noPermissionDescription: "You don't have access to this page.",
    cancel: "Cancel",
    confirm: "Confirm",
    reason: "Reason",
    optional: "Optional",
    required: "Required.",
    delete: "Delete",
    import: "Import",
    save: "Save",
    saved: "Saved",
    edit: "Edit",
    archive: "Archive",
    restore: "Restore",
    activityLog: "Activity Log",
    active: "Active",
    archived: "Archived",
    close: "Close",
    done: "Done",
    actions: "Actions",
    back: "Back",
    home: "Home",
    view: "View",
    status: "Status",
    createdAt: "Created",
    updatedAt: "Updated",
    noActivity: "No activity yet.",
    showArchived: "Show archived",
    confirmArchiveTitle: "Archive this record?",
    confirmArchiveDescription: "It will be hidden from the list. You can restore it later.",
    confirmRestoreTitle: "Restore this record?",
    confirmRestoreDescription: "It will reappear in the active list.",
    confirmDiscardTitle: "Discard unsaved changes?",
    confirmDiscardDescription: "Your changes will be lost.",
    discard: "Discard",
    keepEditing: "Keep editing",
    generalInformation: "General Information",
    expand: "Expand",
    collapse: "Collapse",
    remove: "Remove",
    selectAll: "Select all",
    deselectAll: "Deselect all",
    summary: "Summary",
    moreActions: "More actions",
    previous: "Previous",
    next: "Next",
  },
  accessDenied: {
    title: "Access Denied",
    description:
      "You don't have permission to view this page. Contact an administrator if you believe this is a mistake.",
  },
  phone: {
    countryLabel: "Country",
    numberLabel: "Phone number",
    searchCountryPlaceholder: "Search by name or country code...",
    noCountryFound: "No results.",
    exampleLabel: "Example",
    valid: "Valid phone number",
    clear: "Clear phone number",
    regionMismatchTitle: "A different country code was detected",
    regionMismatchDescription:
      "This number starts with a different country code than the selected country ({country}). Check the number or change the country.",
    errors: {
      EMPTY: "Phone number is required.",
      NOT_A_NUMBER: "This doesn't look like a phone number.",
      TOO_SHORT: "The number is too short.",
      TOO_LONG: "The number is longer than allowed for this country.",
      INVALID_LENGTH: "The number's length is invalid for this country.",
      INVALID_COUNTRY: "The phone number doesn't match the selected country.",
      INVALID_PATTERN: "The phone number doesn't match the selected country's format.",
      exampleSuffix: "Example of a valid number: {example}",
    },
  },
  /**
   * The one place a backend error becomes user-facing text — mirrors ar.ts
   * exactly (see there for the rationale). Kept in English as the fallback
   * for the (currently unused) secondary locale.
   */
  errors: {
    VALIDATION_ERROR: "Check the information you entered and try again.",
    VALIDATION_ERROR_FIELD: 'The value in "{field}" isn\'t valid. Check it, then try saving again.',
    DUPLICATE:
      "This value is already used by another record. Enter a different value and try again.",
    DUPLICATE_FIELD:
      'The value in "{field}" is already used by another record. Enter a different value and try again.',
    DUPLICATE_EMAIL: "This email is already in use.",
    INVALID_CREDENTIALS: "Incorrect email or password.",
    ACCOUNT_DISABLED: "This user is inactive.",
    ACCOUNT_LOCKED: "This account is locked. Contact an administrator.",
    MISSING_EXCHANGE_RATE:
      "An exchange rate is missing for this posting date. Record the rate, then try again.",
    PERMISSION_ERROR: "No permission",
    NOT_FOUND:
      "This item couldn't be found — it may have been deleted or moved. Refresh the page and try again.",
    SERVER_ERROR:
      "An unexpected server error occurred. Try again, and contact support if the problem continues.",
    DATABASE_ERROR:
      "The operation couldn't complete due to a database error. Try again or contact support.",
    DEPENDENCY_ERROR:
      "This action can't be completed because this item is linked to other data in the system.",
    NETWORK_ERROR: "Couldn't reach the server. Check your internet connection and try again.",
    generic: "Something went wrong. Try again.",
    activationBlocked:
      "Can't activate until you complete: {fields}. You can keep working on it as a draft until then.",
    fields: {
      revenueAccountId: "Revenue Account",
      inventoryAccountId: "Inventory Account",
      cogsAccountId: "Cost of Goods Sold Account",
      purchaseAccountId: "Purchase Account",
      parentAccountId: "Parent Account",
      accountId: "Account",
      currencyId: "Currency",
      categoryId: "Category",
      brandId: "Brand",
      unitId: "Unit",
      taxId: "Tax",
      customerGroupId: "Customer Group",
      supplierGroupId: "Supplier Group",
      paymentTermId: "Payment Term",
      paymentMethodId: "Payment Method",
      shippingMethodId: "Shipping Method",
      countryId: "Country",
      cityId: "City",
      warehouseId: "Warehouse",
      preferredWarehouseId: "Default Warehouse",
      preferredPartnerId: "Preferred Supplier",
      analyticAccountId: "Cost Center",
      costCenterId: "Cost Center",
      customerId: "Customer",
      supplierId: "Supplier",
      productId: "Product",
      name: "Name",
      code: "Code",
      email: "Email",
      phone: "Phone",
      mobile: "Mobile",
      sku: "SKU",
      username: "Username",
      password: "Password",
      text: "Note",
      note: "Note",
      notes: "Note",
      file: "Attachment",
      salesPrice: "Sales Price",
      purchasePrice: "Purchase Price",
      weight: "Weight & Dimensions",
      width: "Weight & Dimensions",
      height: "Weight & Dimensions",
      length: "Weight & Dimensions",
    },
  },
  permissions: {
    searchPlaceholder: "Search modules…",
    totalGranted: "{count} permission(s) granted",
    expandAll: "Expand All",
    collapseAll: "Collapse All",
    columnModule: "Module",
    columnGranted: "Granted",
    noResults: "No modules match your search.",
    sections: {
      sales: "Sales",
      hr: "HR",
      investors: "Investors",
    },
    actions: {
      view: "View",
      create: "Create",
      edit: "Edit",
      delete: "Delete",
      confirm: "Confirm",
      approve: "Approve",
      cancel: "Cancel",
      post: "Post",
      reverse: "Reverse",
      print: "Print",
      export: "Export",
      import: "Import",
      manage: "Manage",
    },
    modules: {
      dashboard: "Dashboard",
      partners: "Partners",
      leads: "Leads / Orders",
      products: "Products",
      inventory: "Inventory",
      salesQuotations: "Sales Quotations",
      salesOrders: "Sales Orders",
      salesInvoices: "Sales Invoices",
      salesReturns: "Sales Returns",
      customerReceipts: "Customer Receipts",
      customerRefunds: "Customer Refunds",
      purchaseQuotations: "Purchase Quotations",
      purchaseOrders: "Purchase Orders",
      purchaseInvoices: "Purchase Invoices",
      purchaseReturns: "Purchase Returns",
      landedCost: "Landed Cost",
      costExplorer: "Cost Explorer",
      costAnalytics: "Cost Analytics & P&L",
      costAllocationRules: "Cost Allocation Rules",
      supplierPayments: "Supplier Payments",
      expensePayments: "Expense Payments",
      journalEntries: "Journal Entries",
      chartOfAccounts: "Chart of Accounts",
      bankTransactions: "Bank Transactions",
      openingBalances: "Opening Balances",
      openingInventory: "Opening Inventory",
      financialReports: "Financial Reports",
      inventoryReports: "Inventory Reports",
      importCenter: "Import Center",
      settings: "Settings",
      storeOrders: "Store Orders",
      shipping: "Shipping",
      departments: "Departments",
      customerClassifications: "Lead Classifications",
      noPurchaseReasons: "No Purchase Reasons",
      leadFollowUpTypes: "Lead Follow-up Types",
      salesTeams: "Sales Teams",
      jobTitles: "Job Titles",
      employees: "Employees",
      payrollComponents: "Payroll Components",
      kpiTemplates: "KPI Templates",
      kpiEvaluations: "KPI Evaluations",
      salesTargets: "Monthly Sales Targets",
      commissionPlans: "Commission Plans",
      commissions: "Commissions",
      payroll: "Payroll Runs",
      warehouses: "Warehouses",
      warehouseLocations: "Warehouse Locations",
      units: "Units of Measure",
      unitConversions: "Unit Conversions",
      categories: "Product Categories",
      brands: "Product Brands",
      currencies: "Currencies",
      taxes: "Taxes",
      countries: "Countries",
      cities: "Cities",
      languages: "Languages",
      transactionTypes: "Transaction Types",
      workflowStatuses: "Workflow Statuses",
      customerGroups: "Customer Groups",
      supplierGroups: "Supplier Groups",
      shippingCompanies: "Shipping Companies",
      shippingStatuses: "Shipping Statuses",
      paymentMethods: "Payment Methods",
      paymentTerms: "Payment Terms",
      analyticPlans: "Analytic Plans",
      analyticAccounts: "Analytic Accounts",
      costCenters: "Cost Centers",
      projects: "Projects",
      journals: "Journals",
      expenses: "Expenses",
      fixedAssets: "Fixed Assets",
      prepaidExpenses: "Prepaid Expenses",
      accruedExpenses: "Accrued Expenses",
      exchangeRates: "Exchange Rates",
      fxRevaluations: "FX Revaluation",
      workflowTransitions: "Workflow Transitions",
      analyticDistributions: "Analytic Distributions",
      paymentSources: "Payment Sources",
      fulfillmentCostRules: "Fulfillment Cost Rules",
      carrierReconciliation: "Carrier Reconciliation",
      receivingAccounts: "Receiving Accounts",
      numbering: "Document Numbering",
      costEngine: "Cost Engine",
      costComponents: "Cost Categories",
      fiscalConfiguration: "Fiscal Configuration",
      physicalCount: "Physical Count",
      investors: "Investors",
      investmentOpportunities: "Investment Opportunities",
      investorSubscriptions: "Investor Subscriptions",
      capitalContributions: "Capital Contributions",
      investmentSales: "Investment Sales Allocation",
      investmentExpenses: "Opportunity Expenses",
      investmentProfit: "Investor Profit Engine",
      investmentSettlement: "Opportunity Settlement",
      investmentDistributions: "Profit Distributions",
      investmentPayments: "Investor Payments",
      investorLedger: "Investor Ledger",
      capitalReturns: "Capital Returns",
      investmentAccounting: "Investor Accounting Settings",
      investorSettings: "Investor Settings",
      investorPortal: "Investor Portal",
    },
  },
  nav: {
    dashboard: "Dashboard",
    dashboardSubtitle: "Overview across every OMS module",
    crm: "CRM",
    crmLeads: "Leads",
    crmSalesTeams: "Sales Teams",
    storeOrders: "Store Orders",
    storeOrdersList: "Store Orders",
    storeOrdersImport: "Import Orders",
    storeOrdersNeedsReview: "Needs Review",
    storeOrdersShipping: "Shipping",
    storeOrdersShippingImport: "Import Shipping",
    shipping: "Shipping",
    sales: "Sales",
    salesCustomers: "Customers",
    salesQuotations: "Quotations",
    salesOrders: "Orders",
    salesInvoices: "Invoices",
    salesReturns: "Returns",
    purchasing: "Purchasing",
    purchasingSuppliers: "Suppliers",
    purchasingQuotations: "Quotations",
    purchasingOrders: "Purchase Orders",
    purchasingInvoices: "Purchase Invoices",
    purchasingReturns: "Returns",
    purchasingLandedCost: "Landed Cost",
    products: "Products & Warehouses",
    productsList: "Products",
    inventory: "Inventory",
    inventoryMovements: "Movements",
    inventoryStock: "Actual Stock",
    inventoryPhysicalCount: "Physical Count",
    expenses: "Expenses",
    expensesComponents: "Cost Categories",
    expensesProductCost: "Product Cost",
    expensesCostExplorer: "Cost Explorer",
    finance: "Finance",
    financeChartOfAccounts: "Chart of Accounts",
    financeBankTransactions: "Financial Operations",
    financeJournals: "Journals",
    financePaymentSources: "Payment Sources",
    financeFulfillmentCostRules: "Fulfillment Cost Rules",
    financeCostAllocationRules: "Cost Allocation Rules",
    financeCarrierReconciliation: "Carrier Reconciliation",
    financeReceivingAccounts: "Accounts",
    financeProjects: "Projects",
    financeCostCenters: "Cost Centers",
    financeExpenses: "Expenses",
    financeFixedAssets: "Fixed Assets",
    financePrepaidExpenses: "Prepaid Expenses",
    financeAccruedExpenses: "Accrued Expenses",
    financeFx: "Exchange Rates & FX",
    financeJournalEntries: "Journal Entries",
    financeCustomerReceipts: "Receipts",
    financeSupplierPayments: "Payments",
    financePaymentReview: "Payment Review",
    financeFiscalPeriods: "Fiscal Years & Periods",
    financeOpeningBalances: "Opening Balances",
    financeYearClosing: "Year Closing",
    financeAccountingSettings: "Accounting Settings",
    dataManagement: "Data Management",
    dataManagementImportCenter: "Import Center",
    masterData: "Master Data",
    masterDataPartners: "Partners",
    reports: "Reports",
    reportsExecutive: "Executive Reports",
    reportsSales: "Sales Reports",
    reportsCrm: "CRM Reports",
    reportsPurchasing: "Purchasing Reports",
    reportsInventory: "Inventory Reports",
    reportsFinance: "Finance Reports",
    reportsExpenses: "Expense Reports",
    reportsCustomers: "Customer Reports",
    reportsSuppliers: "Supplier Reports",
    reportsPartners: "Partner Statement",
    reportsShipping: "Shipping Reports",
    reportsCustom: "Custom Reports",
    reportsAnalytics: "Analytical Reports",
    settings: "Settings",
    settingsGeneral: "General",
    settingsUsers: "Users",
    settingsRoles: "Roles",
    settingsPermissions: "Permissions",
    settingsDocumentNumbering: "Document Numbering",
    settingsPrintSettings: "Print Settings",
    settingsNotifications: "Notifications",
    settingsIntegrations: "Integrations",
    settingsSecurity: "Security",
    settingsBackup: "Backup",
    hr: "HR",
    hrMyProfile: "My Profile",
    hrEmployees: "Employees",
    hrKpiTemplates: "KPI Templates",
    hrKpiEvaluations: "KPI Evaluations",
    hrSalesTargets: "Monthly Sales Targets",
    hrRanking: "Ranking",
    hrCommissionPlans: "Commission Plans",
    hrCommissions: "Commissions",
    hrPayroll: "Payroll Runs",
    hrPayrollComponents: "Payroll Components",
    investors: "Investors",
    investorsDashboard: "Investor Dashboard",
    investorsList: "Investors",
    investorsOpportunities: "Investment Opportunities",
    investorsSettings: "Investor Settings",
  },
  datePicker: {
    open: "Open date picker",
    today: "Today",
    clear: "Clear",
    apply: "Apply",
    time: "Time",
    invalid: "Enter a valid date as DD MMM YYYY (e.g. 15 Aug 2026).",
    from: "From",
    to: "To",
    selectRange: "Select date range",
    quickRange: "Quick Range",
    presets: {
      TODAY: "Today",
      YESTERDAY: "Yesterday",
      LAST_7_DAYS: "Last 7 Days",
      LAST_30_DAYS: "Last 30 Days",
      THIS_MONTH: "This Month",
      LAST_MONTH: "Last Month",
      THIS_YEAR: "This Year",
    },
    clearDates: "Clear dates",
    cancel: "Cancel",
  },
  monthPicker: {
    open: "Open month picker",
    previousYear: "Previous year",
    nextYear: "Next year",
    currentMonth: "Current month",
  },
  company: {
    switcherLabel: "Active company",
    companies: "Companies",
    branches: "Branch",
    noBranch: "No branch",
  },
  sidebar: {
    pinned: "Pinned",
    recent: "Recent",
    modules: "Modules",
    pin: "Pin",
    unpin: "Unpin",
  },
  topbar: {
    searchPlaceholder: "Search…",
    notifications: "Notifications",
    notificationsEmptyTitle: "You're all caught up",
    notificationsEmptyDescription: "Nothing new to review yet.",
    quickActionsEmptyTitle: "No quick actions yet",
    quickActionsEmptyDescription: "Business modules will register actions here.",
    themeLight: "Light",
    themeDark: "Dark",
    themeSystem: "System",
    changeTheme: "Change theme",
    changeLanguage: "Change language",
    profileMenu: "Open profile menu",
    openNavigation: "Open navigation",
    profile: "Profile",
    accountSettings: "Account Settings",
    logout: "Log out",
    guestUser: "Guest User",
    notSignedIn: "Not signed in",
    commandPaletteTitle: "Command Palette",
    commandPaletteDescription: "Search modules and pages",
    commandPalettePlaceholder: "Search modules and pages…",
    commandPaletteGroupNavigate: "Navigate",
  },
  reportExport: {
    excel: "Excel (.xlsx)",
    csv: "CSV (.csv)",
    company: "Company",
    period: "Period",
    allDates: "All dates",
    printedAt: "Printed at",
    printedBy: "Printed by",
    documentNumber: "No.",
    generatedBy: "Generated by OMS",
    page: "Page",
    of: "of",
    printExpired: "This print job has expired. Close this tab and print again.",
  },
  table: {
    sortAscending: "Sort ascending",
    sortDescending: "Sort descending",
    hideColumn: "Hide column",
    columns: "Columns",
    export: "Export",
    exportComingSoon: "Export is coming soon.",
    selectAll: "Select all rows",
    selectRow: "Select row",
    selectionMenuLabel: "Selection options",
    selectionSelectPage: "Select current page",
    selectionSelectAllFiltered: "Select all results",
    selectionSelectCustomCount: "Select a specific number...",
    rowsSelected: "selected",
    selectAllMatching: "Select all {count} records matching filters",
    selectingAllMatching: "Selecting…",
    allMatchingSelected: "All {count} matching records selected",
    allFilteredSelected: "All results matching current filters selected: {count} records",
    usePageSelection: "Use current page selection only",
    clearSelection: "Clear selection",
    rowsPerPage: "Rows per page",
    pageOf: "Page {page} of {pageCount}",
    goToFirstPage: "Go to first page",
    goToPreviousPage: "Go to previous page",
    goToNextPage: "Go to next page",
    goToLastPage: "Go to last page",
    noResults: "No results.",
    filterPlaceholder: "Filter…",
    clearSearch: "Clear search",
    noSearchResults: 'No results for "{term}".',
    noSearchResultsHint: "Check the spelling or try a different search term.",
    refresh: "Refresh",
    print: "Print",
    options: "Table options",
    density: "Density",
    densityComfortable: "Comfortable",
    densityCompact: "Compact",
    duplicate: "Duplicate",
    exportDialogTitle: "Export to CSV",
    exportDialogDescription: "Choose which columns to include.",
    selectAllColumns: "Select all",
    exportConfirm: "Export",
    importDialogTitle: "Import from CSV",
    importDialogDescription: "Upload a CSV file to preview its rows before importing.",
    importChooseFile: "Choose File",
    importPreviewRows: "{count} rows found",
    importConfirm: "Import",
    importNoFile: "No file selected yet.",
    addSortAscending: "Add sort: ascending",
    addSortDescending: "Add sort: descending",
    clearSort: "Clear sort",
    pinColumnStart: "Pin to start",
    pinColumnEnd: "Pin to end",
    unpinColumn: "Unpin column",
    filterColumn: "Filter column",
    clearFilter: "Clear filter",
    clearFilters: "Clear filters",
    activeFilterCount: "{count} filters active",
    filterAll: "All",
    filterSelectedCount: "{count} selected",
    resultCount: "{count} results",
    showMore: "Show {count} more",
    lineItems: "Line items",
    customerDetails: "Customer details",
    supplierDetails: "Supplier details",
    notes: "Notes",
    restoreDefaultLayout: "Restore default layout",
    layoutRestored: "Table layout restored to defaults.",
    resizeColumn: "Resize column",
    moveColumnStart: "Move toward start",
    moveColumnEnd: "Move toward end",
    copied: "Copied to clipboard.",
    loadFailed: "Couldn't load this list. Try again.",
  },
  dashboard: {
    welcomeTitle: "Dashboard",
    welcomeSubtitle: "Track system activity and performance metrics.",
    kpiTitle: "Key Metrics",
    kpiEmptyTitle: "No metrics yet",
    kpiEmptyDescription: "Metrics appear once business modules report data.",
    quickActionsTitle: "Quick Actions",
    recentActivityTitle: "Recent Activity",
    recentActivityEmptyTitle: "No recent activity",
    recentActivityEmptyDescription: "Activity across OMS will show up here.",
    pinnedModulesTitle: "Pinned Modules",
    pinnedModulesEmptyTitle: "Nothing pinned yet",
    pinnedModulesEmptyDescription: "Pin a page from the sidebar to find it here.",
  },
  designSystem: {
    title: "Design System",
    subtitle: "The permanent OMS reference — every future component is added here first.",
    typography: "Typography",
    colors: "Colors",
    buttons: "Buttons",
    inputs: "Inputs",
    cards: "Cards",
    tables: "Tables",
    dialogs: "Dialogs",
    sidebarTopbar: "Sidebar & Topbar",
    emptyStates: "Empty States",
    icons: "Icons",
    spacing: "Spacing",
    shadows: "Shadows",
    radius: "Radius",
    animations: "Animations",
    primaryButton: "Primary",
    secondaryButton: "Secondary",
    outlineButton: "Outline",
    ghostButton: "Ghost",
    destructiveButton: "Destructive",
    sampleCardTitle: "EnterpriseCard Title",
    sampleCardDescription: "Every card follows Title → Content → Footer.",
    sampleCardFooterAction: "Action",
    dialogTrigger: "Open Dialog",
    dialogTitle: "Dialog Title",
    dialogDescription: "Dialogs are 30px radius with a soft shadow — no glass, no blur.",
    alertDialogTrigger: "Open Confirmation",
    emptyStateTitle: "No data yet",
    emptyStateDescription: "This is the shared empty state used everywhere in OMS.",
    emptyStateAction: "Primary Action",
    animationTrigger: "Replay animation",
    sidebarPreviewNote:
      "The Sidebar is the primary navigation workspace — no search box, theme-adaptive surface, multiple modules can stay expanded at once, and labels never truncate.",
    topbarPreviewNote:
      "The Topbar is a fixed, full-width App Bar — Search, Language, Theme, Notifications, User. Solid background, no glass, never scrolls away.",
    navigation: "Navigation",
    navigationNote:
      "Navigation items come only from navigation.config.ts — never hand-built per page.",
    loadingStates: "Loading States",
    loadingOverlayTrigger: "Show loading overlay",
  },
  auth: {
    loginTitle: "Sign in",
    loginSubtitle: "Sign in to your account to access the Operations Management System.",
    email: "Email",
    emailPlaceholder: "you@company.com",
    password: "Password",
    passwordPlaceholder: "••••••••",
    showPassword: "Show password",
    hidePassword: "Hide password",
    rememberMe: "Remember me",
    forgotPassword: "Forgot password?",
    signIn: "Sign in",
    signingIn: "Signing in…",
    invalidCredentials: "Incorrect email or password.",
    genericError: "Something went wrong. Please try again.",
    emailRequired: "Enter a valid email address.",
    passwordRequired: "Password is required.",
    backToLogin: "Back to sign in",
    forgotTitle: "Forgot your password?",
    forgotSubtitle: "Enter your email and we'll send you a reset link.",
    sendResetLink: "Send reset link",
    sendingResetLink: "Sending…",
    forgotSuccessTitle: "Check your email",
    forgotSuccessDescription: "If an account exists for this email, a reset link has been sent.",
    resetTitle: "Reset your password",
    resetSubtitle: "Choose a new password for your account.",
    newPassword: "New password",
    confirmPassword: "Confirm password",
    passwordMinLength: "Must be at least 8 characters.",
    passwordsDontMatch: "Passwords don't match.",
    resetSubmit: "Reset password",
    resetSubmitting: "Resetting…",
    resetSuccessTitle: "Password updated",
    resetSuccessDescription: "Your password has been reset. You can now sign in.",
    resetInvalidToken: "This reset link is invalid or has expired.",
    missingToken: "No reset token was provided.",
    requestNewLink: "Request a new link",
  },
  businessComponents: {
    title: "Business Components",
    subtitle:
      "Reusable enterprise building blocks every future module composes from — no business logic, sample data only.",
    entityHeader: "Entity Header",
    statusBadge: "Status EnterpriseBadge",
    moneyBadge: "Money EnterpriseBadge & Currency Display",
    partyCards: "Customer & Supplier Cards",
    timelines: "Timelines",
    addressCard: "Address EnterpriseCard",
    quickStatsCard: "Quick Stats EnterpriseCard",
    infoSection: "Info Section",
    summaryCard: "Summary EnterpriseCard",
    entityTabs: "Entity Tabs",
    quickActions: "Quick Actions",
    sampleEntitySubtitle: "Sample Customer — #CUST-1042",
    sampleAddressDefault: "Default",
    tabOverview: "Overview",
    tabActivity: "Activity",
    tabDocuments: "Documents",
    overviewSample: "Sample overview content goes here.",
    documentsSample: "No documents attached yet.",
    actionPrint: "Print",
    actionDuplicate: "Duplicate",
    actionArchive: "Archive",
  },
  documentEngine: {
    title: "Document Engine",
    subtitle:
      "Architecture foundation — one shared template every printable document renders through. No PDF generation yet.",
    previewHeading: "Sample preview — Quotation",
    printEngineBody:
      "Printing renders this document through the Enterprise Print Engine's isolated print view, never the app screen.",
    printPreview: "Print preview",
    pdfExportComingSoon: "PDF export (coming soon)",
    quotationTitle: "Quotation",
    billTo: "Bill To",
    documentNumber: "Document No.",
    documentDate: "Date",
    description: "Description",
    quantity: "Qty",
    unitPrice: "Unit Price",
    lineTotal: "Total",
    notes: "Notes",
    subtotal: "Subtotal",
    tax: "Tax (15%)",
    total: "Total",
    validUntil: "Valid Until",
    signatureCustomer: "Customer Signature",
    signaturePreparedBy: "Prepared By",
    supportedTypesTitle: "Supported document types",
    printSettingsTitle: "Print settings supported",
    printSettingsBody:
      "A4 Portrait · A4 Landscape · Print Preview · Browser Print · PDF Export (planned) · RTL · LTR · Bilingual documents.",
    brandingTitle: "Per-company branding",
    brandingBody:
      "Logo, primary/secondary color, header/footer text, paper size, and language are configurable per company and feed the same template — never a redesign.",
  },
  masterData: {
    actions: {
      addNew: "Add New",
      saveAndNew: "Save & New",
      quickPreview: "Quick Preview",
      viewActivity: "View Activity",
      bulkArchivePartialFailure: "Failed to archive {count} of the selected items.",
    },
    bulkSelection: {
      selectionCleared: "Selection cleared because the filters changed.",
      customCountPartial: "Only {count} matching records were available.",
    },
    fields: {
      code: "Code",
      name: "Name",
      description: "Description",
      symbol: "Symbol",
      rate: "Rate (%)",
      inclusive: "Tax Inclusive",
      company: "Company",
      country: "Country",
      isDefault: "Default Warehouse",
      warehouseType: "Warehouse Type",
      isActive: "Active",
      type: "Type",
      nativeName: "Native Name",
      direction: "Text Direction",
      logoUrl: "Logo URL",
      primaryColor: "Primary Color",
      secondaryColor: "Secondary Color",
      internalDelivery: "Internal Delivery",
      externalCompany: "External Company",
      ltr: "LTR",
      rtl: "RTL",
      analyticPlan: "Analytic Plan",
      parentAccount: "Parent Account",
      notes: "Notes",
      displayOrder: "Display Order",
      warehouse: "Warehouse",
      fromUnit: "From Unit",
      toUnit: "To Unit",
      conversionRatio: "Conversion Ratio",
      manager: "Manager",
      defaultAnalyticAccount: "Default Analytic Account",
      days: "Days",
      color: "Color",
      sortOrder: "Sort Order",
      nameEn: "English Name",
      accountingClass: "Accounting Class",
      capitalizable: "Capitalizable",
      defaultAccount: "Default Account",
      accountType: "Account Type",
      accountTypeAsset: "Asset",
      accountTypeLiability: "Liability",
      accountTypeEquity: "Equity",
      accountTypeRevenue: "Revenue",
      accountTypeExpense: "Expense",
      journalType: "Journal Type",
      sequencePrefix: "Sequence Prefix",
      defaultDebitAccount: "Default Debit Account",
      defaultCreditAccount: "Default Credit Account",
      currency: "Currency",
      branch: "Branch",
      allowReconciliation: "Allow Reconciliation",
      account: "Linked Account",
      feePercentage: "Fee Percentage (%)",
      feeFixedAmount: "Fee Fixed Amount",
      costAmount: "Cost Amount",
      effectiveFrom: "Effective From",
      effectiveTo: "Effective To",
    },
    colors: {
      neutral: "Gray",
      info: "Blue",
      warning: "Orange",
      success: "Green",
      destructive: "Red",
    },
    hub: {
      title: "Master Data",
      description:
        "The reference lists every OMS module builds on — organized by Analytic Accounting and Warehouses in this single-company setup.",
    },
    warehouses: {
      title: "Warehouses",
      description: "Manage physical and virtual storage locations.",
    },
    currencies: {
      title: "Currencies",
      description: "Manage currencies used across OMS.",
    },
    costCenters: {
      title: "Cost Centers",
      description: "Manage cost centers for journal entries and orders.",
    },
    expenses: {
      title: "Expenses",
      description: "Record expenses — no approval workflow or accounting posting yet.",
      fields: {
        date: "Date",
        amount: "Amount",
        costCenter: "Cost Center",
        paymentMethod: "Payment Method",
      },
    },
    fixedAssets: {
      title: "Fixed Assets",
      description: "Fixed-asset register with capitalization, depreciation, and disposal posting.",
      fields: {
        acquisitionDate: "Acquisition Date",
        cost: "Cost",
        usefulLifeMonths: "Useful Life (Months)",
        salvageValue: "Salvage Value",
        depreciationStartDate: "Depreciation Start",
        accumulatedDepreciation: "Accumulated Depreciation",
        status: "Status",
        disposalAmount: "Disposal Proceeds",
        receivingAccount: "Payment Source",
        partner: "Supplier",
      },
      actions: {
        capitalize: "Capitalize",
        dispose: "Dispose / Write-off",
        runDepreciation: "Run Depreciation",
      },
      toasts: {
        capitalized: "Asset capitalized and posted to the GL.",
        disposed: "Asset disposed and posted to the GL.",
        depreciationRun: "{count} depreciation period(s) posted.",
      },
      validation: {
        usefulLifeRequired: "Set useful life (months) before capitalizing.",
        paymentSourceRequired: "Select a Payment Source / Receiving Account or a supplier.",
      },
    },
    projects: {
      title: "Projects",
      description: "Manage projects for journal entries and orders.",
    },
    taxes: {
      title: "Taxes",
      description: "Manage tax rates applied to transactions.",
    },
    units: {
      title: "Units of Measure",
      description: "Manage the units products are measured in.",
    },
    unitConversions: {
      title: "Unit Conversions",
      description: "Manage conversion ratios between units of measure (e.g. Box to Piece).",
    },
    warehouseLocations: {
      title: "Warehouse Locations",
      description:
        "Manage bins, shelves, and zones inside each warehouse — unlimited, parent/child.",
      addLocation: "Add Location",
      addChild: "Add Child",
      empty: "No locations yet.",
      selectWarehouse: "Select a warehouse to manage its locations.",
      parentLocation: "Parent Location",
      nameRequired: "Name is required.",
    },
    categories: {
      title: "Categories",
      description: "Manage product categories.",
      helperText: {
        name: "The name that will appear throughout the system.",
        revenueAccountId: "The account this category's product sales revenue posts to.",
        inventoryAccountId: "The account representing this category's inventory value.",
        cogsAccountId: "The account used to record the cost of goods sold from this category.",
        purchaseAccountId: "The account used when purchasing products in this category.",
      },
    },
    brands: {
      title: "Brands",
      description: "Manage product brands.",
    },
    costComponents: {
      title: "Cost Categories",
      description:
        "Classify acquisition, fulfillment, and operating costs — accounting behavior stays fixed even if a category is renamed.",
      helperText: {
        capitalizable: "Only capitalizable categories can be added to a Landed Cost document.",
        defaultAccountId: "Resolved when posting — falls back to a system default if left empty.",
      },
    },
    costAccountingClass: {
      INVENTORY_ACQUISITION: "Inventory Acquisition",
      COGS: "Cost of Goods Sold",
      FULFILLMENT: "Fulfillment",
      TRANSACTION: "Transaction",
      OPERATING_EXPENSE: "Operating Expense",
      OTHER: "Other",
    },
    analyticPlans: {
      title: "Analytic Plans",
      description:
        "Manage analytic plans used to dimension transactions (projects, branches, departments, campaigns).",
    },
    analyticAccounts: {
      title: "Analytic Accounts",
      description: "Manage analytic accounts within each plan, with unlimited hierarchy.",
    },
    chartOfAccounts: {
      title: "Chart of Accounts",
      description:
        "Manage the ledger accounts (Asset/Liability/Equity/Revenue/Expense) other modules point at, with unlimited hierarchy.",
      addAccount: "New Account",
      addChild: "Add Sub-Account",
      empty: "No accounts yet.",
      searchPlaceholder: "Search code or name...",
      filters: {
        allTypes: "All Types",
        allStatuses: "All Statuses",
        active: "Active",
        archived: "Archived",
      },
      invalidHierarchy: "An account cannot be its own ancestor.",
      proposedCodeHint:
        "The system generates the account code automatically based on its position in the account tree — a normal employee cannot type one manually.",
      codeOverrideLabel: "Custom code (advanced permission only)",
      codeOverrideHint:
        'Enter a manual code instead of the proposed one — only available to someone with the "Manage" permission on Chart of Accounts.',
      natureLabel: "Account Nature",
      natureMain: "Main Account",
      natureSub: "Sub-Account",
      selectParent: "Select the parent account...",
      parentHint: "Choose the account this one will appear under.",
      parentRequired: "A sub-account needs a parent account.",
      postingAccount: "Posting Account",
      groupAccount: "Group Account",
      expandAll: "Expand All",
      collapseAll: "Collapse All",
      deleteAction: "Delete",
      confirmDeleteTitle: "Delete this account?",
      confirmDeleteDescription:
        "This permanently removes the account. Only possible when it has no references anywhere in the accounting system.",
      deleteSuccess: "Account deleted.",
      protectedTooltip: "This is a protected system account and cannot be deleted.",
      selectVisible: "Select visible accounts",
      selectionCount: "{count} accounts selected",
      includeDescendantsTitle: "Include sub-accounts?",
      includeDescendantsDescription:
        "“{name}” has child accounts. Include all descendants in the selection, or only this account?",
      includeDescendantsYes: "Include descendants",
      includeDescendantsNo: "This account only",
      bulkArchive: "Archive eligible selected",
      bulkArchiveConfirmTitle: "Archive selected accounts?",
      bulkArchiveConfirmDescription:
        "Only unused leaf accounts will be archived. Roots 1–5 and accounts with dependencies are blocked. Selected: {count}.",
      bulkArchiveResult: "Archived {success}. Blocked/skipped: {blocked}.",
    },
    bankTransactions: {
      title: "Cash Flow",
      description:
        "The gateway between bank/cash accounts and Store Orders, B2B Sales Invoices, Purchase Invoices, and Expenses — synchronized from Google Sheets or imported from a bank statement.",
      empty: "No transactions yet.",
      runMatching: "Run Matching",
      review: "Review",
      reconcile: "Reconcile",
      classify: "Classify",
      confirmMatch: "Confirm Match",
      noCandidates: "No suggested matches for this transaction.",
      methodMismatch:
        "Warning: expected payment method differs from actual cash source — matching still allowed",
      methodMismatchTitle: "Payment method mismatch",
      methodMismatchDescription:
        "Expected {expected}, but cash landed in {actual}. Choose how to proceed — matching remains allowed.",
      methodMismatchMatchAnyway: "Match anyway",
      methodMismatchUpdateExpected: "Match and update expected method",
      matchConfirmed: "Match confirmed.",
      matchingRunSuccess: "{count} transaction(s) classified.",
      tabs: { incoming: "Incoming", outgoing: "Outgoing" },
      status: {
        UNMATCHED: "Unmatched",
        POTENTIAL: "Suggested",
        PARTIALLY_MATCHED: "Partially Matched",
        MATCHED: "Matched",
        DUPLICATE: "Duplicate",
        CONFLICT: "Conflict",
        MANUAL_REVIEW: "Needs Review",
      },
      fields: {
        date: "Date",
        transactionId: "External Transaction ID",
        cashSource: "Cash Source",
        description: "Description",
        reference: "Reference",
        amount: "Amount",
        classification: "Type",
      },
      summary: {
        total: "Total",
        matched: "Matched",
        partiallyMatched: "Partial",
        unmatched: "Unmatched",
        conflicts: "Conflicts",
        storeOrderMatches: "Store Order Matches",
        b2bMatches: "B2B Invoice Matches",
        supplierPayments: "Supplier Payments",
        expenses: "Expenses",
        unclassified: "Unclassified",
        pendingVoucher: "Pending Voucher",
        posted: "Posted",
      },
      candidates: {
        payment: "Existing Payment",
        storeOrder: "Store Order",
        salesInvoice: "B2B Sales Invoice",
        purchaseInvoice: "Purchase Invoice",
        confirm: "Confirm",
      },
      manual: {
        searchStoreOrder: "Search a Store Order manually",
        searchSalesInvoice: "Search a B2B Sales Invoice manually",
        searchPurchaseInvoice: "Search a Purchase Invoice manually",
        searchSupplier: "Search a Supplier",
        searchExpenseAccount: "Search an Expense account",
        searchPlaceholder: "Type to search…",
        noResults: "No results.",
      },
      classifyDialog: {
        title: "Classify Outgoing Transaction",
        outgoingType: "Transaction Type",
        supplierPayment: "Supplier Payment",
        expense: "Expense",
        supplier: "Partner/Supplier",
        expenseAccount: "Expense Account",
        costCenter: "Cost Center",
        project: "Project",
        save: "Save Classification",
        saved: "Classification saved.",
      },
      voucher: {
        paymentSource: "Payment Source",
        createExpenseVoucher: "Create & Post Expense Voucher",
        expenseVoucherCreated: "Expense Payment Voucher created and posted.",
        supplierPaymentCreated: "Supplier Payment Voucher created and posted.",
        receiptCreated: "Customer Receipt created and posted.",
        storeOrderPaymentCreated: "Payment recorded and matched to the Store Order.",
      },
      bulk: {
        selectedCount: "{count} selected",
        confirmVouchers: "Create Vouchers",
        classifySelected: "Classify Selected",
      },
      allocation: {
        title: "Allocate to invoices",
        outstanding: "Outstanding",
        allocated: "Allocated",
        remaining: "Remaining on transaction",
        confirm: "Confirm Allocation",
        selectAtLeastOne: "Select at least one invoice to allocate the amount to.",
        feeToggle: "Record a bank fee / settlement difference",
        feeAmount: "Fee amount",
        feeAccount: "Bank fee account",
        feeAccountRequired: "Enter a fee amount and select a bank fee account.",
      },
      transfer: {
        title: "Internal Transfer between accounts",
        find: "Find matching transfer",
        noCandidates: "No matching opposite transaction on a different Financial Account.",
        confirm: "Confirm Internal Transfer",
        confirmed: "Internal transfer confirmed between the two accounts.",
      },
      unreconcile: {
        action: "Unreconcile",
        title: "Unreconcile this transaction?",
        impact: "The transaction will reopen and its linked journal entry will be reversed.",
        reasonLabel: "Reason for unreconciling",
        reasonRequired: "A reason is required to unreconcile.",
        confirm: "Unreconcile",
        success: "Unreconciled",
      },
    },
    journals: {
      title: "Journals",
      description:
        "The books of entry (Sales/Purchase/Cash/Bank/General) operational documents and manual entries post into.",
      addNew: "New Journal",
      type: {
        SALES: "Sales Journal",
        PURCHASE: "Purchase Journal",
        CASH: "Cash Journal",
        BANK: "Bank Journal",
        GENERAL: "General Journal",
      },
    },
    paymentMethods: {
      title: "Payment Methods",
      description: "Manage accepted payment methods.",
    },
    paymentSources: {
      title: "Payment Sources",
      description:
        "Manage how customers pay (Visa, Mada, STC Pay, ...) and their optional fee estimation config.",
    },
    fulfillmentCostRules: {
      title: "Fulfillment Cost Rules",
      description:
        "Manage the flat Packaging/Direct Fulfillment cost applied automatically when an Order's Sales Invoice is generated.",
    },
    costAllocationRules: {
      title: "Cost Allocation Rules",
      description:
        "Spread an indirect-cost pool (a GL Expense account, or a manually entered amount) across Product/Channel/Customer/Employee/Country for a chosen period — a separate management ledger, never posted to the GL.",
      method: "Allocation Method",
      targetDimension: "Target Dimension",
      methodValues: {
        BY_QUANTITY: "By Quantity",
        BY_COST: "By Cost",
        EQUAL: "Equal Split",
        MANUAL: "Manual",
      },
      dimensionValues: {
        PRODUCT: "Product",
        CHANNEL: "Channel",
        CUSTOMER: "Customer",
        EMPLOYEE: "Employee",
        COUNTRY: "Country",
      },
      runs: {
        title: "Runs",
        selectRule: "Select a Rule",
        newRun: "New Run",
        newRunTitle: "Create Allocation Run",
        periodStart: "Period Start",
        periodEnd: "Period End",
        sourceType: "Pool Source",
        sourceGl: "GL Expense Account",
        sourceManual: "Manual Amount",
        sourceAccount: "Source Account",
        manualPoolAmount: "Pool Amount",
        manualBasesTitle: "Manual Weights",
        addRow: "Add Row",
        dimensionValue: "Dimension Value",
        dimensionLabel: "Label",
        weight: "Weight",
        notes: "Notes",
        create: "Create Run",
        status: "Status",
        totalAmount: "Total Amount",
        period: "Period",
        createdAt: "Created",
        viewResults: "View Breakdown",
        post: "Post",
        cancel: "Cancel Run",
        postConfirmTitle: "Post this Run?",
        postConfirmDescription:
          "Locks the Run permanently — it never posts to the GL, only to this management ledger.",
        cancelConfirmTitle: "Cancel this Run?",
        cancelConfirmDescription:
          "This DRAFT Run will be marked cancelled and excluded from reporting.",
        resultsTitle: "Allocation Breakdown",
        basisAmount: "Basis",
        allocatedAmount: "Allocated Amount",
        empty: "No Runs yet for this Rule.",
        created: "Run created.",
        posted: "Run posted.",
        cancelled: "Run cancelled.",
        statusValues: {
          DRAFT: "Draft",
          POSTED: "Posted",
          CANCELLED: "Cancelled",
        },
      },
    },
    paymentTerms: {
      title: "Payment Terms",
      description: "Manage when payment is due (Net 30, Due on Receipt, ...).",
    },
    departments: {
      title: "Departments",
      description:
        "Company organizational departments. New departments appear immediately in User and Sales Team selectors.",
      select: "Select department",
    },
    jobTitles: {
      title: "Job Titles",
      description:
        "Descriptive employment positions — independent of Department and of system Role/Permissions. Changing a Job Title never changes what a user can do in OMS.",
      department: "Department",
      noDepartment: "No department (generic)",
    },
    customerClassifications: {
      title: "Lead Classifications",
      description:
        "Descriptive commercial segmentation for prospects — independent of Lead status and never a workflow action.",
      select: "Select Lead classification",
      preview: "Preview",
    },
    noPurchaseReasons: {
      title: "No Purchase Reasons",
      description: "Configurable reasons for closing a Lead without converting it to an order.",
      select: "Select no-purchase reason",
      classifications: "Suggested classifications (optional)",
    },
    leadFollowUpTypes: {
      title: "Lead Follow-up Types",
      description:
        "How a follow-up happens (call, WhatsApp, email, meeting, ...) — the method, not the result.",
      select: "Select follow-up type",
    },
    investorTypes: {
      title: "Investor Types",
      description:
        "Configurable Investor classification (Individual, Company, Institution, Family Office, Fund, or any custom type) — never a hardcoded list.",
      select: "Select investor type",
      inactiveSuffix: "(Inactive)",
    },
    shippingMethods: {
      title: "Shipping Methods",
      description: "Manage available shipping methods.",
    },
    shippingCompanies: {
      title: "Shipping Companies",
      description: "Manage the carriers used on shipments and store orders.",
    },
    shippingStatuses: {
      title: "Shipping Statuses",
      description:
        "The shipping lifecycle statuses — managed as master data and used in shipments, store orders, imports, and Google Sheets sync.",
      role: "Role",
      default: "Default",
      system: "System",
      protectedHint:
        'The default status "{status}" is a system status — it cannot be deleted, and it is the initial status for any order ready to ship.',
      setDefault: "Set as Default",
      setDefaultSuccess: "Default shipping status updated.",
      setDefaultError: "Could not change the default shipping status.",
      syncBehavior: {
        label: "Sync Behavior",
        underSync: "Under Sync",
        final: "Final",
        helperText:
          "Under Sync: the shipment stays eligible for future Shipping Sync checks and updates. Final: the shipment is considered complete — the system will not re-sync it or show it in repeated update reports.",
      },
    },
    workflowStatuses: {
      title: "Workflow Statuses",
      description:
        "Create, rename, recolor, reorder, archive, and restore the status values each workflow uses — protected statuses that a live business flow depends on stay locked.",
      workflowType: "Workflow",
      system: "System",
      final: "Final",
      code: "Code",
      types: {
        LEAD: "Leads",
        ORDER: "Orders",
        PAYMENT: "Payment",
        FULFILLMENT: "Shipping / Fulfillment",
        MATCHING: "Matching",
        RECONCILIATION: "Reconciliation",
      },
    },
    customerGroups: {
      title: "Customer Groups",
      description: "Manage customer segmentation groups.",
    },
    supplierGroups: {
      title: "Supplier Groups",
      description: "Manage supplier segmentation groups.",
    },
    countries: {
      title: "Countries",
      description: "Manage countries referenced across OMS.",
      codeHint:
        'ISO 3166-1 two-letter code (e.g. SA). If a country is missing, check "Show archived" and restore it instead of creating a duplicate.',
      codeInvalid: "Code must be exactly two Latin letters A–Z (e.g. SA).",
    },
    cities: {
      title: "Cities",
      description: "Manage cities within each country.",
    },
    languages: {
      title: "Languages",
      description: "Manage languages available in OMS.",
    },
    transactionTypes: {
      title: "Transaction Types",
      descriptionIncoming:
        "System and custom types for incoming cash movements — why money came in.",
      descriptionOutgoing:
        "System and custom types for outgoing cash movements — why money went out.",
      tabs: {
        incoming: "Incoming",
        outgoing: "Outgoing",
      },
      system: "System",
      custom: "Custom",
      inactive: "Inactive",
      transferBadge: "Internal Transfer",
      createNew: "New Transaction Type",
      fields: {
        code: "Code",
        nameAr: "Arabic Name",
        nameEn: "English Name",
        direction: "Direction",
        matchingTarget: "Matching Target",
        defaultAccount: "Default Account",
        accountingTreatment: "Accounting Treatment",
        isActive: "Active",
      },
      direction: {
        IN: "In",
        OUT: "Out",
      },
      matchingTarget: {
        STORE_ORDER: "Store Order",
        SALES_INVOICE: "Sales Invoice",
        PURCHASE_INVOICE: "Purchase Invoice",
        CUSTOMER: "Customer",
        VENDOR: "Vendor",
        EMPLOYEE: "Employee",
        FINANCIAL_ACCOUNT: "Financial Account",
        EXPENSE_ACCOUNT: "Expense Account",
        LIABILITY: "Liability",
        EQUITY_OR_PARTNER: "Equity / Partner",
        INVESTMENT: "Investment",
        ACCOUNT: "Account",
      },
      accountingTreatment: {
        OPERATING_REVENUE: "Operating Revenue",
        OPERATING_EXPENSE: "Operating Expense",
        EQUITY_MOVEMENT: "Equity Movement",
        LIABILITY_MOVEMENT: "Liability Movement",
        TRANSFER: "Internal Transfer",
        NEUTRAL: "Unclassified",
      },
    },
  },
  reports: {
    hub: {
      title: "Reports",
      description:
        "Reporting categories are prepared as a foundation — no reports are generated yet.",
    },
    inventory: {
      description:
        "Movement, valuation, stock, and warehouse-balance reports — all read straight from the movement ledger.",
      movements: "Inventory Movement",
      valuation: "Inventory Valuation",
      currentStock: "Current Stock",
      negativeStock: "Negative Stock",
      negativeStockEmpty: "No products with negative stock.",
      warehouseBalance: "Warehouse Balance",
    },
    finance: {
      description:
        "General Ledger, Trial Balance, Journal Report, Account Statement, AR/AP Aging, and partner statements — all read straight from posted Journal Entries.",
      generalLedger: "General Ledger",
      trialBalance: "Trial Balance",
      journalReport: "Journal Report",
      balanceSheet: "Balance Sheet",
      incomeStatement: "Income Statement",
      cashFlow: "Cash Flow Statement",
      cashAvailability: "Available Cash & Bank Balances",
      cashAvailabilityReport: {
        estimateBadge: "Estimate — not bank-confirmed",
        account: "Account",
        currency: "Currency",
        bookBalance: "Book balance",
        holds: "Holds",
        committed: "Committed out",
        available: "Available to spend",
        egpAvailable: "EGP available",
        egpConsolidated: "EGP consolidated",
        empty: "No cash or bank receiving accounts found.",
        asOf: "As of",
      },
      arAging: "AR Aging",
      apAging: "AP Aging",
      customerStatement: "Customer Statement",
      supplierStatement: "Supplier Statement",
      asOfDate: "As of",
      cashFlowHint:
        "Cash movements are classified into Operating, Investing, and Financing from posted journal sources. Unsupported financing/investing documents stay in Other rather than being invented.",
      viewMovements: "View movements",
      viewLines: "View lines",
      totals: "Totals",
      balanced: "Balanced",
      unbalanced: "Not balanced",
      expandAll: "Expand all",
      collapseAll: "Collapse all",
      cashFlowSections: {
        operating: "Operating Activities",
        operatingNet: "Net cash from operating activities",
        investing: "Investing Activities",
        investingNet: "Net cash from investing activities",
        financing: "Financing Activities",
        financingNet: "Net cash from financing activities",
        other: "Other",
        otherNet: "Net cash from other activities",
        openingCash: "Opening cash",
        closingCash: "Closing cash",
        netChange: "Net increase (decrease) in cash",
        inflows: "Cash inflows",
        outflows: "Cash outflows",
        inflowsDebit: "Inflows (debit)",
        outflowsCredit: "Outflows (credit)",
        cashAccount: "Cash / bank account",
      },
      cashFlowView: {
        label: "Cash flow view",
        activities: "By activity",
        movement: "Cash-account movement",
      },
      accountStatement: {
        title: "Account Statement",
        selectAccountTitle: "Select an account",
        selectAccountDescription: "Choose an account above to view its statement.",
      },
      partnerStatement: {
        title: "Partner Statement",
        selectTitle: "Select a partner",
        selectDescription: "Choose a customer or supplier to view the subledger statement.",
      },
      aging: {
        current: "0–30",
        days31to60: "31–60",
        days61to90: "61–90",
        over90: "90+",
      },
      fields: {
        accountCode: "Account Code",
        accountName: "Account Name",
        openingBalance: "Opening Balance",
        debit: "Debit",
        credit: "Credit",
        closingBalance: "Closing Balance",
        debitTotal: "Debit Total",
        creditTotal: "Credit Total",
        balance: "Balance",
        entryNumber: "Journal Number",
        entryDate: "Date",
        sourceDocument: "Source Document",
        description: "Description",
        postedBy: "Posted By",
        runningBalance: "Running Balance",
        assets: "Assets",
        liabilities: "Liabilities",
        equity: "Equity",
        currentEarnings: "Current Earnings (unclosed)",
        totalAssets: "Total Assets",
        totalLiabilities: "Total Liabilities",
        totalEquity: "Total Equity",
        revenue: "Revenue",
        expense: "Expense",
        totalRevenue: "Total Revenue",
        totalExpense: "Total Expense",
        netIncome: "Net Income",
        netProfit: "Net Profit",
        netLoss: "Net Loss",
        totalLiabilitiesAndEquity: "Total Liabilities and Equity",
        sourceType: "Source Type",
        netChange: "Net Change",
        partnerNumber: "Partner Number",
        partnerName: "Partner",
        journal: "Journal",
        reference: "Reference",
        account: "Account",
      },
      ledger: {
        accounts: "Accounts",
        accountsRange: "Accounts {from}–{to} of {total}",
        periodMovement: "Period movement",
        signHint:
          "Balances are debit-positive (debit − credit); a credit balance shows as negative.",
      },
      filters: {
        company: "Company",
        allCompanies: "All Companies",
        branch: "Branch",
        allBranches: "All Branches",
        costCenter: "Cost Center",
        allCostCenters: "All Cost Centers",
        project: "Project",
        allProjects: "All Projects",
        currency: "Currency",
        allCurrencies: "All Currencies",
        postedOnly: "Posted Only",
        postedOnlyHint: "Uncheck to include draft journal entries.",
        includeOpeningBalance: "Include Opening Balance",
        allAccounts: "All Accounts",
        selectAccountRequired: "Select account",
      },
    },
    shellDescription:
      "This report category is prepared as a foundation. Reports will be generated here once the reporting engine ships.",
    categories: {
      executive: "High-level KPIs across every module, for leadership.",
      sales: "Revenue, orders, and sales performance.",
      crm: "Lead volume, conversion, and follow-up activity.",
      purchasing: "Purchase orders and supplier spend.",
      inventory: "Stock levels, movements, and valuation.",
      finance: "Accounts, cash flow, and financial statements.",
      expenses: "Cost components and product cost trends.",
      customers: "Customer activity and segmentation.",
      suppliers: "Supplier performance and reliability.",
      shipping: "Delivery times and shipment outcomes.",
      custom: "Build your own report (planned).",
      analytics:
        "Cross-module analytical breakdowns by Analytic Plan (project, department, campaign).",
    },
  },
  analyticDistributions: {
    title: "Analytic Distribution",
    description: "Assign one analytic account per plan to this document.",
    save: "Save Distribution",
    saved: "Analytic distribution saved.",
    saveError: "Could not save the analytic distribution.",
    selectAccount: "Select account",
    noPlans: "No active analytic plans yet — add one in Master Data first.",
  },
  settings: {
    documentNumbering: {
      title: "Document Numbering",
      description:
        "Configure how every document type is numbered. Numbers are generated automatically — nobody types one by hand.",
      addNew: "Add Number Series",
      editTitle: "Edit Number Series",
      addTitle: "Add Number Series",
      readOnlyNotice: "You have view-only access. Contact an administrator to change numbering.",
      fields: {
        documentType: "Document Type",
        documentTypeHint:
          "Stable key (UPPER_SNAKE_CASE) a module calls generateNumber() with, e.g. SALES_ORDER. Cannot be changed after creation.",
        label: "Label",
        docCode: "Doc Code ({DOC})",
        padding: "Padding (digits)",
        separator: "Separator",
        template: "Template",
        templateHint: "Build the number from placeholders. Click one to insert it.",
        nextNumber: "Next Number",
        currentNumber: "Current Number",
        resetRules: "Reset Rules",
        yearReset: "Reset yearly",
        monthReset: "Reset monthly",
        dayReset: "Reset daily",
        livePreview: "Live Preview",
      },
      table: {
        label: "Label",
        documentType: "Document Type",
        template: "Template",
        preview: "Next Number Preview",
        padding: "Padding",
        reset: "Reset Rule",
        status: "Status",
      },
      resetRule: {
        year: "Yearly",
        month: "Monthly",
        day: "Daily",
        none: "Never",
      },
      status: {
        active: "Active",
        disabled: "Disabled",
      },
      actions: {
        enable: "Enable",
        disable: "Disable",
        resetNext: "Reset Next Number",
      },
      confirmDisableTitle: "Disable this number series?",
      confirmDisableDescription:
        "Any module that requests a number for this document type will fail until it's re-enabled.",
      confirmResetTitle: "Reset the next number to 1?",
      confirmResetDescription:
        "The next document generated for this type will start over from 1. This cannot be undone.",
      toggleSuccess: "Number series updated.",
      resetSuccess: "Next number reset to 1.",
      noResults: "No document types configured yet.",
    },
    users: {
      title: "Users",
      description:
        "Manage user accounts and their permissions. Job Title is a label only — access is controlled by Permissions.",
      newUser: "New User",
      empty: "No users yet.",
      fields: {
        fullName: "Full Name",
        username: "Username",
        email: "Email",
        mobile: "Mobile",
        password: "Password",
        jobTitle: "Job Title",
        department: "Department",
        branch: "Branch",
        active: "Active",
        status: "Status",
        lastLogin: "Last Login",
      },
      status: {
        active: "Active",
        inactive: "Inactive",
        locked: "Locked",
      },
      actions: {
        lock: "Lock",
        unlock: "Unlock",
        resetPassword: "Reset Password",
        forcePasswordChange: "Force Password Change",
      },
      toasts: {
        locked: "User locked.",
        unlocked: "User unlocked.",
        created: "User created.",
        saved: "User saved.",
        archived: "User archived.",
        forcePasswordChangeSet: "User must change their password on next login.",
      },
      confirmLockTitle: "Lock this user?",
      confirmUnlockTitle: "Unlock this user?",
      confirmResetPasswordTitle: "Reset this user's password?",
      confirmResetPasswordDescription:
        "A new temporary password will be generated and the user will be required to change it on next login.",
      confirmForcePasswordChangeTitle: "Force a password change on next login?",
      confirmArchiveTitle: "Archive this user?",
      passwordDialog: {
        title: "Password created successfully",
        description: "Save or copy the password now to use it at sign-in.",
        copy: "Copy",
        copied: "Copied",
        copyTooltip: "Copy password",
        copyFailed: "Couldn't copy the password. Copy it manually.",
      },
      editor: {
        newTitle: "New User",
        sectionDetails: "Details",
        sectionPermissions: "Permissions",
        copyPermissionsFrom: "Copy Permissions From",
        selectUser: "Select User",
        loadPermissions: "Load Permissions",
        permissionsLoaded: "Permissions loaded — review and save.",
        generatePassword: "Generate password automatically",
        validationRequired: "Full Name, Username, and Email are required.",
        validationDepartment: "Department is required.",
        validationPassword: "Password must be at least 8 characters.",
      },
    },
  },
  products: {
    title: "Products",
    printTitle: "Products List",
    description: "Manage stockable products, services, and consumables.",
    addNew: "Add Product",
    addTitle: "Add Product",
    editTitle: "Edit Product",
    duplicateTitle: "Duplicate Product",
    quickPreview: "Quick Preview",
    tabs: {
      general: "General",
      classification: "Classification",
      sales: "Sales",
      purchasing: "Purchasing",
      inventory: "Inventory",
      cost: "Cost",
      stockMovements: "Stock Movements",
      bom: "Bill of Materials (BOM)",
      variants: "Variants",
      attachments: "Attachments",
    },
    fields: {
      name: "Arabic Name",
      nameEn: "English Name",
      internalName: "Internal Name",
      displayName: "Display Name",
      sku: "Product Code (SKU)",
      skuHint: "Generated automatically after saving.",
      barcode: "Barcode",
      qrCodeValue: "QR Code Value",
      imageUrl: "Image URL",
      description: "Description",
      shortDescription: "Short Description",
      longDescription: "Long Description",
      internalNotes: "Internal Notes",
      tags: "Tags",
      tagsHint: "Comma-separated.",
      type: "Product Type",
      status: "Status",
      category: "Category",
      brand: "Brand",
      unit: "Unit of Measure",
      taxGroup: "Tax Group",
      costCenter: "Cost Center (Analytic Account)",
      salesPrice: "Sales Price",
      salesTaxIncluded: "Tax Included",
      salesDescription: "Sales Description",
      allowDiscount: "Allow Discount",
      availableForSale: "Available For Sale",
      purchasePrice: "Purchase Price",
      preferredSupplier: "Preferred Supplier",
      purchaseDescription: "Purchase Description",
      availableForPurchase: "Available For Purchase",
      trackInventory: "Track Inventory",
      availableForInvestmentOpportunities: "Available for investment opportunities",
      availableForInvestmentOpportunitiesHint:
        "When enabled, this product (if active) appears in the Investment Opportunity product selector. Disabling it later keeps it on existing opportunities but blocks selecting it for new ones.",
      reorderLevel: "Reorder Point",
      reorderQuantity: "Reorder Quantity",
      safetyStock: "Safety Stock",
      minQuantity: "Minimum Quantity",
      maxQuantity: "Maximum Quantity",
      storageLocation: "Storage Location",
      serialNumberTracking: "Serial Number Tracking",
      batchTracking: "Batch Tracking",
      preferredWarehouse: "Preferred Warehouse",
      costingMethod: "Costing Method",
      inventoryTracking: "Inventory Tracking",
      weight: "Weight",
      width: "Width",
      height: "Height",
      length: "Length",
      dimensionsHint: "Required when Track Inventory is on.",
      color: "Color",
      size: "Size",
      variantWeight: "Weight",
      priceAdjustment: "Price Adjustment",
      variantSku: "Variant SKU",
      fileUrl: "File URL",
      fileName: "File Name",
      attachmentDescription: "Description",
      createdBy: "Created By",
      createdAt: "Created Date",
      updatedBy: "Updated By",
      updatedAt: "Updated Date",
    },
    type: {
      PURCHASE_ONLY: "Purchase Only",
      SALES_ONLY: "Sales Only",
      PURCHASE_AND_SALE: "Purchase & Sales",
      MANUFACTURED: "Manufactured Product",
      SERVICE: "Service",
      EXPENSE_ITEM: "Expense Item",
    },
    typeHint: {
      PURCHASE_ONLY: "Enters inventory, never sold.",
      SALES_ONLY: "Sold from stock — not purchased through this system.",
      PURCHASE_AND_SALE: "Normal inventory item — bought and sold.",
      MANUFACTURED: "Sold as one item, built from component products (BOM).",
      SERVICE: "No inventory.",
      EXPENSE_ITEM: "Bought and expensed — not stocked or sold.",
    },
    costingMethod: {
      AVERAGE: "Average Cost",
      FIFO: "FIFO",
      STANDARD: "Standard Cost",
    },
    inventoryTracking: {
      NONE: "None",
      BATCH: "Batch",
      SERIAL: "Serial Number",
    },
    status: {
      DRAFT: "Draft",
      ACTIVE: "Active",
      INACTIVE: "Inactive",
    },
    table: {
      sku: "SKU",
      name: "Name",
      type: "Type",
      category: "Category",
      salesPrice: "Sales Price",
      status: "Status",
    },
    actions: {
      duplicate: "Duplicate",
      addVariant: "Add Variant",
      addAttachment: "Add Attachment",
      deleteVariant: "Remove",
    },
    variantsEmpty: "No variants yet — add one above (e.g. Color: Red, Size: L).",
    attachmentsEmpty: "No attachments yet.",
    noSupplier: "No preferred supplier",
    noCategoryYet: "Create a Category and Unit in Master Data first.",
    addCategory: "Add new category",
    categoryQuickCreate: {
      title: "Quick-create a category",
      description:
        "Create a category without leaving this product. You can add its accounting overrides later in Master Data.",
      success: "Category created and selected.",
      fields: {
        name: "Category name",
        description: "Description",
      },
    },
    saved: "Product saved.",
    createdSuccess: "Product created successfully.",
    archived: "Product archived.",
    restored: "Product restored.",
    variantSaved: "Variant saved.",
    variantRemoved: "Variant removed.",
    attachmentAdded: "Attachment added.",
    createDialog: {
      title: "Add Product",
      description: "Only what's needed to save — everything else can be added after.",
    },
    wizard: {
      steps: {
        basics: "Basics",
        pricing: "Pricing & Commerce",
        inventory: "Inventory",
        review: "Review & Create",
      },
      back: "Back",
      next: "Next",
      createDraft: "Create as Draft",
      createDraftNow: "Create as Draft Now",
      reviewIntro:
        "Review before creating — any field can be completed later from the product page.",
      willStartAsDraft:
        'The product will start as a Draft — use the "Activate Product" action later to make it available for sale and purchase.',
      notProvided: "Not provided",
    },
    detail: {
      activate: "Activate Product",
      activated: "Product activated.",
      draftExcludedHint:
        "This product is a Draft — it will not appear in sales/purchase lists or Google Sheets reference data until activated.",
      sections: {
        basics: "Basics",
        status: "Status",
      },
      tabs: {
        overview: "Overview",
        pricing: "Pricing",
        inventory: "Inventory",
        activity: "Activity",
      },
    },
    successDialog: {
      title: "Product created successfully",
      description: "What would you like to do next?",
      addAnother: "Add another product",
      openProduct: "Open this product",
      returnToList: "Return to product list",
      createOpeningBalance: "Create opening inventory balance",
    },
    openingBalance: {
      title: "Opening Inventory Balance",
      description: "Record the starting quantity for this product in a warehouse.",
      warehouse: "Warehouse",
      quantity: "Quantity",
      averageCost: "Average Cost",
      notes: "Notes",
      submit: "Save Opening Balance",
      success: "Opening balance recorded.",
    },
    kit: {
      title: "Kit Components (BOM)",
      component: "Component Product",
      quantity: "Quantity",
      addComponent: "Add Component",
      removeComponent: "Remove",
      componentsEmpty: "No components yet — add the products this Kit is built from.",
      componentSaved: "Component saved.",
      componentRemoved: "Component removed.",
    },
    inventory: {
      stockSummary: "Stock Summary",
      incoming: "Incoming",
      outgoing: "Outgoing",
      status: "Inventory Status",
      inStock: "In Stock",
      outOfStock: "Out of Stock",
    },
    cost: {
      currentCost: "Current Cost",
      lastCostUpdate: "Last Cost Update",
      methodHint: "Architecture only — no automatic cost calculation yet.",
      architectureHint:
        "The Cost Engine records costs; it does not calculate them yet. Standard/Average/FIFO math and cost allocation come with a future Purchasing-driven pass.",
    },
    stockMovements: {
      reference: "Reference",
      before: "Before",
      cost: "Cost",
      empty: "No stock movements recorded for this product yet.",
    },
    typeChangeWarningTitle: "Change Product Operation Type?",
    typeChangeWarningDescription:
      "Switching type may hide some tabs and fields. Existing data is kept and reappears if you switch back.",
  },
  inventory: {
    movements: {
      description:
        "Complete, read-only audit trail of every stock movement — never edited or deleted.",
      sourceDocumentsFilter: "Source documents ({count})",
    },
    filters: {
      allTypes: "All Types",
    },
    stock: {
      description:
        "Every inventory product's current quantity, reserved quantity, cost, and value.",
    },
    fields: {
      movementNumber: "Movement No.",
      type: "Type",
      product: "Product",
      warehouse: "Warehouse",
      quantity: "Quantity",
      balanceAfter: "Balance After",
      date: "Date",
      onHand: "On Hand",
      reserved: "Reserved",
      available: "Available",
      averageCost: "Average Cost",
      lastCost: "Last Cost",
      stockValue: "Stock Value",
      lastMovement: "Last Movement",
      reference: "Reference",
      runningCost: "Running Cost",
    },
    movementType: {
      OPENING_BALANCE: "Opening Balance",
      ADJUSTMENT: "Inventory Adjustment",
      TRANSFER: "Transfer",
      RESERVATION: "Reservation",
      RESERVATION_RELEASE: "Reservation Release",
      DAMAGE: "Damage",
      EXPIRED: "Expired",
      ASSEMBLY: "Assembly",
      DISASSEMBLY: "Disassembly",
      PRODUCTION: "Production",
      PURCHASE_RECEIPT: "Purchase Receipt",
      PURCHASE_RETURN: "Purchase Return",
      SALES_DELIVERY: "Sales Delivery",
      SALES_RETURN: "Sales Return",
      PRODUCTION_CONSUMPTION: "Production Consumption",
      PRODUCTION_OUTPUT: "Production Output",
      PHYSICAL_COUNT: "Physical Count",
    },
    valuationMethod: {
      title: "Inventory Valuation Method",
      description:
        "System-wide setting. Average Cost is the default — FIFO calculation is architecture only for now.",
      AVERAGE_COST: "Average Cost",
      FIFO: "FIFO",
    },
    createMovement: {
      title: "New Movement",
      description:
        "Record a manual stock movement — Adjustment covers both Receive (positive) and Issue (negative).",
      kind: "Movement Type",
      adjustment: "Adjustment (Receive / Issue)",
      transfer: "Transfer",
      adjustmentHint: "Positive quantity receives stock in, negative issues it out.",
      quantityHint: "e.g. 10 to receive, -10 to issue",
      sourceWarehouse: "Source Warehouse",
      destinationWarehouse: "Destination Warehouse",
      success: "Movement recorded.",
    },
    openingInventory: {
      title: "Opening Inventory",
      description: "Record the starting quantity for a product in a warehouse.",
      success: "Opening inventory recorded.",
    },
    adjustment: {
      title: "Inventory Adjustment",
      description:
        "Increase or decrease stock for a product at a warehouse, with a required reason.",
      direction: "Direction",
      increase: "Increase Quantity",
      decrease: "Decrease Quantity",
      reason: "Reason",
      reasonPlaceholder: "Describe the reason",
      reasons: {
        DAMAGED: "Damaged",
        LOST: "Lost",
        FOUND: "Found",
        EXPIRED: "Expired",
        CORRECTION: "Correction",
        OTHER: "Other",
      },
    },
    transfer: {
      title: "Stock Transfer",
      description: "Move one or more products from one warehouse to another.",
      lines: "Products",
      addLine: "Add Product",
      sameWarehouseError: "Source and destination warehouses must be different.",
    },
    physicalCount: {
      title: "Physical Count",
      description: "Count documents comparing system quantity against what was physically counted.",
      countNumber: "Count No.",
      lines: "Lines",
      createTitle: "New Physical Count",
      createDescription:
        "Select a warehouse and the products to count — defaults to every active inventory item.",
      createdSuccess: "Physical count created.",
      productsToCount: "Products to Count",
      systemQuantity: "System Qty.",
      countedQuantity: "Counted Qty.",
      difference: "Difference",
      enterAllHint: "Enter a counted quantity for every line before confirming.",
      confirmCount: "Confirm Count",
      confirmTitle: "Confirm this Physical Count?",
      confirmDescription:
        "This generates real stock adjustment movements for every line with a difference. This cannot be undone.",
      confirmedSuccess: "Physical count confirmed.",
      cancelCount: "Cancel Count",
      cancelTitle: "Cancel this Physical Count?",
      cancelDescription: "The count will be discarded — no stock will be changed.",
      cancelledSuccess: "Physical count cancelled.",
      status: {
        DRAFT: "Draft",
        CONFIRMED: "Confirmed",
        CANCELLED: "Cancelled",
      },
    },
  },
  workflow: {
    actions: {
      title: "Workflow",
    },
    transition: {
      reasonPrompt: "This action requires a reason.",
      reasonPlaceholder: "Enter reason…",
    },
    transitions: {
      title: "Advanced Transition Rules",
      description:
        "Which status a workflow can move to next, and under what conditions. This is advanced configuration behind the Workflow Statuses screen — protected business actions (Lead conversion, payment reconciliation) can never be reassigned to a plain transition here.",
      workflow: "Workflow",
      from: "From",
      to: "To",
      label: "Label",
      businessAction: "Business Action",
      requiresReason: "Requires Reason",
      requiresApproval: "Requires Approval",
      active: "Active",
      inactive: "Inactive",
      protected: "Protected",
      protectedHint:
        "This transition runs through a protected business action and cannot be reconfigured.",
      add: "Add Transition",
      edit: "Edit Transition",
      fromStatus: "From Status",
      toStatus: "To Status",
      labelAr: "Label (Arabic)",
      labelEn: "Label (English)",
      requiredPermission: "Required Permission",
      sortOrder: "Sort Order",
      businessActionNone: "—",
      businessActionLeadConvert: "Lead Conversion",
      businessActionPaymentReconcile: "Payment Reconciliation",
      businessActionShipmentCreate: "Shipment Creation",
      createSuccess: "Transition created.",
      updateSuccess: "Transition updated.",
      deactivateConfirmTitle: "Deactivate this transition?",
      deactivateConfirmDescription:
        "Leads/records currently one step away from this status will no longer see it as an available action.",
      activateSuccess: "Transition activated.",
      deactivateSuccess: "Transition deactivated.",
    },
    approvals: {
      title: "Pending Approvals",
      approve: "Approve",
      reject: "Reject",
      approved: "Approved",
      rejected: "Rejected",
    },
    funnel: {
      title: "Lead Funnel",
      description:
        "Lead lifecycle stages from StatusHistory events — not inferred from current status alone.",
      dateRange: "Date range",
      events: "Status events",
      stages: {
        CREATED: "Lead created",
        ASSIGNED: "Assigned",
        IN_PROGRESS: "In progress",
        FOLLOW_UP: "Follow-up",
        QUALIFIED: "Qualified",
        CONVERTED: "Converted",
        ORDER: "Order",
        PAID: "Paid",
        SHIPPED: "Shipped",
        DELIVERED: "Delivered",
        NEW: "New",
        CONTACTED: "Contacted",
        LOST: "Lost",
        DISQUALIFIED: "Disqualified",
      },
    },
    fields: {
      storeOrder: "Store Order",
    },
  },
  crm: {
    leads: {
      title: "Leads",
      description: "CRM prospects — converted to a Store Order only when qualified.",
      searchPlaceholder: "Lead #, name, phone",
      addNew: "New Lead",
      view: "View",
      assign: "Assign",
      fields: {
        leadNumber: "Lead No.",
        customerName: "Customer Name",
        mobileNumber: "Phone",
        customer: "Customer",
        quantity: "Quantity",
        assignedTo: "Assigned To",
        createdAt: "Created Date",
        country: "Country",
        city: "City",
        address: "Detailed Address",
        currency: "Currency",
        externalOrderId: "External Order ID",
        source: "Source",
        product: "Product",
        paidAmount: "Paid Amount",
        nextFollowUp: "Next Follow-up",
        classification: "Lead Classification",
        noPurchaseReason: "No Purchase Reason",
      },
      sections: {
        general: "Customer Information",
        assignment: "Assignment",
        timeline: "Timeline",
        notes: "Notes",
        followUps: "Follow-ups",
      },
      createDialog: {
        title: "Add New",
        description: "Enter the minimum required now — you can complete the rest later.",
        modeLead: "Lead",
        modeLeadHint: "Just name, phone, and country — complete the rest later.",
        modeOrder: "Order",
        modeOrderHint: "A complete order: address, product, and paid amount are required.",
        saveAsLead: "Save as Lead",
        saveAsOrder: "Save as Order",
        productHelper: "The product the customer ordered.",
        paidAmountHelper:
          "The amount the customer actually paid — this creates a payment record automatically.",
        quantityHelper: "Quantity — defaults to 1 if not specified.",
        currencyHelper: "Derived automatically from the selected country if not specified.",
        validation: {
          customerName: "Customer name is required",
          country: "Country is required",
          address: "Address is required",
          paidAmount: "Paid amount is required",
        },
      },
      status: {
        NEW: "New",
        UNDER_FOLLOW_UP: "Under Follow-up",
        PAID: "Paid",
        ARCHIVED: "Archived",
      },
      source: {
        MANUAL: "Manual",
        EXCEL: "Import",
        GOOGLE_SHEETS: "Google Sheets",
      },
      possibleDuplicate: "Possible Duplicate",
      noCustomerLinked: "No customer linked yet.",
      ownership: {
        assigned: "Assigned",
      },
      actions: {
        addFollowUp: "Add Follow-up",
        convert: "Convert to Order",
        reopen: "Reopen",
        startFollowUp: "Start Follow-up",
        markPaid: "Mark Paid",
        archive: "Archive",
        assign: "Assign",
        addNote: "Add Note",
        reassign: "Transfer to employee",
        closeWithoutPurchase: "Close without purchase",
        transfer: "Transfer to employee",
        exportSelected: "Export",
      },
      toasts: {
        created: "Customer added",
      },
      assignDialog: {
        title: "Assign Leads",
        description: "Only active employees with permission to handle Leads can be selected.",
        selectEmployee: "Select Employee",
        balanced: "Balanced distribution (auto-pick)",
        confirm: "Assign",
        success: "Lead assigned.",
        noEligibleEmployees:
          'No eligible active employee found — grant the "Leads" permission to at least one active user.',
      },
      filters: {
        employee: "Assigned Employee",
        followUp: "Follow-up",
        followUpAll: "All follow-ups",
        followUpUpcoming: "Upcoming",
        followUpNone: "No follow-up",
      },
      bulkSelection: {
        customCountTitle: "Select a number of leads",
        customCountLabel: "Number of leads",
        customCountHint:
          "The first {count} leads matching the current filters and sort will be selected.",
        customCountConfirm: "Select leads",
        customCountInvalid: "Enter a whole number greater than zero.",
      },
      bulkStatus: {
        action: "Change Status",
        dialogTitle: "Change Status",
        selectedCount: "{count} leads selected",
        newStatusLabel: "New status",
        newStatusPlaceholder: "Select a status",
        reasonLabel: "Reason (optional)",
        reasonPlaceholder: "Add a note for this change...",
        submit: "Change Status",
        confirmTitle: "Confirm status change",
        confirmDescription: "Change {count} selected leads to {status}?",
        confirmAction: "Confirm",
        successMessage: "{count} leads updated successfully.",
        allFailedMessage:
          "Could not change status for {count} leads — no valid transition from their current status.",
        partialFailureMessage: "{count} skipped — no valid transition from their current status.",
      },
      distribution: {
        title: "Distribute Leads",
        description: "Automatic Round Robin or manual assignment for unowned Leads.",
        action: "Distribute Leads",
        unassigned: "Unassigned",
        tabs: {
          continuous: "Continuous",
          hours: "24 hours",
          manual: "Manual",
        },
        continuousHint: "Every new unowned Lead is assigned automatically until you turn this off.",
        hoursHint: "Automatic assignment stays on for 24 hours from activation, then stops.",
        manualHint:
          "Assign selected Leads, or the first N unassigned Leads matching the current filters.",
        status: "Status",
        active: "Active",
        inactive: "Off",
        strategy: "Strategy",
        roundRobin: "Round Robin",
        eligible: "Eligible sales employees",
        activate: "Activate",
        deactivate: "Turn off",
        activate24h: "Activate for 24 hours",
        startedAt: "Started",
        expiresAt: "Expires",
        remaining: "Remaining",
        hours: "hours",
        selectedCount: "Selected Leads",
        customN: "Custom N",
        customNPlaceholder: "e.g. 50",
        reason: "Reason",
        activated: "Continuous distribution is on.",
        activated24h: "24-hour distribution is on.",
        activatedManual:
          "Manual distribution is selected. Imported batches stay unassigned until you release them.",
        deactivated: "Automatic distribution is off.",
        paused: "Distribution Paused",
        pausedToast: "Lead distribution paused.",
        pausedHint: "New imported batches stay unassigned until you choose a distribution mode.",
        pause: "Pause Distribution",
        start: "Start Distribution",
        running: "Distribution is active",
        states: {
          continuous: "Continuous",
          hours: "Every 24 Hours",
          manual: "Manual",
          paused: "Paused",
        },
        heldCount: "{count} held",
        pendingCount: "{count} pending",
        lastRun: "Last run",
        lastRunAssigned: "{count} assigned",
        failureReason: "Issue",
        noEligible: "No eligible employees",
        heldTitle: "Batches waiting for distribution",
        heldHint:
          "Imported while distribution was paused, or blocked because no employee qualified. Activating Continuous or 24-hour mode distributes pending unowned leads.",
        latencyHint:
          "Assignment runs immediately on create and on activate (typically under 2 seconds).",
        noBatch: "No batch id",
        releaseAuto: "Distribute this batch",
        released: "Held leads released for distribution.",
        manualNeedSelection: "Select Leads or enter N.",
        unsavedDiscarded: "Changes were not saved.",
      },
      followUp: {
        title: "Add Follow-up",
        description: "Record the contact outcome and the next follow-up time.",
        type: "Follow-up type",
        outcome: "Contact outcome",
        nextAt: "Next follow-up",
        note: "Notes",
        saved: "Follow-up recorded.",
        overdue: "Overdue",
        today: "Today",
        tomorrow: "Tomorrow",
        outcomes: {
          answered: "Answered",
          noAnswer: "No answer",
          interested: "Interested",
          callback: "Call back later",
          wrongNumber: "Wrong number",
          notInterested: "Not interested",
        },
      },
      convert: {
        title: "Convert to Order",
        description: "Create a Store Order from this Lead with products, payment, and shipping.",
        cta: "Convert to Order",
        convertedTo: "Converted to order",
        success: "Order created",
        confirmCreate: "Confirm and create order",
        backToEdit: "Back to edit",
        unitPrice: "Selling price",
        agreedAmount: "Agreed amount",
        paymentType: "Settlement mode",
        prepaid: "Prepaid",
        cod: "Cash on delivery",
        fulfillmentMethod: "Fulfillment",
        shipping: "Shipping",
        pickup: "Pickup from premises",
        paymentMethod: "Payment method",
        amountPaid: "Amount paid",
        paymentReference: "Payment reference",
        paymentProof: "Attach payment receipt",
        addProduct: "Add product",
        selectProduct: "Select product",
        quantity: "Quantity",
        orderTotal: "Order total",
        remaining: "Remaining",
        summary: "Order summary",
        sectionCustomer: "Customer",
        sectionProducts: "Products",
        sectionPayment: "Payment / Settlement",
        sectionShipping: "Shipping address",
        sectionAttachments: "Attachments",
        sectionSummary: "Order summary",
        owner: "Sales owner",
        validation: {
          product: "Select the product",
          quantity: "Enter the quantity",
          amount: "Enter the agreed amount (greater than 0) for every product",
          total: "Enter the agreed amount — the order total cannot be 0.00",
          paymentMethod: "Select the payment method",
          shipping: "Enter the shipping address",
        },
      },
      closeWithoutPurchase: {
        title: "Close without purchase",
        description: "The Lead is archived and kept for analytics. A reason is required.",
        reason: "No purchase reason",
        notes: "Notes",
        confirm: "Close Lead",
        success: "Lead closed without purchase.",
      },
      lifecycle: {
        active: "Active",
        converted: "Converted",
        closed: "Closed / Archived",
        all: "All",
      },
      dashboard: {
        newLeads: "New Leads",
        inProgress: "In progress",
        followUp: "Follow-ups",
        dueToday: "Due today",
        overdue: "Overdue",
        converted: "Converted",
        orders: "My orders",
        delivered: "Delivered",
        conversionRate: "Conversion rate",
        ranking: "Your rank this period",
        of: "of",
        today: "Today",
        week: "This week",
        month: "This month",
        teamTitle: "Team performance",
      },
      notesPanel: {
        placeholder: "Write a note…",
        empty: "No notes yet.",
      },
      customerOrders: {
        title: "Orders",
        description: "Every Lead/Order linked to this customer — click a row to open it.",
        empty: "No orders yet for this customer.",
      },
    },
    salesTeams: {
      title: "Sales Teams",
      description: "Working groups under a Department — not Departments themselves.",
      new: "New Sales Team",
      empty: "No sales teams yet.",
      selectManager: "Select team manager",
      noEligibleMembers: "No eligible users found.",
      validationRequired: "Team name, department, and manager are required.",
      archiveTitle: "Archive this sales team?",
      fields: {
        code: "Code",
        name: "Team Name",
        department: "Department",
        manager: "Team Manager",
        members: "Members",
        notes: "Notes",
      },
      editor: {
        newTitle: "New Sales Team",
        sectionDetails: "Team",
      },
      toasts: {
        created: "Sales team created.",
        saved: "Sales team saved.",
        archived: "Sales team archived.",
      },
    },
  },
  partners: {
    title: "Partners",
    description:
      "The unified registry of every counterparty — customers, suppliers, employees, and owners.",
    addNew: "Add Partner",
    viewProfile: "View Profile",
    fields: {
      partnerNumber: "Partner No.",
      name: "Name",
      legalName: "Legal Name",
      commercialName: "Commercial Name",
      entityType: "Entity Type",
      phone: "Phone",
      mobile: "Mobile",
      email: "Email",
      website: "Website",
      taxNumber: "Tax Number",
      commercialRegistration: "Commercial Registration",
      currency: "Currency",
      country: "Country",
      city: "City",
      address: "Address",
      notes: "Notes",
      status: "Status",
      source: "Source",
      createdAt: "Created At",
      roles: "Roles",
      receivableBalance: "Receivable",
      payableBalance: "Payable",
      netExposure: "Net Exposure",
    },
    entityType: {
      PERSON: "Person",
      ORGANIZATION: "Organization",
    },
    roles: {
      CUSTOMER: "Customer",
      SUPPLIER: "Supplier",
      EMPLOYEE: "Employee",
      OWNER: "Owner / Partner",
      OTHER: "Other",
    },
    source: {
      MANUAL: "Manual",
      WEBSITE: "Website",
      SALLA: "Salla",
      API: "API",
      IMPORT: "Import",
      GOOGLE_SHEETS: "Google Sheets",
      LEAD_CONVERSION: "Lead Conversion",
      OTHER: "Other",
    },
    sections: {
      general: "General Information",
      contact: "Contact Information",
      commercial: "Commercial Information",
      roles: "Roles",
      customerSettings: "Customer Settings",
      supplierSettings: "Supplier Settings",
      employeeSettings: "Employee Settings",
      addresses: "Addresses",
      notes: "Notes",
    },
    rolesHelp: "To change an existing partner's roles, use its detail page.",
    picker: {
      selectPartner: "Select a partner",
      placeholder: "Search by number, name, phone, or email…",
      noResults: "No matching partners.",
      recent: "Recent",
      quickCreate: "Quick Create Partner",
    },
    quickCreate: {
      title: "Quick Create Partner",
      description: "Create a partner without leaving this document.",
      success: "Partner created and selected.",
      reused: "An existing partner with the same phone/email was selected instead.",
    },
    profile: {
      sectionsTab: {
        overview: "Overview",
        roles: "Roles",
        receivable: "Receivable",
        payable: "Payable",
        statement: "Statement",
        documents: "Documents",
        information: "Information",
      },
      addRole: "Add Role",
      removeRole: "Remove Role",
      netExposure: "Net Exposure",
    },
    statement: {
      title: "Partner Statement",
      description: "Posted movements against this partner from the ledger.",
      selectPartner: "Select a partner to view its statement",
      viewAll: "All Activity",
      viewReceivable: "Receivable",
      viewPayable: "Payable",
      netExposure: "Net Exposure (analytical only)",
      columns: {
        date: "Date",
        document: "Document",
        type: "Type",
        description: "Description",
        debit: "Debit",
        credit: "Credit",
        balance: "Balance",
      },
      noPartnerSelected: "Select a partner to view its statement.",
      empty: "No movements in this range.",
    },
    duplicateWarning: "A possible matching partner already exists.",
  },
  sales: {
    customers: {
      title: "Customers",
      description: "Manage the commercial accounts every Sales document is billed to.",
      addNew: "Add Customer",
      viewProfile: "View Profile",
      fields: {
        customerNumber: "Customer Code",
        name: "Customer Name",
        commercialName: "Commercial Name",
        phone: "Phone",
        mobile: "Mobile",
        email: "Email",
        website: "Website",
        taxNumber: "Tax Number",
        commercialRegistration: "Commercial Registration",
        customerGroup: "Customer Group",
        currency: "Currency",
        paymentTerm: "Payment Terms",
        creditLimit: "Credit Limit",
        balance: "Current Balance",
        country: "Country",
        city: "City",
        address: "Address",
        notes: "Notes",
        status: "Status",
        source: "Source",
        createdAt: "Created Date",
      },
      sections: {
        general: "General Information",
        contact: "Contact Information",
        commercial: "Commercial Information",
        addresses: "Addresses",
        notes: "Notes",
      },
      source: {
        MANUAL: "Manual",
        WEBSITE: "Website",
        SALLA: "Salla",
        API: "API",
        IMPORT: "Import",
        GOOGLE_SHEETS: "Google Sheets",
        LEAD_CONVERSION: "Lead Conversion",
        OTHER: "Other",
      },
      filters: {
        source: "Source",
        allSources: "All Sources",
      },
      picker: {
        placeholder: "Search by code, name, phone, or email…",
        noResults: "No customers found.",
        recent: "Recent",
        quickCreate: "New customer",
        selectCustomer: "Select Customer",
      },
      quickCreate: {
        title: "Quick Create Customer",
        description: "Create a customer without leaving this document.",
        success: "Customer created and selected.",
        reused: "An existing customer with this phone/email was selected instead.",
      },
      profile: {
        sectionsTab: {
          general: "General",
          commercial: "Commercial",
          addresses: "Addresses",
          contacts: "Contacts",
          activity: "Activity",
          statistics: "Sales Statistics",
          notes: "Notes",
          documents: "Documents",
          quotations: "Quotations",
          orders: "Sales Orders",
          invoices: "Invoices",
          returns: "Returns",
          payments: "Payments",
        },
        statistics: {
          balance: "Current Balance",
          creditLimit: "Credit Limit",
          creditAvailable: "Credit Available",
        },
        payments: {
          paidAmount: "Paid Amount",
        },
        comingSoon: "Coming soon — this tab will show real data once that module is built.",
        print: "Print",
      },
      duplicatePhoneOrEmail: "A customer with this phone or email already exists.",
      toasts: {
        archived: "Customer archived.",
      },
    },
    editor: {
      header: {
        documentDate: "Document Date",
        expectedDate: "Expected Date",
        company: "Company",
        branch: "Branch",
        salesperson: "Salesperson",
        noSalesperson: "Unassigned",
        currency: "Currency",
        baseCurrency: "Base Currency",
        currencyNote:
          "Amounts shown in {code}. Exchange-rate conversion is not yet configured — the Ledger currently records this document's amounts as entered.",
      },
      sections: {
        customer: "Customer",
        documentInfo: "Document Information",
        referenceNumber: "Reference Number",
        productLines: "Product Lines",
        notes: "Notes",
        terms: "Terms",
        attachments: "Attachments",
        attachmentsComingSoon: "Attachments coming soon",
        moreDetails: "More Details",
      },
      grid: {
        product: "Product",
        descriptionPlaceholder: "Add a description…",
        warehouse: "Warehouse",
        quantity: "Quantity",
        unit: "Unit",
        unitPrice: "Unit Price",
        discount: "Disc. %",
        tax: "Tax",
        noTax: "No Tax",
        lineTotal: "Line Total",
        addLine: "Add Line",
        selectProduct: "Select Product",
        selectWarehouse: "Select Warehouse",
        productSearchPlaceholder: "Search by name, SKU, or barcode…",
        warehouseSearchPlaceholder: "Search warehouses…",
        warehouseRequired: "Warehouse is required.",
        lineNote: "Line note",
        lineDetails: "More",
        loadingProducts: "Loading products...",
        noActiveProducts: "No active products",
        noMatchingProducts: "No matching results",
        productsLoadError: "Could not load products",
        agreedAmount: "Agreed amount",
      },
      totals: {
        subtotal: "Subtotal",
        discount: "Discount",
        tax: "Tax",
        shipping: "Shipping",
        grandTotal: "Grand Total",
      },
      sidebar: {
        customerSummary: "Customer Summary",
        financialSummary: "Financial Summary",
        documentSummary: "Document Summary",
        activity: "Activity Timeline",
        approvalTimeline: "Approval Timeline",
      },
    },
    quotations: {
      title: "Quotations",
      description: "Prepare and send price quotes to customers before they become Sales Orders.",
      addNew: "New Quotation",
      editorTitle: "Quotation",
      open: "Open",
      empty: "No quotations yet.",
      fields: {
        number: "Quotation No.",
        customer: "Customer",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allCustomers: "All Customers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        cancel: "Cancel",
        convertToOrder: "Convert to Order",
      },
      toasts: {
        submitted: "Quotation submitted for approval.",
        approved: "Quotation approved.",
        cancelled: "Quotation cancelled.",
        duplicated: "Quotation duplicated as a new draft.",
        archived: "Quotation archived.",
        bulkArchived: "{count} quotation(s) archived.",
        bulkArchiveFailed: "{count} quotation(s) could not be archived.",
      },
      validation: {
        customerRequired: "Select a customer before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this quotation?",
      confirmCancelDescription: "This cannot be undone. The quotation will move to Cancelled.",
      confirmArchiveTitle: "Archive this quotation?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} quotation(s)?",
      },
      convertToOrder: {
        title: "Convert to Sales Order",
        description:
          "Assign a warehouse to every line — quantities can be adjusted before confirming.",
        confirm: "Convert to Order",
        success: "Sales Order created from this quotation.",
      },
    },
    orders: {
      title: "Sales Orders",
      description: "Confirmed customer orders — reserve inventory and prepare for invoicing.",
      addNew: "New Sales Order",
      editorTitle: "Sales Order",
      open: "Open",
      empty: "No sales orders yet.",
      fields: {
        number: "Order No.",
        customer: "Customer",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allCustomers: "All Customers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        confirmed: "Confirmed",
        partiallyDelivered: "Partially Delivered",
        delivered: "Delivered",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        confirm: "Confirm",
        cancel: "Cancel",
        convertToInvoice: "Convert to Invoice",
      },
      toasts: {
        submitted: "Sales Order submitted for approval.",
        approved: "Sales Order approved.",
        confirmed: "Sales Order confirmed — inventory reserved.",
        cancelled: "Sales Order cancelled.",
        duplicated: "Sales Order duplicated as a new draft.",
        archived: "Sales Order archived.",
        bulkArchived: "{count} sales order(s) archived.",
        bulkArchiveFailed: "{count} sales order(s) could not be archived.",
      },
      validation: {
        customerRequired: "Select a customer before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this sales order?",
      confirmCancelDescription: "This cannot be undone. The sales order will move to Cancelled.",
      confirmArchiveTitle: "Archive this sales order?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} sales order(s)?",
      },
      fromQuotation: "From Quotation",
      relatedInvoices: "Sales Invoice(s)",
      convertToInvoice: {
        title: "Convert to Sales Invoice",
        description: "Select which lines to invoice now — the rest can be invoiced later.",
        confirm: "Convert to Invoice",
        success: "Sales Invoice created from this order.",
        remaining: "Remaining",
        noRemainingLines: "Every line on this order has already been fully invoiced.",
        noLinesSelected: "Select at least one line to invoice.",
      },
    },
    invoices: {
      title: "Sales Invoices",
      description: "Bill customers and reduce inventory once confirmed.",
      addNew: "New Sales Invoice",
      editorTitle: "Sales Invoice",
      open: "Open",
      empty: "No sales invoices yet.",
      fields: {
        number: "Invoice No.",
        customer: "Customer",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allCustomers: "All Customers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        confirmed: "Confirmed",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        confirm: "Confirm",
        cancel: "Cancel",
        createReturn: "Create Return",
        receivePayment: "Receive Payment",
      },
      toasts: {
        submitted: "Sales Invoice submitted for approval.",
        approved: "Sales Invoice approved.",
        confirmed: "Sales Invoice confirmed — inventory delivered.",
        cancelled: "Sales Invoice cancelled.",
        duplicated: "Sales Invoice duplicated as a new draft.",
        archived: "Sales Invoice archived.",
        bulkArchived: "{count} sales invoice(s) archived.",
        bulkArchiveFailed: "{count} sales invoice(s) could not be archived.",
      },
      validation: {
        customerRequired: "Select a customer before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this sales invoice?",
      confirmCancelDescription: "This cannot be undone. The sales invoice will move to Cancelled.",
      confirmArchiveTitle: "Archive this sales invoice?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} sales invoice(s)?",
      },
      fromOrder: "From Sales Order",
      relatedReceipts: "Customer Receipt Voucher(s)",
      relatedReturns: "Sales Return(s)",
      relatedJournalEntry: "Journal Entry",
      createReturn: {
        title: "Create Sales Return",
        description: "Select which invoiced lines the customer is returning.",
        confirm: "Create Return",
        success: "Sales Return created from this invoice.",
        invoicedQuantity: "Invoiced",
        alreadyReturned: "Already returned",
        fullyReturned: "Fully returned",
        noLinesSelected: "Select at least one line to return.",
      },
    },
    returns: {
      title: "Sales Returns",
      description: "Process customer returns and increase inventory once confirmed.",
      addNew: "New Sales Return",
      editorTitle: "Sales Return",
      open: "Open",
      empty: "No sales returns yet.",
      fields: {
        number: "Return No.",
        customer: "Customer",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allCustomers: "All Customers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        confirmed: "Confirmed",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        confirm: "Confirm",
        cancel: "Cancel",
        refund: "Refund",
      },
      toasts: {
        submitted: "Sales Return submitted for approval.",
        approved: "Sales Return approved.",
        confirmed: "Sales Return confirmed — inventory increased.",
        cancelled: "Sales Return cancelled.",
        duplicated: "Sales Return duplicated as a new draft.",
        duplicateMissingInvoice: "This return has no source invoice and can't be duplicated.",
        archived: "Sales Return archived.",
        refunded: "Refund {number} confirmed and posted.",
        bulkArchived: "{count} sales return(s) archived.",
        bulkArchiveFailed: "{count} sales return(s) could not be archived.",
      },
      validation: {
        customerRequired: "Select a customer before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this sales return?",
      confirmCancelDescription: "This cannot be undone. The sales return will move to Cancelled.",
      confirmArchiveTitle: "Archive this sales return?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} sales return(s)?",
      },
      fromInvoice: "From Invoice",
      relatedJournalEntry: "Journal Entry",
    },
    receipts: {
      title: "Customer Receipt Vouchers",
      description:
        "Record money received from customers and settle it against open Sales Invoices.",
      addNew: "New Receipt Voucher",
      editorTitle: "Customer Receipt Voucher",
      open: "Open",
      empty: "No Customer Receipt Vouchers yet.",
      partyLabel: "Customer",
      allocatedInvoices: "Allocated Invoice(s)",
      relatedJournalEntry: "Journal Entry",
      fields: {
        number: "Voucher No.",
        customer: "Customer",
        amount: "Amount",
        reference: "Reference",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allCustomers: "All Customers",
      },
    },
    refunds: {
      title: "Customer Refunds",
      description: "Money paid back to customers against the credit of posted Sales Returns.",
      editorTitle: "Customer Refund",
      empty: "No customer refunds yet.",
      partyLabel: "Customer",
      tabs: {
        receipts: "Receipts",
        refunds: "Refunds",
      },
      fields: {
        number: "Refund No.",
        salesReturn: "Sales Return",
      },
      dialog: {
        title: "Refund to customer",
        description: "Pay the customer back from cash or bank against this return's credit.",
        returnTotal: "Return total",
        refunded: "Already refunded",
        customerCredit: "Customer credit balance",
        refundable: "Refundable now",
        amount: "Refund amount",
        paidFrom: "Paid from (cash / bank)",
        nothingToRefund:
          "Nothing left to refund on this return — it is fully refunded or the customer holds no credit.",
        amountExceeds: "The refund cannot exceed the refundable amount ({amount}).",
        submit: "Confirm refund",
      },
    },
  },
  carrierReconciliation: {
    title: "Carrier Reconciliation",
    description:
      "Reconcile carrier-invoiced actual shipping charges against Shipment Attempts. A CONFIRMED charge becomes the authoritative shipping cost in Order Economics.",
    empty: "No carrier charges imported yet.",
    searchPlaceholder: "Search carrier, reference, tracking number…",
    fields: {
      carrier: "Carrier",
      reference: "Reference / Tracking",
      order: "Order / Attempt",
      amount: "Charge Amount",
      chargeDate: "Charge Date",
    },
    state: {
      ALL: "All States",
      UNMATCHED: "Unmatched",
      MATCHED: "Matched",
      REVIEW_REQUIRED: "Review Required",
      CONFIRMED: "Confirmed",
    },
    actions: {
      match: "Match to Shipment",
      confirm: "Confirm as Actual Cost",
      unmatch: "Unmatch",
    },
    import: {
      action: "Import Carrier Charges",
      summary:
        "Imported: {matched} matched, {review} need review, {unmatched} unmatched, {duplicate} duplicate.",
      errors: "{count} row(s) failed validation and were skipped.",
    },
    match: {
      title: "Match Carrier Charge",
      orderNumber: "Order Number",
      orderNumberPlaceholder: "Enter the Order Number…",
      noShipments: "No Shipment Attempts found for this Order Number.",
      shipmentAttempt: "Shipment Attempt",
      selectAttempt: "Select an attempt…",
      saved: "Charge matched to the selected Shipment Attempt.",
    },
    confirmDialog: {
      title: "Confirm as Authoritative Actual Cost?",
      description:
        "This becomes the CONFIRMED ACTUAL shipping cost for that Shipment Attempt in Order Economics — it always wins over the operationally-entered cost.",
      saved: "Carrier charge confirmed as the authoritative shipping cost.",
    },
    unmatchDialog: {
      title: "Unmatch this Charge?",
      description:
        "Clears the match (and, if CONFIRMED, removes it as the authoritative shipping cost) — the charge returns to Unmatched.",
      saved: "Carrier charge unmatched.",
    },
  },
  storeOrders: {
    title: "Store Orders",
    description:
      "External store orders — imported or entered manually, matched to Customers by phone, reconciled and shipped.",
    searchPlaceholder: "Order #, name, phone",
    empty: "No store orders yet.",
    open: "Open",
    viewOrder: "View order",
    toasts: {
      archived: "Order archived.",
    },
    lineAmounts: {
      title: "Agreed amounts",
      description:
        "Enter the amounts agreed with the customer for {order}. Allowed only before any invoice or verified payment; every change is logged on the order.",
      quantity: "Qty {quantity}",
      agreedAmount: "Agreed amount",
      total: "Order total",
      totalRequired: "Enter the agreed amount — the order total cannot be 0.00.",
      saved: "Agreed amounts saved for {order} — total {total}.",
      action: "Correct agreed amounts",
    },
    createDialog: {
      trigger: "New Order",
      title: "New Store Order",
      description:
        "Enter a Store Order manually — the customer is matched or created by phone, exactly like Import.",
      success: "Store order created.",
      submit: "Create Order",
      sections: {
        orderInfo: "Order",
        customer: "Customer",
        items: "Line Items",
        payment: "Payment",
        notes: "Notes",
        receipts: "Attachments",
      },
      fields: {
        customer: "Customer",
        customerName: "Customer Name",
        customerEmail: "Email",
        country: "Country",
        city: "City",
        address: "Address",
        currency: "Currency",
        notes: "Notes",
        paymentAmount: "Amount Paid",
        paymentSource: "Payment Source",
        receivingAccount: "Receiving Account",
        senderName: "Sender Name",
        receiptName: "File name",
        receiptUrl: "Attachment URL",
      },
      totals: {
        subtotal: "Subtotal",
        total: "Total",
        paid: "Paid",
        balance: "Balance",
      },
      summary: {
        title: "Order summary",
        product: "Product",
        quantity: "Quantity",
        paymentAwaitingMatch: "Awaiting confirmation",
      },
      items: {
        title: "Line Items",
        add: "Add Row",
        remove: "Remove row",
        required: "Add at least one line item with a product and quantity.",
        lineTotal: "Line Total",
        agreedUnitPrice: "Agreed unit price",
      },
      paymentIncomplete:
        "Complete payment method, receiving account, and sender name to record a payment with the order.",
      receiptIncomplete: "Enter both a file name and URL, or leave the attachment empty.",
      existingCustomer: {
        checking: "Checking for an existing customer...",
        badge: "Existing customer",
        previousOrders: "Has {count} previous order(s)",
        lastOrder: "Last order",
        useData: "Use this customer's data",
        applied: "Existing customer data applied.",
      },
    },
    globalLookup: {
      trigger: "Look up by phone or order number",
      title: "Global Customer / Order Lookup",
      description:
        "Exact lookup by phone number or order number — never a full browse of Customers or Orders.",
      methodPhone: "Phone number",
      methodOrder: "Order number",
      phonePlaceholder: "Enter a phone number",
      orderPlaceholder: "Enter an order number",
      search: "Search",
      searching: "Searching...",
      notFoundCustomer: "No customer matches this number.",
      notFoundOrder: "No order matches this number.",
      error: "Lookup failed. Please try again.",
      customer: {
        totalOrders: "Total previous orders",
        lastOrderDate: "Last order date",
        recentOrders: "Recent orders",
        addNewOrder: "Add a new order for this customer",
      },
      order: {
        orderDate: "Order date",
        products: "Products",
        paymentStatus: "Payment status",
        shippingStatus: "Shipping status",
        readOnlyNotice: "Read-only view via global lookup.",
      },
    },
    filters: {
      paymentStatus: "Payment Status",
      allPaymentStatuses: "All Payment Statuses",
      shippingStage: "Shipping Stage",
      allShippingStages: "All Shipping Stages",
      source: "Source",
      allSources: "All Sources",
    },
    fields: {
      order: "Order",
      internalOrderId: "Order #",
      externalOrderId: "External Order ID",
      customer: "Customer",
      phone: "Phone",
      orderDate: "Order Date",
      payment: "Payment",
      shipping: "Shipping",
      paymentStatus: "Payment Status",
      paymentType: "Payment Type",
      shippingStage: "Shipping Stage",
      total: "Total",
      source: "Source",
      sourceChannel: "Source Channel",
      currency: "Currency",
      employee: "Employee",
      product: "Product",
      quantity: "Qty",
      unitPrice: "Unit price",
      tracking: "Tracking",
    },
    source: {
      MANUAL: "Manual",
      IMPORT: "Import",
    },
    paymentStatus: {
      PAYMENT_PENDING: "Payment Pending",
      AWAITING_RECONCILIATION: "Awaiting Reconciliation",
      AWAITING_COLLECTION: "Awaiting Collection",
      PARTIALLY_PAID: "Partially Paid",
      FULLY_PAID_RECONCILED: "Fully Paid (Reconciled)",
      OVERPAID: "Overpaid",
      UNMATCHED: "Unmatched",
      PAYMENT_REVIEW: "Payment Review",
      explanation: {
        PAYMENT_PENDING: "No payment has been matched to this order yet.",
        PARTIALLY_PAID: "Part of the order total has been paid and matched so far.",
        FULLY_PAID_RECONCILED:
          "The full order amount has been paid and reconciled — this order is eligible to be marked Ready for Shipping.",
        OVERPAID: "The matched payments exceed the order total — review before proceeding.",
        UNMATCHED: "A payment was found but could not be automatically matched to this order.",
        PAYMENT_REVIEW: "This order's payment matching needs manual review.",
      },
    },
    paymentType: {
      PREPAID: "Prepaid",
      CASH_ON_DELIVERY: "Cash on Delivery",
    },
    fulfillmentMethod: {
      SHIPPING: "Shipping",
      PICKUP: "Pickup",
    },
    shippingStage: {
      NOT_READY: "Not Ready",
      READY_FOR_SHIPPING: "Ready for Shipping",
    },
    profitability: {
      viewInCostExplorer: "View in Cost Explorer",
      netRevenue: "Net Revenue",
      cogs: "Product Cost (COGS)",
      grossProfit: "Gross Product Profit",
      grossProductProfit: "Gross Product Profit",
      grossMargin: "Gross Margin",
      directCosts: "Direct Costs",
      shipping: "Shipping",
      fulfillmentCost: "Packaging / Fulfillment",
      paymentFee: "Payment Fee",
      contributionProfit: "Contribution Profit",
      contributionMargin: "Contribution Margin",
      costState: "Cost Completeness",
      shippingAttempts: "Shipping Attempts",
      attempt: "Attempt",
      cost: "Cost",
      itemBreakdown: "Item Breakdown",
      quantity: "Quantity",
      unitCost: "Historical Unit Cost",
      paymentFees: "Payment Fees",
      amount: "Amount",
      fee: "Fee",
      feeSource: "Source",
      feeSourceValues: {
        ACTUAL: "Actual",
        ESTIMATED: "Estimated",
        UNKNOWN: "Unknown",
      },
      costStateValues: {
        COMPLETE: "Complete",
        PARTIAL: "Partial",
        UNKNOWN: "Unknown",
      },
      lossMakingFilter: "Loss Making Only",
      filterCappedNotice:
        "Showing results from the first 500 matching Orders — narrow your filters for a complete count.",
    },
    detail: {
      tabs: {
        overview: "Overview",
        details: "Details",
        activity: "Activity",
        attachments: "Attachments",
        profitability: "Profitability",
      },
      sections: {
        orderSummary: "Order",
        customer: "Customer",
        items: "Items",
        payments: "Payment",
        invoice: "Invoice",
        shipping: "Shipping",
        shippingSummary: "Shipping",
        shipmentHistory: "Shipment History",
        receipts: "Receipts",
        notes: "Notes",
        activity: "Activity",
        assignment: "Assignment",
      },
      items: {
        product: "Product",
        quantity: "Quantity",
        unitPrice: "Unit Price",
      },
      payments: {
        empty: "No payments linked to this order yet.",
        number: "Payment #",
        date: "Date",
        amount: "Amount",
        paid: "Paid",
        remaining: "Remaining",
        recordStatus: {
          PENDING: "Awaiting confirmation",
          MATCHED: "Matched — not posted",
          VERIFIED: "Confirmed & posted",
          REJECTED: "Rejected",
        },
        add: "Add Payment",
        addTitle: "Add Payment",
        addDescription:
          "Records a normal Payment against this order and re-evaluates its reconciliation status.",
        currency: "Currency",
        method: "Payment Method",
        selectMethod: "Select payment method…",
        receivingAccount: "Receiving Account",
        selectAccount: "Select receiving account…",
        reference: "Reference / Transaction ID",
        sender: "Sender Name",
        notes: "Notes",
        receipt: "Payment receipts",
        receipts: "Payment receipts",
        receiptCount: "{count} receipt(s)",
        added: "Payment added.",
        orderTotal: "Order total",
        suggestedAmount: "Suggested amount (outstanding)",
        overpaymentWarning: "Amount exceeds outstanding — it will be recorded as an overpayment.",
        settled: "Paid / Settled",
        fee: "Fee",
        setFee: "Set Fee",
        setFeeTitle: "Record Actual Transaction Fee",
        setFeeDescription:
          "This always supersedes the Payment Source's fee estimate for this payment in Order Economics.",
        feeAmount: "Fee Amount",
        feeSaved: "Transaction fee recorded.",
      },
      invoice: {
        notGenerated: "No invoice has been generated for this order yet.",
        generate: "Generate Invoice",
        generated: "Invoice generated.",
        gatedHint: "Available once the order's payment is Fully Paid (Reconciled).",
      },
      shippingSummary: {
        notReadyHint:
          "Ready for Shipping is only reachable once payment is Fully Paid (Reconciled).",
      },
      shipmentHistory: {
        empty: "No shipment attempts yet.",
        attempt: "Attempt",
      },
      receipts: {
        empty: "No receipts attached yet.",
        fileName: "File Name",
        fileNamePlaceholder: "e.g. receipt-1.pdf",
        url: "File URL (legacy)",
        attach: "Attach",
        attached: "Receipt attached.",
        attachFailed: "Could not attach the file.",
        removed: "Attachment removed.",
        removeFailed: "Could not remove the attachment.",
        downloadFailed: "Could not open the attachment.",
        dropHint: "Drop JPG, PNG, WEBP, or PDF files here, or choose files.",
        chooseFiles: "Upload receipt",
        acceptedTypes: "Accepted: JPG / PNG / WEBP / PDF",
        uploading: "Uploading {name}…",
        uploadSuccess: "Uploaded",
        uploadFailed: "Upload failed",
        retry: "Retry",
        open: "Open",
        download: "Download",
        previewUnavailable: "This file cannot be previewed here. Use download.",
        confirmRemoveTitle: "Remove this attachment?",
        confirmRemoveDescription:
          "The file will be removed from this order. This cannot be undone.",
        errors: {
          empty: "The selected file is empty.",
          type: "Only JPG, PNG, WEBP, and PDF files are allowed.",
          size: "The file is larger than 10 MB.",
        },
      },
      notes: {
        addLabel: "Add a note",
        placeholder: "Write a note…",
        save: "Save Note",
        added: "Note added.",
        empty: "Enter a note before saving.",
        saveFailed: "Could not save the note.",
      },
      paymentDerivedHint:
        "Payment status is calculated from recorded payments and cannot be set manually.",
      activity: {
        showMore: "Show {count} more",
      },
      edit: {
        saved: "Changes saved.",
        assignmentTitle: "Assign employee",
        customerTitle: "Edit contact and address",
        notesTitle: "Edit internal notes",
        shippingTitle: "Edit shipping",
      },
    },
    import: {
      description:
        "Import Store Orders from Excel/CSV or Google Sheets — an existing Customer is matched by phone, never duplicated.",
      typeUnavailable: "The Store Orders import type is not registered yet.",
    },
    needsReview: {
      description:
        "Rows where an existing Customer was matched by phone — confirm to attach the order to that Customer, or reject to discard the row.",
      selectJob: "Select an import job",
      noJobs: "No Store Orders import jobs yet.",
      empty: "No rows need review.",
      matchedCustomer: "Matched Customer",
      reason: "Reason",
      confirm: "Confirm",
      reject: "Reject",
      bulkConfirm: "Confirm Selected",
      bulkReject: "Reject Selected",
      confirmRejectTitle: "Reject this row?",
      confirmRejectDescription:
        "The row will be discarded — no order or customer link will be created.",
      confirmBulkRejectTitle: "Reject {count} selected rows?",
      viewNeedsReview: "Needs Review",
      viewRejected: "Rejected",
      rejectReason: {
        label: "Rejection Reason",
        placeholder: "Select a reason…",
        noteLabel: "Custom Reason",
        notePlaceholder: "Describe the reason…",
        codes: {
          DUPLICATE_ORDER: "Duplicate Order",
          INVALID_PHONE: "Invalid Phone Number",
          INVALID_CUSTOMER_DATA: "Invalid Customer Data",
          MISSING_REQUIRED_DATA: "Missing Required Data",
          CUSTOMER_CANCELLED: "Customer Requested Cancellation",
          INVALID_ORDER: "Invalid Order",
          OTHER: "Other",
        },
      },
      toasts: {
        confirmed: "Row confirmed and attached to the customer.",
        rejected: "Row rejected.",
        bulkConfirmed: "{count} rows confirmed.",
        bulkRejected: "{count} rows rejected.",
        bulkFailed: "{count} rows failed.",
      },
    },
    bulkSelection: {
      customCountTitle: "Select a number of orders",
      customCountLabel: "Number of orders",
      customCountHint:
        "The first {count} orders matching the current filters and sort will be selected.",
      customCountConfirm: "Select orders",
      customCountInvalid: "Enter a whole number greater than zero.",
      customCountPartial: "Selected all available results: {count} orders",
      selectionCleared: "Selection was cleared because the search or filter results changed.",
    },
    bulkShipping: {
      button: "Change Shipping Status",
      dialogTitle: "Change Shipping Status",
      selectedCount: "{count} orders selected",
      newStatusLabel: "New status",
      newStatusPlaceholder: "Select a shipping status…",
      submit: "Change Status",
      confirmTitle: "Confirm shipping status change",
      confirmDescription:
        "Shipping status will be changed for {count} orders to: {status}. Continue?",
      confirmAction: "Confirm Change",
      successMessage: "{count} orders updated successfully",
      failureMessage: "{count} orders failed to update",
    },
  },
  shipping: {
    title: "Shipping",
    description:
      "Every shipment across every Store Order — track, update status, and bulk-update in one flat list.",
    empty: "No shipments yet.",
    open: "Open",
    filters: {
      status: "Status",
      allStatuses: "All Statuses",
      company: "Shipping Company",
      allCompanies: "All Companies",
      country: "Country",
      allCountries: "All Countries",
      source: "Source",
      allSources: "All Sources",
      tracking: "Tracking Number",
      trackingAll: "All",
      trackingHas: "Has tracking number",
      trackingMissing: "Missing tracking number",
      attachment: "Attachments",
      attachmentAll: "All",
      attachmentHas: "Has attachment",
      attachmentMissing: "Missing attachment",
    },
    fields: {
      internalOrderId: "Order #",
      externalOrderId: "External Order ID",
      customer: "Customer",
      shippingCompany: "Shipping Company",
      trackingNumber: "Tracking Number",
      status: "Status",
      shippedAt: "Ship Date",
      attachments: "Attachments",
    },
    quickEdit: {
      selectCompanyPlaceholder: "Select carrier…",
      trackingPlaceholder: "Enter tracking number…",
      trackingEmpty: "No tracking number",
      saving: "Saving…",
      saved: "Saved",
      saveFailed: "Could not save.",
      statusChangeFailed: "Could not change the shipping status.",
      companyChangeFailed: "Could not update the shipping company.",
      trackingChangeFailed: "Could not update the tracking number.",
      noPermission: "You don't have permission for this action.",
      attachmentsTooltip: "Shipping attachments",
      attachmentsTitle: "Shipping Attachments",
      attachmentsEmpty: "No attachments yet.",
      uploadAttachment: "Upload Attachment",
      uploading: "Uploading…",
      uploadFailed: "Could not upload the attachment.",
      downloadFailed: "Could not open the attachment.",
      view: "View",
      remove: "Remove",
      removeFailed: "Could not remove the attachment.",
      removeConfirmTitle: "Remove this attachment?",
      removeConfirmDescription: "This shipping attachment will be permanently removed.",
      acceptedFileTypes: "JPG, PNG, WEBP, or PDF — up to 10 MB.",
    },
    manage: {
      title: "Manage Shipment",
      currentStatus: "Current Status",
      selectCompany: "Select shipping company…",
      trackingNumber: "Tracking Number",
      trackingNumberPlaceholder: "Enter tracking number…",
      shippingCost: "Shipping Cost",
      shippingCostHint: "Feeds Order Economics as this attempt's direct shipping cost.",
      label: "Shipping Label / AWB",
      viewCurrentLabel: "View current label",
      labelAttachHint: "Paste the label/AWB file URL to attach it to this shipment.",
      labelReplaceHint: "Paste a new URL to replace the current label.",
      saved: "Shipment updated.",
      changeStatus: "Shipping Status",
      finalHint:
        "This shipment is in a Final status — it is excluded from future Shipping Sync runs. Choose an Under Sync status to reopen it.",
      reopenConfirmTitle: "Reopen this shipment?",
      reopenConfirmDescription:
        "This shipment is currently Final. Moving it to an Under Sync status will make it eligible for future Shipping Sync runs again.",
      reopenConfirmAction: "Reopen Shipment",
    },
    status: {
      READY_FOR_SHIPPING: "Ready for Shipping",
      LABEL_CREATED: "Label Created",
      SHIPPED: "Shipped",
      OUT_FOR_DELIVERY: "Out for Delivery",
      DELIVERED: "Delivered",
      DELIVERY_FAILED: "Delivery Failed",
      NEEDS_RESHIPMENT: "Needs Reshipment",
    },
    bulk: {
      selectStatus: "Select status…",
      updateStatus: "Update Status",
      confirmTitle: "Update status for {count} selected shipments?",
      confirmDescription:
        'All selected shipments will be set to "{status}". This cannot be undone.',
      success: "{count} shipments updated.",
      partialFailure: "{count} shipments failed to update.",
    },
    import: {
      description:
        "Import shipping status updates (carrier, tracking number, status) for existing orders — a failed delivery can also trigger a reship.",
    },
  },
  purchasing: {
    suppliers: {
      title: "Suppliers",
      description: "Manage the vendor accounts every Purchasing document is billed from.",
      addNew: "Add Supplier",
      viewProfile: "View Profile",
      fields: {
        supplierNumber: "Supplier Code",
        code: "Code",
        name: "Supplier Name",
        commercialName: "Commercial Name",
        phone: "Phone",
        mobile: "Mobile",
        email: "Email",
        website: "Website",
        taxNumber: "Tax Number",
        commercialRegistration: "Commercial Registration",
        currency: "Currency",
        paymentTerm: "Payment Terms",
        supplierGroup: "Supplier Group",
        creditLimit: "Credit Limit",
        country: "Country",
        city: "City",
        address: "Address",
        notes: "Notes",
        status: "Status",
        isPreferred: "Preferred Supplier",
        createdAt: "Created Date",
      },
      sections: {
        general: "General Information",
        contact: "Contact Information",
        commercial: "Commercial Information",
        addresses: "Addresses",
        notes: "Notes",
      },
      filters: {
        allStatuses: "All Statuses",
      },
      picker: {
        placeholder: "Search by code, name, phone, or email…",
        noResults: "No suppliers found.",
        recent: "Recent",
        quickCreate: "New supplier",
        selectSupplier: "Select Supplier",
      },
      quickCreate: {
        title: "Quick Create Supplier",
        description: "Create a supplier without leaving this document.",
        success: "Supplier created and selected.",
        reused: "An existing supplier with this phone/email was selected instead.",
      },
      profile: {
        sectionsTab: {
          general: "General",
          commercial: "Commercial",
          addresses: "Addresses",
          activity: "Activity",
          notes: "Notes",
          payments: "Payments",
        },
        payments: {
          outstandingPayables: "Outstanding Payables",
          paidAmount: "Paid Amount",
        },
        comingSoon: "Coming soon — this tab will show real data once that module is built.",
        print: "Print",
      },
      activate: "Activate",
      toasts: {
        archived: "Supplier archived.",
      },
    },
    editor: {
      sections: {
        supplier: "Supplier",
      },
      sidebar: {
        supplierSummary: "Supplier Summary",
      },
    },
    quotations: {
      title: "Purchase Quotations",
      description:
        "Request and compare price quotes from suppliers before they become Purchase Orders.",
      addNew: "New Purchase Quotation",
      editorTitle: "Purchase Quotation",
      open: "Open",
      empty: "No purchase quotations yet.",
      fields: {
        number: "Quotation No.",
        supplier: "Supplier",
        purchaseType: "Purchase Type",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allSuppliers: "All Suppliers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        cancel: "Cancel",
        convertToOrder: "Convert to Order",
      },
      toasts: {
        submitted: "Purchase Quotation submitted for approval.",
        approved: "Purchase Quotation approved.",
        cancelled: "Purchase Quotation cancelled.",
        duplicated: "Purchase Quotation duplicated as a new draft.",
        archived: "Purchase Quotation archived.",
        bulkArchived: "{count} purchase quotation(s) archived.",
        bulkArchiveFailed: "{count} purchase quotation(s) could not be archived.",
      },
      validation: {
        supplierRequired: "Select a supplier before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this purchase quotation?",
      confirmCancelDescription: "This cannot be undone. The quotation will move to Cancelled.",
      confirmArchiveTitle: "Archive this purchase quotation?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} purchase quotation(s)?",
      },
      convertToOrder: {
        title: "Convert to Purchase Order",
        description: "The whole quotation converts as-is into a new Purchase Order.",
        confirm: "Convert to Order",
        success: "Purchase Order created from this quotation.",
      },
    },
    orders: {
      title: "Purchase Orders",
      description:
        "Agreements to buy from suppliers — never touches inventory, cost, or accounting on their own.",
      addNew: "New Purchase Order",
      editorTitle: "Purchase Order",
      open: "Open",
      empty: "No purchase orders yet.",
      fields: {
        number: "PO No.",
        supplier: "Supplier",
        purchaseType: "Purchase Type",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allSuppliers: "All Suppliers",
      },
      status: {
        draft: "Draft",
        approved: "Approved",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        approve: "Approve",
        cancel: "Cancel",
        convertToInvoice: "Convert to Invoice",
      },
      toasts: {
        approved: "Purchase Order approved.",
        cancelled: "Purchase Order cancelled.",
        archived: "Purchase Order archived.",
        duplicated: "Purchase Order duplicated as a new draft.",
        bulkArchived: "{count} purchase order(s) archived.",
        bulkArchiveFailed: "{count} purchase order(s) could not be archived.",
      },
      validation: {
        supplierRequired: "Select a supplier before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this purchase order?",
      confirmCancelDescription: "This cannot be undone. The purchase order will move to Cancelled.",
      confirmArchiveTitle: "Archive this purchase order?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} purchase order(s)?",
      },
      fromQuotation: "From Purchase Quotation",
      relatedInvoices: "Purchase Invoice(s)",
      convertToInvoice: {
        title: "Goods Receipt — Convert to Invoice",
        description:
          "Every line receives at its ordered quantity. Choose the destination warehouse.",
        warehouse: "Receiving Warehouse",
        confirm: "Convert to Invoice",
        success: "Purchase Invoice created from this order.",
      },
    },
    invoices: {
      title: "Purchase Invoices",
      description: "Goods Receipt documents — confirming one increases inventory.",
      addNew: "New Purchase Invoice",
      editorTitle: "Purchase Invoice",
      open: "Open",
      empty: "No purchase invoices yet.",
      fields: {
        number: "Invoice No.",
        supplier: "Supplier",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allSuppliers: "All Suppliers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        confirmed: "Confirmed",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        confirm: "Confirm",
        cancel: "Cancel",
        createReturn: "Create Return",
        recordPayment: "Record Payment",
      },
      toasts: {
        submitted: "Purchase Invoice submitted for approval.",
        approved: "Purchase Invoice approved.",
        confirmed: "Purchase Invoice confirmed — inventory received.",
        cancelled: "Purchase Invoice cancelled.",
        archived: "Purchase Invoice archived.",
        duplicated: "Purchase Invoice duplicated as a new draft.",
        bulkArchived: "{count} purchase invoice(s) archived.",
        bulkArchiveFailed: "{count} purchase invoice(s) could not be archived.",
      },
      validation: {
        supplierRequired: "Select a supplier before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this purchase invoice?",
      confirmCancelDescription:
        "This cannot be undone. The purchase invoice will move to Cancelled.",
      confirmArchiveTitle: "Archive this purchase invoice?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} purchase invoice(s)?",
      },
      fromOrder: "From Purchase Order",
      relatedPayments: "Supplier Payment Voucher(s)",
      relatedReturns: "Purchase Return(s)",
      relatedJournalEntry: "Journal Entry",
      createReturn: {
        title: "Create Purchase Return",
        description: "Select which received lines are going back to the supplier.",
        confirm: "Create Return",
        success: "Purchase Return created from this invoice.",
        receivedQuantity: "Received",
        alreadyReturned: "Already returned",
        fullyReturned: "Fully returned",
        noLinesSelected: "Select at least one line to return.",
      },
    },
    returns: {
      title: "Purchase Returns",
      description: "Goods going back to suppliers — confirming one decreases inventory.",
      addNew: "New Purchase Return",
      editorTitle: "Purchase Return",
      open: "Open",
      empty: "No purchase returns yet.",
      fields: {
        number: "Return No.",
        supplier: "Supplier",
        reference: "Reference",
        grandTotal: "Grand Total",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allSuppliers: "All Suppliers",
      },
      status: {
        draft: "Draft",
        submitted: "Submitted",
        approved: "Approved",
        confirmed: "Confirmed",
        cancelled: "Cancelled",
        closed: "Closed",
      },
      actions: {
        submit: "Submit",
        approve: "Approve",
        confirm: "Confirm",
        cancel: "Cancel",
      },
      toasts: {
        submitted: "Purchase Return submitted for approval.",
        approved: "Purchase Return approved.",
        confirmed: "Purchase Return confirmed — inventory decreased.",
        cancelled: "Purchase Return cancelled.",
        archived: "Purchase Return archived.",
        bulkArchived: "{count} purchase return(s) archived.",
        bulkArchiveFailed: "{count} purchase return(s) could not be archived.",
      },
      validation: {
        supplierRequired: "Select a supplier before saving.",
        productRequired: "Add at least one product line before saving.",
        quantityPositive: "Every line's quantity must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this purchase return?",
      confirmCancelDescription:
        "This cannot be undone. The purchase return will move to Cancelled.",
      confirmArchiveTitle: "Archive this purchase return?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      bulk: {
        archiveConfirmTitle: "Archive {count} purchase return(s)?",
      },
      fromInvoice: "From Invoice",
      relatedJournalEntry: "Journal Entry",
    },
    landedCost: {
      title: "Landed Cost",
      description:
        "Attach freight, customs, and clearing costs to a received Purchase Invoice and capitalize them into inventory.",
      addNew: "New Landed Cost",
      editorTitle: "Landed Cost",
      open: "Open",
      empty: "No Landed Cost documents yet.",
      fields: {
        number: "Document No.",
        purchaseInvoice: "Purchase Invoice",
        provider: "Freight / Clearing Provider",
        currency: "Currency",
        reference: "Reference",
        date: "Date",
        allocationMethod: "Allocation Method",
        netTotal: "Net Total",
        taxTotal: "VAT Total",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
      },
      allocationMethods: {
        BY_QUANTITY: "By Quantity",
        BY_COST: "By Purchase Value",
      },
      status: {
        draft: "Draft",
        approved: "Approved",
        posted: "Posted",
        cancelled: "Cancelled",
      },
      actions: {
        approve: "Approve",
        post: "Post",
        cancel: "Cancel",
        addLine: "Add Line",
        removeLine: "Remove",
        previewAllocation: "Preview Allocation",
        save: "Save",
      },
      toasts: {
        created: "Landed Cost document created.",
        saved: "Landed Cost document saved.",
        approved: "Landed Cost approved — allocation locked.",
        posted: "Landed Cost posted — capitalized into inventory.",
        cancelled: "Landed Cost cancelled.",
      },
      validation: {
        purchaseInvoiceRequired: "Select a confirmed Purchase Invoice.",
        currencyRequired: "Select a currency.",
        lineRequired: "Add at least one cost line.",
        lineCostComponentRequired: "Every line needs a Cost Category.",
        lineAmountPositive: "Every line's net amount must be greater than zero.",
      },
      confirmCancelTitle: "Cancel this Landed Cost document?",
      confirmCancelDescription: "This cannot be undone. The document will move to Cancelled.",
      purchaseInvoicePicker: {
        placeholder: "Select a confirmed Purchase Invoice…",
        searchPlaceholder: "Search invoice number or supplier…",
        noResults: "No confirmed Purchase Invoices found.",
        helperText:
          "Only CONFIRMED Purchase Invoices — the actual receipt event — can receive Landed Cost.",
      },
      lines: {
        title: "Cost Lines",
        costComponent: "Cost Category",
        description: "Description",
        netAmount: "Net Amount",
        tax: "VAT",
        taxAmount: "VAT Amount",
      },
      allocationPreview: {
        title: "Allocation Preview",
        product: "Product",
        quantity: "Quantity",
        purchaseValue: "Purchase Value",
        allocatedAmount: "Allocated Amount",
        total: "Total",
        difference: "Difference",
        reconciled: "Allocation reconciles exactly — ready to approve.",
        notReconciled: "Allocation does not reconcile yet.",
      },
      relatedPurchaseInvoice: "Purchase Invoice",
      relatedJournalEntry: "Journal Entry",
    },
    payments: {
      title: "Supplier Payment Vouchers",
      description: "Record money paid to suppliers and settle it against open Purchase Invoices.",
      addNew: "New Payment Voucher",
      editorTitle: "Supplier Payment Voucher",
      open: "Open",
      empty: "No Supplier Payment Vouchers yet.",
      partyLabel: "Supplier",
      allocatedInvoices: "Allocated Invoice(s)",
      relatedJournalEntry: "Journal Entry",
      fields: {
        number: "Voucher No.",
        supplier: "Supplier",
        amount: "Amount",
        reference: "Reference",
        date: "Date",
        createdBy: "Created By",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        allSuppliers: "All Suppliers",
      },
    },
  },
  costExplorer: {
    title: "Cost Explorer",
    description:
      "The per-product cost ledger — every change to a product's inventory cost, with what caused it.",
    productPicker: {
      placeholder: "Select a product…",
      searchPlaceholder: "Search SKU or name…",
      noResults: "No inventory products found.",
    },
    empty: "Select a product to view its cost history.",
    emptyHistory: "No cost history recorded for this product yet.",
    currentCost: "Current Cost",
    onHand: "On Hand",
    stockValue: "Stock Value",
    fields: {
      date: "Date",
      previousCost: "Previous Cost",
      newCost: "New Cost",
      delta: "Change",
      reason: "Reason",
      source: "Source",
    },
    source: {
      PURCHASE_INVOICE: "Purchase Invoice",
      LANDED_COST: "Landed Cost",
      PRODUCT_COST: "Manual Entry",
      SALES_RETURN: "Sales Return",
    },
    tabs: {
      product: "Product",
      order: "Order",
      analytics: "Profitability Analytics",
      pnl: "Management P&L",
    },
    pnl: {
      title: "Management P&L",
      revenue: "Revenue",
      cogs: "Cost of Goods Sold",
      grossProfit: "Gross Profit",
      directCosts: "Direct Costs",
      directCostsShipping: "Shipping",
      directCostsPaymentFees: "Payment Fees",
      directCostsFulfillment: "Fulfillment",
      contributionProfit: "Contribution Profit",
      operatingExpenses: "Operating Expenses",
      operatingProfit: "Operating Profit",
      glReconciliation: "GL Reconciliation",
      revenueFromGl: "Revenue (GL)",
      expenseFromGl: "Total Expense (GL)",
      netProfitFromGl: "Net Profit (GL)",
      operatingExpenseAccounts: "Operating Expense Accounts",
      amount: "Amount",
      coverage: "Cost Data Coverage",
      coverageOrders: "Orders in Scope",
      truncatedWarning:
        "This range has more Orders than can be shown at once — narrow the date range for a complete picture.",
    },
    analytics: {
      dimensions: {
        PRODUCT: "Product",
        ORDER: "Order",
        CUSTOMER: "Customer",
        EMPLOYEE: "Employee",
        CHANNEL: "Channel",
        COUNTRY: "Country",
        PERIOD: "Period",
      },
      orderCount: "Orders",
      grossProfit: "Gross Profit",
    },
    order: {
      searchPlaceholder: "Enter Order Number…",
      empty: "Enter an Order Number to trace its cost breakdown.",
      notFound: "No Order found with that number.",
      loadFailed: "Failed to load this Order's cost breakdown.",
      viewOrder: "View Order",
      viewProductCost: "View Product Cost",
      productCogs: "Product Cost of Goods Sold",
      operationalCost: "Operational Cost",
      confirmedCarrierCost: "Confirmed Carrier Cost",
      costSource: "Source",
      costSourceValues: {
        CONFIRMED_ACTUAL: "Confirmed (Carrier)",
        ACTUAL: "Actual (Operational)",
        UNKNOWN: "Unknown",
      },
      fulfillmentRule: "Rule",
    },
  },
  expensesProductCost: {
    title: "Product Cost",
    description:
      "Current inventory cost per product — read-only, sourced from the valuation engine.",
    empty: "Select a product to view its cost.",
    viewFullHistory: "View full cost history",
    fields: {
      averageCost: "Average Cost",
      onHand: "On Hand",
      stockValue: "Stock Value",
      lastMovement: "Last Movement",
    },
    actions: {
      recordCost: "Record Cost",
    },
    recordDialog: {
      title: "Record Cost",
      description:
        "Manual exception path — overrides the current cost directly. Prefer a Purchase Invoice or Landed Cost document whenever possible.",
      cost: "Cost",
    },
    toasts: {
      recorded: "Cost recorded.",
    },
    validation: {
      costRequired: "Enter a cost of zero or more.",
    },
  },
  finance: {
    paymentReview: {
      description:
        "Confirm & post or reject customer-reported payments. Confirming posts one customer receipt and its journal entry in a single step.",
      queue: "Review queue",
      fields: {
        number: "Payment",
        customer: "Customer",
        order: "Order / Invoice",
        amount: "Amount",
        remaining: "Remaining",
        source: "Payment source",
        account: "Receiving account",
        reference: "Reference",
        proof: "Proof",
        date: "Date",
        status: "Status",
        reason: "Reason",
      },
      actions: {
        confirmPost: "Confirm & Post",
        reject: "Reject",
        setPrice: "Set agreed amounts",
      },
      confirmDialog: {
        title: "Confirm & post {payment}?",
        description:
          "{amount} will be verified against order {order} and posted as one customer receipt with its journal entry (Dr receiving account, Cr customer receivable).",
      },
      rejectDialog: {
        description:
          "The reason is saved on the payment and shown to the sales agent. Nothing is posted.",
        placeholder: "e.g. Transfer not found in the bank statement",
        reasonRequired: "Enter the reason for rejecting this payment.",
      },
      needsPrice: "No agreed price on {order} (total 0.00) — set the agreed amounts, then confirm.",
      toasts: {
        confirmedPosted:
          "{payment} confirmed — receipt {receipt} and journal entry {journal} posted.",
        alreadyPosted:
          "{payment} was already posted (receipt {receipt}). Nothing was posted twice.",
        rejected: "{payment} rejected. Reason saved.",
      },
    },
  },
  financialTransactions: {
    types: {
      CUSTOMER_RECEIPT: "Customer collection",
      SUPPLIER_PAYMENT: "Supplier payment",
      EXPENSE_PAYMENT: "Operating expense",
      CUSTOMER_REFUND: "Customer refund",
    },
    direction: {
      in: "Receipt",
      out: "Payment",
    },
    status: {
      draft: "Draft",
      confirmed: "Confirmed",
      cancelled: "Cancelled",
    },
    invoiceStatus: {
      unpaid: "Unpaid",
      partiallyPaid: "Partially Paid",
      paid: "Fully Paid",
      cancelled: "Cancelled",
    },
    paymentSummary: {
      title: "Payment Status",
      total: "Invoice Total",
      paid: "Paid Amount",
      remaining: "Remaining Amount",
    },
    fields: {
      type: "Transaction Type",
      transactionDate: "Date",
      amount: "Amount",
      paymentSource: "Payment Source",
      receivingAccount: "Receiving Account",
      referenceNumber: "Reference Number",
    },
    sections: {
      allocations: "Allocations",
    },
    allocationGrid: {
      empty: "No invoices allocated yet — add one from the Open Invoices list below.",
      invoice: "Invoice",
      remaining: "Remaining",
      amount: "Amount",
      fullyPaid: "Fully Paid",
      overpaid: "Exceeds remaining balance",
    },
    openInvoices: {
      empty: "No open invoices for this party.",
      count: "{count} open invoice(s)",
      payAllRemaining: "Pay All Remaining",
      invoice: "Invoice",
      date: "Date",
      grandTotal: "Grand Total",
      remaining: "Remaining",
      status: "Status",
      allocate: "Allocate",
    },
    summary: {
      amount: "Amount",
      allocated: "Allocated",
      unallocated: "Unallocated Remaining",
      overpaid: "Overpaid — allocated total exceeds the amount",
    },
    allocationSummary: {
      openInvoices: "Open Invoices",
      totalRemaining: "Total Remaining",
    },
    actions: {
      confirm: "Confirm",
      cancel: "Cancel",
    },
    toasts: {
      confirmed: "Confirmed.",
      cancelled: "Cancelled.",
      archived: "Archived.",
      allocated: "Invoice allocated.",
      unallocated: "Allocation removed.",
      payAllRemainingApplied: "Remaining balance applied to open invoices.",
      bulkArchived: "{count} transaction(s) archived.",
      bulkArchiveFailed: "{count} transaction(s) could not be archived.",
      deleted: "Deleted.",
    },
    validation: {
      partyRequired: "Select a party before saving.",
      amountRequired: "Enter an amount greater than zero.",
      allocationExceedsAmount: "Total allocated cannot exceed the amount.",
      receivingAccountRequired: "Select the receiving account (cash or bank) before confirming.",
      allocationExceedsRemaining: "Amount exceeds the invoice's remaining balance.",
    },
    confirmCancelTitle: "Cancel this transaction?",
    confirmCancelDescription:
      "This cannot be undone. Its allocations will be removed and invoice balances recalculated.",
    confirmArchiveTitle: "Archive this transaction?",
    confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
    confirmDeleteTitle: "Delete this draft?",
    confirmDeleteDescription: "This cannot be undone. The draft will be permanently removed.",
    bulk: {
      archiveConfirmTitle: "Archive {count} transaction(s)?",
    },
  },
  accounting: {
    journalEntries: {
      title: "Journal Entries",
      description: "Record and post manual journal entries — the accounting ledger foundation.",
      addNew: "New Journal Entry",
      editorTitle: "Journal Entry",
      open: "View",
      empty: "No journal entries yet.",
      fields: {
        number: "Entry No.",
        entryDate: "Entry Date",
        description: "Description",
        status: "Status",
        totalDebit: "Total Debit",
        totalCredit: "Total Credit",
        reversalOf: "Reversal Of",
        createdBy: "Created By",
        journal: "Journal",
        referenceNumber: "Reference",
        sourceDocument: "Source Document",
        viewJournalEntry: "View Journal Entry",
        viewSourceTransaction: "View Source Transaction",
        currency: "Currency",
        partner: "Partner",
        partnerType: "Partner Type",
        customer: "Customer",
        supplier: "Supplier",
        noPartner: "No Partner",
      },
      status: {
        draft: "Draft",
        posted: "Posted",
        reversed: "Reversed",
      },
      lines: {
        account: "Account",
        description: "Description",
        costCenter: "Cost Center",
        project: "Project",
        debit: "Debit",
        credit: "Credit",
        selectAccount: "Select account",
        selectCostCenter: "Cost center",
        selectProject: "Project",
        addLine: "Add Line",
        empty: "No lines yet — add at least two.",
        totalDebit: "Total Debit",
        totalCredit: "Total Credit",
        balanced: "Balanced",
        unbalanced: "Not balanced",
      },
      filters: {
        status: "Status",
        allStatuses: "All Statuses",
        journal: "Journal",
        allJournals: "All Journals",
      },
      actions: {
        post: "Post",
        reverse: "Reverse",
        resetToDraft: "Return to Draft",
        duplicate: "Duplicate",
        saveAsTemplate: "Save as Template",
        newFromTemplate: "New from Template",
      },
      templates: {
        saveTitle: "Save as Template",
        saveDescription: "Save this journal and line structure to quickly start a new entry later.",
        nameLabel: "Template Name",
        namePlaceholder: "e.g. Monthly Rent Accrual",
        descriptionLabel: "Description",
        descriptionPlaceholder: "Optional notes about when to use this template",
        selectPlaceholder: "Select a template…",
        empty: "No templates saved yet.",
        saved: "Template saved.",
        applied: "Template applied — review and save.",
      },
      toasts: {
        posted: "Journal entry posted.",
        resetToDraft: "Journal entry returned to Draft.",
        reversed: "Journal entry reversed.",
        archived: "Journal entry archived.",
        duplicated: "Journal entry duplicated as a new Draft.",
        bulkArchived: "{count} journal entry(ies) archived.",
        bulkArchiveFailed: "{count} journal entry(ies) could not be archived.",
        deleted: "Journal entry deleted.",
      },
      validation: {
        minLines: "A journal entry needs at least 2 lines.",
        unbalanced: "Total debit must equal total credit.",
        accountRequired: "Select an account for every line.",
        journalRequired: "Select a Journal before posting.",
      },
      confirmPostTitle: "Post this journal entry?",
      confirmPostDescription:
        "It will become part of the ledger. An authorized Finance user can return a manual entry to Draft while the period is open.",
      confirmResetToDraftTitle: "Return this journal entry to Draft?",
      confirmResetToDraftDescription:
        "It will leave the ledger until posted again. The audit trail of the original posting is kept.",
      confirmReverseTitle: "Reverse this journal entry?",
      confirmReverseDescription:
        "A new entry with debits and credits swapped will be posted, and this entry will be marked Reversed.",
      confirmArchiveTitle: "Archive this journal entry?",
      confirmArchiveDescription: "It will be hidden from the list. This does not affect its data.",
      confirmDeleteTitle: "Delete this draft journal entry?",
      confirmDeleteDescription: "This cannot be undone. The draft will be permanently removed.",
      bulk: {
        archiveConfirmTitle: "Archive {count} journal entry(ies)?",
      },
      missingJournal: "Journal entry not posted",
      systemGeneratedHint:
        "This journal was generated from a source transaction. Correct it from that document — it cannot be edited here.",
      sourceTypes: {
        MANUAL: "Manual",
        SALES_INVOICE: "Sales Invoice",
        PURCHASE_INVOICE: "Purchase Invoice",
        SALES_RETURN: "Sales Return",
        PURCHASE_RETURN: "Purchase Return",
        CUSTOMER_RECEIPT: "Customer Receipt",
        SUPPLIER_PAYMENT: "Supplier Payment",
        LANDED_COST: "Landed Cost",
        EXPENSE_PAYMENT: "Expense Payment",
        CUSTOMER_REFUND: "Customer Refund",
        INVENTORY_ADJUSTMENT: "Inventory Adjustment",
        SHIPMENT_COST: "Shipment Cost",
        FULFILLMENT_COST: "Fulfillment Cost",
        PAYROLL_RUN: "Payroll Run",
        PAYROLL_PAYMENT: "Payroll Payment",
        BANK_TRANSACTION_TRANSFER: "Internal Transfer",
        OPENING_BALANCE: "Opening Balance",
        YEAR_CLOSING: "Year Closing",
        FIXED_ASSET_CAPITALIZATION: "Asset Capitalization",
        FIXED_ASSET_DEPRECIATION: "Asset Depreciation",
        FIXED_ASSET_DISPOSAL: "Asset Disposal",
        PREPAID_EXPENSE: "Prepaid Expense",
        PREPAID_RECOGNITION: "Prepaid Recognition",
        ACCRUED_EXPENSE: "Accrued Expense",
        ACCRUED_EXPENSE_SETTLEMENT: "Accrual Settlement",
        FX_REVALUATION: "FX Revaluation",
        CAPITAL_CONTRIBUTION: "Capital Contribution",
        INVESTOR_DISTRIBUTION: "Investor Distribution",
        INVESTOR_PROFIT_PAYMENT: "Investor Profit Payment",
        CAPITAL_RETURN: "Capital Return",
      },
    },
    fiscalYears: {
      description:
        "Open and close accounting periods — no document may post into a closed or locked period.",
      createTitle: "New Fiscal Year",
      createDescription: "Creates the year and its 12 monthly Accounting Periods automatically.",
      createConfirm: "Create",
      periodsHint:
        "12 monthly periods will be created automatically, one per calendar month in this range.",
      periodsDescription:
        "Open, Close, or Lock each period. Select multiple to Bulk Close/Bulk Open.",
      defaultBadge: "Default Fiscal Year",
      fields: {
        name: "Name",
        startDate: "Start Date",
        endDate: "End Date",
        periods: "Periods",
      },
      status: {
        OPEN: "Open",
        CLOSED: "Closed",
      },
      actions: {
        close: "Close",
        reopen: "Reopen",
        setDefault: "Set Default",
      },
      toasts: {
        created: "Fiscal Year created.",
        closed: "Fiscal Year closed.",
        reopened: "Fiscal Year reopened.",
        defaultSet: "Default Fiscal Year updated.",
      },
      validation: {
        required: "Name, start date, and end date are required.",
        endAfterStart: "End date must be after the start date.",
      },
    },
    periods: {
      fields: {
        name: "Period",
        startDate: "Start Date",
        endDate: "End Date",
      },
      status: {
        OPEN: "Open",
        CLOSED: "Closed",
        LOCKED: "Locked",
      },
      actions: {
        close: "Close",
        reopen: "Reopen",
        lock: "Lock",
        unlock: "Unlock",
        bulkClose: "Bulk Close",
        bulkOpen: "Bulk Open",
      },
      lockedHint: "Locked",
      bulk: {
        selected: "{count} period(s) selected",
        success: "{count} period(s) updated.",
        partial: "{succeeded} succeeded, {failed} failed.",
      },
    },
    openingBalances: {
      description:
        "Enter each account's opening balance for a Fiscal Year — generates one balanced Opening Journal Entry.",
      fields: {
        fiscalYear: "Fiscal Year",
        selectFiscalYear: "Select Fiscal Year",
        openingDate: "Opening Date",
      },
      selectFiscalYearHint: "Select a Fiscal Year to enter its opening balances.",
      alreadyExists: "This Fiscal Year already has an Opening Balance entry ({entryNumber}).",
      viewEntry: "View Opening Entry",
      generate: "Generate Opening Entry",
      toasts: {
        created: "Opening Balance entry posted.",
      },
      validation: {
        minLines: "Enter at least one account with a debit or credit amount.",
      },
    },
    yearClosing: {
      description:
        "Transfer a Closed Fiscal Year's net Profit/Loss into Retained Earnings, and optionally carry the ending Balance Sheet forward as next year's Opening Entry.",
      fields: {
        fiscalYear: "Fiscal Year",
        nextFiscalYear: "Next Fiscal Year",
        noRollForward: "Don't generate an Opening Entry",
      },
      mustBeClosedFirst:
        "Close this Fiscal Year first (Finance > Fiscal Years) before running Year Closing.",
      alreadyClosed: "This Fiscal Year already has a Closing entry ({entryNumber}).",
      viewClosingEntry: "View Closing Entry",
      run: "Run Year Closing",
      toasts: {
        completed: "Year Closing completed.",
      },
    },
    prepaid: {
      title: "Prepaid Expenses",
      description: "Pay now, recognize expense across open periods through the Posting Engine.",
      activate: "Activate & Post Payment",
      recognize: "Recognize Due Periods",
      fields: {
        prepaidNumber: "Number",
        startDate: "Start Date",
        endDate: "End Date",
        totalPeriods: "Periods",
        expenseAccount: "Expense Account",
        receivingAccount: "Payment Source",
        recognizedAmount: "Recognized",
        status: "Status",
      },
      toasts: {
        created: "Prepaid expense created.",
        activated: "Payment posted to Prepayments.",
        recognized: "{count} period(s) recognized.",
      },
    },
    accruals: {
      title: "Accrued Expenses",
      description:
        "Recognize the expense now, settle cash later — both through the Posting Engine.",
      recognize: "Recognize",
      settle: "Settle",
      fields: {
        accrualNumber: "Number",
        recognitionDate: "Recognition Date",
        expenseAccount: "Expense Account",
        receivingAccount: "Payment Source",
        status: "Status",
        disposalAmount: "Disposal Proceeds",
      },
      toasts: {
        created: "Accrual created.",
        recognized: "Expense accrued and posted.",
        settled: "Accrual settled and posted.",
      },
    },
    fx: {
      title: "Exchange Rates & Revaluation",
      description:
        "Historical rates are snapshots. Period-end revaluation posts unrealized FX and reverses the prior run.",
      addRate: "Add Exchange Rate",
      runRevaluation: "Run Period-End Revaluation",
      rates: "Exchange Rate Snapshots",
      runs: "Revaluation Runs",
      fields: {
        fromCurrency: "From",
        toCurrency: "To",
        rate: "Rate",
        effectiveDate: "Effective Date",
        rateDate: "Rate Date",
        status: "Status",
      },
      ratePreview:
        "Meaning: 1 {from} = {rate} {to} · 1 {to} = {inverse} {from}. Enter how many units of the To currency one unit of the From currency buys.",
      baseCurrencyNote:
        "Company base currency: {base}. Documents in {base} post at their own amounts; rates convert other currencies into {base}.",
      baseCurrencyMissing:
        "The company base currency is not set. Set it in Accounting Settings before posting documents that carry a currency — OMS will not guess one.",
      toasts: {
        rateCreated: "Exchange rate snapshot saved.",
        revalued: "FX revaluation posted.",
      },
    },
    lifecycleStatus: {
      DRAFT: "Draft",
      CAPITALIZED: "Capitalized",
      DISPOSED: "Disposed",
      ACTIVE: "Active",
      COMPLETED: "Completed",
      RECOGNIZED: "Recognized",
      SETTLED: "Settled",
      POSTED: "Posted",
      PENDING: "Pending",
    },
    settings: {
      title: "Accounting Settings",
      description:
        "Where every Posting Provider posts by default — Product Category, Customer Group, Supplier Group, and Tax records can each override these.",
      sections: {
        general: "General Accounts",
        sales: "Sales Accounts",
        purchasing: "Purchasing Accounts",
        inventory: "Inventory Accounts",
        operating: "Direct Operating Costs",
        assets: "Fixed Assets & Prepayments",
        tax: "Tax Accounts",
        customerDefaults: "Customer Defaults",
        supplierDefaults: "Supplier Defaults",
        equity: "Equity Accounts",
        payroll: "Payroll Accounts",
        investors: "Investor Accounting",
      },
      fields: {
        salesRevenue: "Sales Revenue Account",
        salesDiscount: "Sales Discount Account",
        salesReturn: "Sales Return Account",
        cogs: "Cost Of Goods Sold (COGS)",
        inventoryAsset: "Inventory Asset Account",
        inventoryAdjustment: "Inventory Adjustment Account",
        landedCostClearing: "Landed Cost Clearing Account",
        purchase: "Purchase Account",
        purchaseDiscount: "Purchase Discount Account",
        purchaseReturn: "Purchase Return Account",
        accountsReceivable: "Accounts Receivable Default",
        accountsPayable: "Accounts Payable Default",
        cash: "Cash Account",
        bank: "Bank Account",
        vatOutput: "VAT Output Account",
        vatInput: "VAT Input Account",
        roundDifference: "Round Difference Account",
        exchangeDifference: "Exchange Difference Account",
        suspense: "Suspense Account",
        retainedEarnings: "Retained Earnings Account",
        defaultExpense: "Default Expense Account",
        payrollPayable: "Payroll Payable Account",
        salaryExpense: "Salary Expense Account",
        kpiExpense: "KPI Expense Account",
        commissionExpense: "Commission Expense Account",
        defaultAllowanceExpense: "Default Allowance Account",
        defaultDeduction: "Default Deduction Account",
        investorFunding: "Investor Funding Account",
        investorProfitDistribution: "Investor Profit Distribution Account",
        investorProfitPayable: "Investor Profit Payable Account",
        capitalReturn: "Capital Return Account",
        shippingExpense: "Shipping / Carrier Expense",
        accruedShipping: "Accrued Shipping",
        paymentGatewayFee: "Payment Gateway Fees",
        fulfillmentExpense: "Fulfillment / Packaging Expense",
        accruedFulfillment: "Accrued Fulfillment",
        fixedAssets: "Fixed Assets",
        accumDepreciation: "Accumulated Depreciation",
        depreciationExpense: "Depreciation Expense",
        prepayments: "Prepaid Expenses",
        accruedExpenses: "Accrued Expenses",
        unrealizedFx: "Unrealized FX Gain/Loss",
        otherIncome: "Other Income",
        otherExpense: "Other Expense",
        functionalCurrency: "Functional Currency",
        functionalCurrencyHint:
          "The unit every posted amount is stored in. It cannot be changed after journal entries have been posted.",
        functionalCurrencyMissing:
          "Not set — documents that carry a currency cannot be posted until the company base currency is chosen.",
      },
      picker: {
        select: "Select account",
        searchPlaceholder: "Search chart of accounts...",
        noResults: "No accounts found.",
        clear: "Clear account",
      },
      validation: {
        required: "Required.",
        missingRequired: "{count} required account(s) are missing.",
      },
    },
  },
  importCenter: {
    title: "Import Center",
    description: "The single entry point for importing operational and accounting data into OMS.",
    newImport: "New Import",
    startImport: "Start Import",
    downloadTemplate: "Download Template",
    availableTypes: "Import Types",
    history: "Import History",
    noJobs: "No import jobs yet.",
    notAvailable: "Coming Soon",
    notAvailableHint:
      "Registered as an Import Type — row-by-row import isn't available for this type yet. The Excel Template can still be downloaded.",
    fieldsCount: "{count} fields",
    requiredFieldsCount: "{count} required",
    viewJob: "View",
    cancelJob: "Cancel",
    confirmCancelJobTitle: "Cancel this import job?",
    confirmCancelJobDescription: "The job will stop before any remaining rows are imported.",
    downloadTemplateFailed: "Failed to download template.",
    actions: {
      downloadTemplate: "Download Excel Template",
      uploadDevice: "Upload Excel/CSV from Device",
      googleSheets: "Import from Google Sheets",
    },
    types: {
      leads: {
        label: "Leads",
        description: "Import leads — name, phone, and country only; complete the rest later.",
      },
      orders: {
        label: "Orders",
        description: "Import complete orders — address, product, and paid amount are required.",
      },
      bankTransactions: {
        label: "Bank Statement",
        description: "Import bank statement transactions to match against payments.",
      },
      customers: { label: "Customers", description: "Import customer master records." },
      suppliers: { label: "Suppliers", description: "Import supplier master records." },
      products: { label: "Products", description: "Import product/service master records." },
      warehouses: { label: "Warehouses", description: "Import warehouse master records." },
      chartOfAccounts: {
        label: "Chart of Accounts",
        description: "Import chart of accounts records.",
      },
      taxRates: { label: "Tax Rates", description: "Import tax rate records." },
      costCenters: { label: "Cost Centers", description: "Import cost center records." },
      projects: { label: "Projects", description: "Import project records." },
      inventoryAdjustments: {
        label: "Inventory Adjustments",
        description: "Import stock quantity corrections per product/warehouse.",
      },
      salesQuotations: {
        label: "Sales Quotations",
        description: "Import sales quotations with line items.",
      },
      salesOrders: { label: "Sales Orders", description: "Import sales orders with line items." },
      salesInvoices: {
        label: "Sales Invoices",
        description: "Import sales invoices with line items.",
      },
      salesReturns: {
        label: "Sales Returns",
        description: "Import sales returns with line items.",
      },
      purchaseQuotations: {
        label: "Purchase Quotations",
        description: "Import purchase quotations with line items.",
      },
      purchaseOrders: {
        label: "Purchase Orders",
        description: "Import purchase orders with line items.",
      },
      purchaseInvoices: {
        label: "Purchase Invoices",
        description: "Import purchase invoices with line items.",
      },
      purchaseReturns: {
        label: "Purchase Returns",
        description: "Import purchase returns with line items.",
      },
      customerReceipts: {
        label: "Customer Receipt Vouchers",
        description: "Import Customer Receipt Vouchers and their invoice allocations.",
      },
      supplierPayments: {
        label: "Supplier Payment Vouchers",
        description: "Import Supplier Payment Vouchers and their invoice allocations.",
      },
      manualJournalEntries: {
        label: "Manual Journal Entries",
        description: "Import manual journal entries with debit/credit lines.",
      },
      openingBalances: {
        label: "Opening Balances",
        description: "Import opening balances per account for a Fiscal Year.",
      },
      openingStock: {
        label: "Opening Stock",
        description: "Import opening warehouse stock quantities per product.",
      },
      storeOrders: {
        label: "Store Orders",
        description: "Import Store Orders — the Customer is matched automatically by phone.",
      },
      shippingUpdates: {
        label: "Shipping Updates",
        description:
          "Import shipping status updates (carrier, tracking number, status) for existing orders.",
      },
    },
    fields: {
      name: "Name",
      code: "Code",
      commercialName: "Commercial Name",
      phone: "Phone",
      mobile: "Mobile",
      email: "Email",
      taxNumber: "Tax Number",
      city: "City",
      address: "Address",
      notes: "Notes",
      productType: "Product Type",
      categoryName: "Category",
      unitName: "Unit",
      barcode: "Barcode",
      salesPrice: "Sales Price",
      purchasePrice: "Purchase Price",
      description: "Description",
      productSku: "Product SKU",
      warehouseCode: "Warehouse Code",
      warehouseType: "Warehouse Type",
      parentWarehouseName: "Parent Warehouse",
      accountType: "Account Type",
      parentAccountCode: "Parent Account Code",
      accountKind: "Account Kind",
      currencyCode: "Currency Code",
      allowReconciliation: "Allow Reconciliation",
      rate: "Rate (%)",
      quantity: "Quantity",
      reason: "Reason",
      documentNumber: "Document Number",
      customerName: "Customer Name",
      supplierName: "Supplier Name",
      unitPrice: "Unit Price",
      transactionNumber: "Transaction Number",
      amount: "Amount",
      invoiceNumber: "Invoice Number",
      allocatedAmount: "Allocated Amount",
      financialTransactionType: "Financial Transaction Type",
      entryDate: "Entry Date",
      accountCode: "Account Code",
      debit: "Debit",
      credit: "Credit",
      fiscalYear: "Fiscal Year",
      mobileNumber: "Mobile Number",
      countryName: "Country",
      agentEmail: "Assign To (Agent Email)",
      unitCost: "Unit Cost",
      referenceNumber: "Reference Number",
      documentDate: "Document Date",
      lineDescription: "Line Description",
      warehouseName: "Warehouse",
      discountPercent: "Discount %",
      taxName: "Tax",
      purchaseType: "Purchase Type",
      journalCode: "Journal Code",
      costCenterCode: "Cost Center Code",
      projectCode: "Project Code",
      openingDate: "Opening Date",
      externalOrderId: "External Order ID",
      orderDate: "Order Date",
      paidAmount: "Paid Amount",
      paymentMethodLabel: "Payment Method",
      paymentType: "Payment Type",
      receipt1: "Receipt 1",
      receipt2: "Receipt 2",
      receipt3: "Receipt 3",
      bankTransactionId: "Bank Transaction ID",
      transactionDate: "Transaction Date",
      valueDate: "Value Date",
      bankAccountNumber: "Account Number",
      bankReference: "Bank Reference",
      bankAmount: "Amount",
      balance: "Balance",
      bankName: "Bank Name",
      branch: "Branch",
      cashSource: "Cash Source",
      cashFlowOutgoingType: "Transaction Type",
      expenseAccount: "Expense Account",
      partnerSupplier: "Partner/Supplier",
      costCenter: "Cost Center",
      project: "Project",
      shippingLabelUrl: "Shipping Label URL",
    },
    status: {
      DRAFT: "Draft",
      UPLOADING: "Uploading",
      MAPPING: "Mapping",
      VALIDATING: "Validating",
      IMPORTING: "Importing",
      COMPLETED: "Completed",
      FAILED: "Failed",
      CANCELLED: "Cancelled",
    },
    table: {
      importType: "Import Type",
      status: "Status",
      fileName: "File",
      source: "Source",
      sourceFile: "File Upload",
      sourceGoogleSheets: "Google Sheets",
      user: "User",
      totalRows: "Total Rows",
      successCount: "Success",
      errorCount: "Errors",
      duration: "Duration",
      createdAt: "Created",
      lastSynced: "Last Synced",
    },
    wizard: {
      title: "Import {type}",
      cancel: "Cancel Import",
      cancelConfirmTitle: "Cancel this import job?",
      cancelConfirmDescription: "The job will be marked Cancelled. This cannot be undone.",
      steps: {
        upload: "Upload File",
        mapping: "Map Columns",
        preview: "Preview",
        results: "Results",
      },
      upload: {
        title: "Upload a File",
        description:
          "Upload the filled-in Excel Template (.xlsx), a CSV file, or paste a Google Sheets URL.",
        dropHint: "Drop an Excel or CSV file here, or click to choose from your device.",
        chooseFile: "Choose File",
        selectedFile: "Selected file",
        uploadButton: "Upload & Continue",
        uploading: "Uploading…",
        sourceFile: "File",
        sourceGoogleSheets: "Google Sheets URL",
      },
      googleSheets: {
        urlLabel: "Google Sheets URL",
        urlHint: "Share the sheet with the integration's service-account email address.",
        refresh: "Refresh Data",
        refreshing: "Refreshing data...",
        refreshTooltip: "Fetch the latest data from Google Sheets",
        refreshed: "Re-fetched the latest data from Google Sheets.",
        lastSynced: "Last successful sync: {datetime}",
        readFailed: "Couldn't read that sheet — check the link and sharing.",
      },
      mapping: {
        title: "Map Columns",
        description: "Match every required field to a column from your file.",
        requiredFields: "Required Fields",
        optionalFields: "Optional Fields",
        selectColumn: "Select column",
        noColumn: "— Not mapped —",
        loadTemplate: "Load Mapping Template",
        saveTemplate: "Save as Template",
        templateName: "Template Name",
        templateNamePlaceholder: "e.g. Monthly Supplier File",
        saveTemplateConfirm: "Save Template",
        templateSaved: "Mapping template saved.",
        continue: "Continue to Preview",
        missingRequired: "Map every required field before continuing.",
      },
      preview: {
        title: "Preview",
        description: "First rows of your file, mapped to Import fields. Nothing is imported yet.",
        rowNumber: "Row",
        runImport: "Run Import",
        running: "Importing…",
        totalRowsHint: "{count} total row(s) will be imported.",
        summaryTotal: "Total",
        summaryNew: "New",
        summaryDuplicate: "Duplicate",
        summaryInvalid: "Invalid",
      },
      validation: {
        running: "Validating…",
        passed: "Validation passed — no issues found. Nothing has been imported yet.",
        failed: "{count} issue(s) found. Fix them before running the import.",
        duplicatesFound: "{count} duplicate group(s) found within this file.",
      },
      results: {
        title: "Import Results",
        completed: "Import completed.",
        failed: "Import failed — no rows were imported.",
        totalRows: "Total Rows",
        successCount: "Imported",
        errorCount: "Failed",
        duration: "Duration",
        errorsTitle: "Row Errors",
        noErrors: "No errors — every row imported successfully.",
        column: "Column",
        error: "Error",
        suggestedFix: "Suggested Fix",
        downloadErrorReport: "Download Error Report",
        done: "Done",
      },
    },
    sync: {
      button: "Sync Data",
      buttonTooltip:
        "Reads the latest data from Google Sheets and applies validation and duplicate-detection rules before anything is entered into the system.",
      loading: "Syncing…",
      noSource: "No sync source is configured for this module yet.",
      noSourceShipping: "The Store Orders Google Sheets source is not configured yet.",
      configureHint: "Configure a sync source in Import Center to enable this.",
      configureHintShipping:
        "Configure the Store Orders Google Sheets source — Shipping Sync uses that same source.",
      disabledSource: "This sync source is disabled.",
      previewTitle: "Sync Preview — {label}",
      previewDescription: "Review what will change before committing anything.",
      summaryTotal: "Total Rows",
      summaryNew: "New",
      summaryWillImport: "Will Import",
      summaryDuplicate: "Duplicate",
      summaryNeedsReview: "Needs Review",
      summaryRejected: "Rejected",
      summaryErrors: "Errors",
      confirmButton: "Confirm & Sync",
      confirming: "Syncing…",
      cancel: "Cancel",
      success: "Sync completed — {imported} of {total} rows imported.",
      partial: "Sync completed with issues — {imported} of {total} rows imported, {errors} failed.",
      failed: "Sync failed — no rows were imported.",
      lastSync: "Last Sync",
      lastSyncBy: "By {user}",
      lastSyncLine: "Last sync: {value}",
      lastSyncMinutesAgo: "{minutes} minutes ago",
      statusSuccess: "Succeeded",
      statusPartial: "Partial",
      statusFailed: "Failed",
      statusNeverRun: "Never synced",
      statusNeverRunShort: "Not yet",
      card: {
        lastUpdate: "Last update",
        by: "By",
        never: "No sync has run yet",
      },
      cards: {
        storeOrders: {
          title: "Sync Store Orders",
          description:
            "Sync completed orders from Google Sheets into the system and review before approving.",
        },
        leads: {
          title: "Sync Leads",
          description:
            "Sync lead data from Google Sheets into the system, applying duplicate-prevention and review rules.",
        },
        cashFlow: {
          title: "Sync Cash Flow",
          description:
            "Sync bank statements and payment methods from Google Sheets to match payments with orders.",
        },
        shippingUpdates: {
          title: "Sync Shipping Updates",
          description: "Uses the same Google Sheets source as Store Orders synchronization.",
        },
        listSheet: {
          title: "Sync OMS Data to List Sheet",
          description: "Update Google Sheets reference lists from the current system data.",
        },
      },
      listSheet: {
        direction: "OMS → Google Sheets",
        success: "OMS data was synchronized to Google Sheets successfully.",
        partial: "Synchronization completed partially — not every list was updated.",
        partialDetail: "Could not update: {lists}",
        failed: "Could not synchronize OMS data to Google Sheets.",
        lastSync: "Last sync: {datetime}",
        lastSyncNever: "Last sync: not yet",
        summary:
          "Countries {countries} · Products {products} · Currencies {currencies} · Payment methods {paymentMethods} · Employees {employees} · Shipping statuses {shippingStatuses} · Shipping companies {shippingCompanies} · Payment types {paymentTypes} · Transaction types {financialTransactionTypes} · {datetime}",
        lists: {
          country: "Countries",
          product: "Products",
          currency: "Currencies",
          paymentMethod: "Payment methods",
          employeeEmail: "Employees",
          shippingStatus: "Shipping statuses",
          shippingCompany: "Shipping companies",
          paymentType: "Payment types",
          financialTransactionType: "Financial transaction types",
        },
        errors: {
          generic: "Could not synchronize OMS data to Google Sheets. Try again.",
        },
      },
      report: {
        title: "Sync Report",
        externalOrderId: "Order ID",
        result: "Result",
        message: "Message",
        close: "Close",
        resultUpdated: "Updated",
        resultNoChange: "No Change",
        resultRejected: "Rejected",
        resultNotFound: "Not Found",
        resultNeedsReview: "Needs Review",
        resultFinal: "Final",
        resultSkippedFinal: "Skipped (Final)",
        skippedFinalSummary: "{count} final shipments were skipped.",
        showSkippedFinal: "Show final shipments",
        hideSkippedFinal: "Hide final shipments",
      },
      review: {
        title: "Sync Review — {label}",
        description: "Review, validate, and decide what to import before anything is written.",
        source: "Source",
        sourceGoogleSheets: "Google Sheets",
        sheet: "Sheet",
        syncTime: "Sync time",
        rowsReceived: "Rows received",
        rowsAccepted: "Rows accepted",
        rowsRejected: "Rows rejected",
        rowsPending: "Pending decision",
        duplicates: "Duplicates",
        kpiTotal: "Total",
        kpiNew: "New rows",
        kpiRetry: "Retry",
        kpiReady: "Ready to import",
        kpiWarning: "Needs review",
        kpiError: "Errors",
        kpiDuplicate: "Duplicate",
        kpiSkipped: "Already synced / ignored",
        filterAll: "All",
        colRow: "Row",
        colOrder: "Order",
        colCustomer: "Customer",
        colPhone: "Mobile",
        colProduct: "Product",
        colStatus: "Status",
        colIssue: "Issue",
        colAction: "Action",
        statusReady: "Ready to import",
        statusWarning: "Needs review",
        statusError: "Invalid",
        statusDuplicate: "Duplicate",
        lifecycleNew: "New",
        lifecycleRetry: "Retry",
        lifecycleImported: "Already imported",
        lifecycleSkipped: "Ignored",
        lifecycleError: "Error",
        lifecycleModified: "Modified",
        lifecycleExternalDup: "Duplicate external order ID",
        externalDupReconciled: "Order exists in OMS — link restored",
        lifecyclePhoneMatch: "Pending decision — same phone",
        lifecycleDeleted: "Deleted",
        phoneMatchAccepted: "Accepted as new order",
        phoneMatchRejected: "Rejected",
        nothingToSync: "No new data to synchronize.",
        original: "Original",
        normalized: "Normalized",
        value: "Value",
        country: "Country",
        existingOrder: "Existing order",
        orderDate: "Order date",
        sourceRow: "Source row",
        phoneMatchInfo: "Phone match",
        phoneMatchScopeBatch: "Matches another row in this import file",
        phoneMatchScopeExisting: "Matches a previous order already in the system",
        phoneMatchScopeBoth:
          "Matches both another row in this file and a previous order in the system",
        phoneMatchGroupRows: "{count} other row(s) share this phone number:",
        phoneMatchRecommendation:
          "Confirm whether this is really a new order for the same customer before accepting it.",
        phoneMatchConfirmTitle: "Import as new order?",
        phoneMatchConfirmDescription:
          "{count} row(s) with a matching phone number will be imported as separate new orders and labeled مكرر.",
        phoneMatchConfirmAction: "Import as new order",
        phoneMatchRejectConfirmTitle: "Confirm rejecting this order?",
        phoneMatchRejectConfirmDescription:
          "{count} row(s) will be finally rejected and will not be imported; the rejection result will be written to the sheet. This can only be undone with a new sync.",
        phoneMatchRejectConfirmAction: "Confirm reject",
        phoneReady: "Ready",
        errorsInRow: "{count} errors on this row",
        viewErrors: "View errors",
        digestTitle: "{count} errors were found.",
        digestShow: "Show errors",
        digestField: "{count} errors in {field}",
        digestOther: "{count} other errors",
        details: "Details",
        originalValues: "Original values",
        normalizedValues: "Normalized values",
        validationErrors: "Validation errors",
        warnings: "Warnings",
        duplicateInfo: "Duplicate",
        suggestedCorrection: "Suggested correction",
        availableActions: "Available actions",
        actionView: "View details",
        actionAccept: "Import this order",
        actionAcceptWarning: "Accept despite warning",
        actionAcceptAsNewOrder: "Import as new order",
        actionReject: "Reject",
        actionSkip: "Skip",
        actionRetry: "Retry",
        actionRevalidate: "Revalidate",
        bulkAcceptSelected: "Import selected as new orders",
        bulkAcceptReady: "Import all ready orders",
        bulkReviewSelected: "Review selected",
        bulkRejectSelected: "Reject selected",
        bulkRejectAll: "Reject all",
        bulkRetrySelected: "Retry selected",
        bulkRetryEligible: "Retry all failed",
        bulkDownloadErrors: "Download errors",
        bulkSelectStatus: "Select all with this status",
        bulkClearOrphanResults: "Reset sync results for selected rows",
        clearOrphanTitle: "Reset sync result columns?",
        clearOrphanDescription:
          "This clears only Sync Status, System Order ID, and Error Message (Q:R:S) for {count} selected row(s). Customer and order input columns are not changed. After reset, run sync again so the row can be validated as new.",
        clearOrphanConfirm: "Reset Q:R:S",
        clearOrphanSuccess: "Cleared sync results for {count} row(s).",
        clearOrphanFailed: "Could not clear sync result columns.",
        confirmTitle: "Confirm synchronization",
        confirmIntro: "The following will be imported:",
        confirmOrders: "{count} records",
        confirmReady: "Ready",
        confirmWarnings: "Warnings",
        confirmErrorsExcluded: "Errors excluded",
        confirmDuplicates: "Duplicate external order ID",
        confirmPhoneMatchAccepted: "Accepted as new order",
        confirmPhoneMatchPending: "Pending decision — same phone",
        confirmPrimary: "Confirm sync",
        completeSync: "Complete sync",
        revalidate: "Revalidate",
        noImportable: "No importable rows are selected.",
        empty: "No rows match this filter.",
        reasons: {
          phoneInvalidCountry: "The mobile number is not valid for the selected country.",
          phoneRequired: "Mobile number is required.",
          phoneTooShort: "The mobile number is too short for the selected country.",
          phoneTooLong: "The mobile number is too long for the selected country.",
          phoneNotANumber: "This does not look like a mobile number.",
          phoneInvalid: "The mobile number is not valid for the selected country.",
          unrecognizedCountry: "This country is not recognized. Choose an existing country.",
          duplicateOrder: "This order already exists in the system.",
          phoneMatch: "Phone match — review",
          orphanLink: "Stale system number — not found in OMS",
          duplicateInFile: "This value is duplicated in the source file.",
          needsReviewCustomer:
            "An existing customer was found by mobile number — the new order will be linked to that customer.",
          sourceDeleted:
            "The source row was removed from Google Sheets. Confirm to archive the Store Order.",
          sourceLocked:
            "This Store Order can no longer be updated from Google Sheets because payment, shipping, or invoicing has already started.",
          required: "{field} is required.",
          correctThenRevalidate: "Correct the source data, then revalidate.",
        },
        downloadFileName: "sync-errors.csv",
      },
      sources: {
        title: "Sync Sources",
        description: "Google Sheets sources this module can synchronize from.",
        new: "New Source",
        newDescription: "Point a source module at a spreadsheet and map its columns.",
        label: "Label",
        type: "Type",
        enabled: "Enabled",
        created: "Sync source created.",
        pushReferenceData: "Push Reference Data",
        pushReferenceDataDescription:
          "Writes the current Master Data (Countries, Currencies, Products, ...) into a dedicated tab on the target spreadsheet, for the business tab's dropdowns to reference — never actual business data.",
        referenceDataPushed: "Reference data pushed to the sheet.",
      },
    },
  },
  hr: {
    myProfile: {
      noEmployeeTitle: "No employee record linked",
      noEmployeeDescription:
        "Your user account is not linked to an employee record yet. Ask HR to link it to see your profile and sales ranking.",
    },
    employees: {
      title: "Employees",
      description: "The central Employee record — job info, compensation, and login account.",
      addNew: "Add Employee",
      fields: {
        employeeCode: "Employee Code",
        name: "Name",
        mobile: "Mobile",
        email: "Email",
        hireDate: "Hire Date",
        department: "Department",
        jobTitle: "Job Title",
        salesTeam: "Sales Team",
        manager: "Manager",
        employmentStatus: "Employment Status",
        noDepartment: "No department",
        noJobTitle: "No job title",
        noSalesTeam: "No team",
        noManager: "No manager",
      },
      picker: {
        select: "Select employee",
        searchPlaceholder: "Search by name or code...",
        noResults: "No employees found.",
      },
      status: {
        ACTIVE: "Active",
        INACTIVE: "Inactive",
        TERMINATED: "Terminated",
      },
      wizard: {
        title: "Add New Employee",
        steps: {
          basic: "Basic Info",
          work: "Job",
          compensation: "Compensation",
          account: "Account",
        },
        skipCompensation: "Compensation can be added later from the Employee profile",
        createLoginAccount: "Create a login account for this employee",
        loginEmail: "Login Email",
        role: "Role",
        roles: {
          EMPLOYEE: "Employee (self-view only)",
          MANAGER: "Manager",
          HR: "HR",
          SALES_MANAGER: "Sales Manager",
          FINANCE: "Finance",
        },
        username: "Username",
        back: "Back",
        next: "Next",
        finish: "Save Employee",
      },
      profile: {
        tabs: {
          general: "General",
          work: "Job",
          compensation: "Compensation",
          account: "Account",
          kpi: "Performance",
          targets: "Targets",
          commissions: "Commissions",
          activity: "Activity",
        },
        account: {
          hasAccount: "Linked to a login account",
          noAccount: "No login account",
          createAccount: "Create Login Account",
        },
        addRevision: "Add Salary Revision",
        noCompensation: "This employee has no compensation record yet.",
        currentCompensation: "Current Compensation",
        compensationHistory: "Salary Revision History",
      },
      toasts: {
        created: "Employee added",
        updated: "Changes saved",
        archived: "Employee archived",
        restored: "Employee restored",
        compensationSaved: "Compensation saved",
        accountCreated: "Account created",
      },
      errors: {
        selectEmployee: "Select an employee",
        enterSalary: "Enter the salary",
      },
    },
    compensation: {
      title: "Compensation",
      fields: {
        effectiveFrom: "Effective From",
        basicSalary: "Basic Salary",
        kpiMaxPay: "KPI Maximum Pay",
        notes: "Notes",
        lines: "Allowances & Deductions",
        component: "Component",
        amount: "Amount",
      },
      addLine: "Add Line",
    },
    payrollComponents: {
      title: "Payroll Components",
      description:
        "The dynamic payroll component catalog — earnings and deductions used in compensation and payroll.",
      fields: {
        nameAr: "Name (Arabic)",
        nameEn: "Name (English)",
        type: "Type",
        calculationType: "Calculation Type",
        defaultValue: "Default Value",
        accountingMappingAccountId: "Accounting Account",
      },
      type: {
        EARNING: "Earning",
        DEDUCTION: "Deduction",
      },
      calculationType: {
        FIXED: "Fixed",
        PERCENTAGE: "Percentage",
        VARIABLE: "Variable",
      },
    },
    kpiTemplates: {
      title: "KPI Templates",
      description: "Weighted monthly evaluation criteria — active item weights must total 100%.",
      addNew: "New Template",
      fields: {
        name: "Template Name",
        nameEn: "Name (English)",
        description: "Description",
        isActive: "Active",
      },
      items: {
        title: "Evaluation Criteria",
        addItem: "Add Criterion",
        criterionAr: "Criterion (Arabic)",
        criterionEn: "Criterion (English)",
        weight: "Weight %",
        itemType: "Evaluation Method",
        evaluatorSource: "Evaluator",
        autoMetricSource: "Auto Metric Source",
        dropdownOptions: "Dropdown Options",
        optionLabel: "Label",
        optionScore: "Score",
        addOption: "Add Option",
        totalWeight: "Total Weight",
        weightMustTotal100: "Active criteria weights must total 100%",
      },
      itemType: {
        YES_NO: "Yes / No",
        PERCENTAGE: "Percentage",
        RATING_1_TO_5: "Rating (1-5)",
        DROPDOWN: "Dropdown",
        AUTO_METRIC: "Auto Metric",
      },
      evaluatorSource: {
        MANAGER: "Manager",
        HR: "HR",
        SYSTEM: "System",
      },
      autoMetricSource: {
        SALES_TARGET_ACHIEVEMENT: "Sales Target Achievement",
      },
      assignment: {
        title: "Assignments",
        addAssignment: "New Assignment",
        scope: "Scope",
        target: "Target",
      },
      assignmentScope: {
        EMPLOYEE: "Specific Employee",
        JOB_TITLE: "Job Title",
        DEPARTMENT: "Department",
      },
      toasts: {
        saved: "Template saved",
        assigned: "Assignment added",
        unassigned: "Assignment removed",
      },
    },
    kpiEvaluations: {
      title: "Monthly KPI Evaluations",
      description: "Monthly employee evaluation against their assigned KPI template.",
      startNew: "Start New Evaluation",
      fields: {
        employee: "Employee",
        period: "Period",
        template: "Template",
        status: "Status",
        finalScore: "Final Score",
        kpiPay: "KPI Pay",
      },
      status: {
        DRAFT: "Draft",
        MANAGER_SUBMITTED: "Manager Submitted",
        HR_APPROVED: "HR Approved",
        INCLUDED_IN_PAYROLL: "Included in Payroll",
      },
      scoring: {
        title: "Score Entry",
        yesNoYes: "Yes",
        yesNoNo: "No",
        commentLabel: "Comment",
        commentRequired: "Enter a comment — score is below 50%",
        recompute: "Recompute Auto Metrics",
        weightedScore: "Weighted Score",
        normalizedScore: "Score",
      },
      actions: {
        submit: "Submit for Approval",
        approve: "Approve",
        reopen: "Reopen",
        reopenReason: "Reopen Reason",
      },
      toasts: {
        started: "Evaluation started",
        saved: "Evaluation saved",
        submitted: "Evaluation submitted for approval",
        approved: "Evaluation approved",
        reopened: "Evaluation reopened",
      },
      errors: {
        completeEvaluation: "Complete the evaluation",
        enterReopenReason: "Enter the reopen reason",
        alreadyExists: "An evaluation for this employee already exists for this period",
      },
    },
    salesTargets: {
      title: "Monthly Sales Targets",
      description: "Monthly sales targets — for an employee or a sales team.",
      addNew: "New Target",
      fields: {
        period: "Period",
        scopeType: "Scope",
        employee: "Employee",
        salesTeam: "Sales Team",
        metric: "Metric",
        targetAmount: "Target Amount",
        actualAmount: "Actual",
        achievement: "Achievement",
      },
      scopeType: {
        EMPLOYEE: "Employee",
        TEAM: "Sales Team",
      },
      metric: {
        SALES_REVENUE: "Sales Revenue",
        COLLECTED_SALES: "Collected Sales",
        ORDERS_COUNT: "Orders Count",
      },
      toasts: {
        saved: "Target saved",
      },
      errors: {
        selectScope: "Select an employee or a team",
      },
    },
    ranking: {
      title: "Ranking",
      description: "Achieved sales ranked against target for the selected period.",
      rank: "Rank",
      of: "of {total}",
      noData: "No ranking data for this period.",
    },
    commissionPlans: {
      title: "Commission Plans",
      description:
        "Commission calculation rules — flat percentage, achievement tiers, or a fixed bonus.",
      addNew: "New Plan",
      fields: {
        name: "Plan Name",
        description: "Description",
        basis: "Basis",
        ruleType: "Rule Type",
        isActive: "Active",
      },
      basis: {
        COLLECTED_SALES: "Collected Sales",
        SALES_REVENUE: "Sales Revenue",
        ORDERS_COUNT: "Orders Count",
      },
      ruleType: {
        FLAT_PERCENTAGE: "Flat Percentage",
        ACHIEVEMENT_TIER: "Achievement Tier",
        FIXED_BONUS: "Fixed Bonus",
      },
      tiers: {
        title: "Tiers",
        addTier: "Add Tier",
        minAchievementPercent: "From Achievement %",
        maxAchievementPercent: "To Achievement %",
        percentage: "Percentage %",
        fixedAmount: "Fixed Amount",
        openEnded: "No upper limit",
      },
      assignment: {
        title: "Assignments",
        addAssignment: "New Assignment",
        scope: "Scope",
        target: "Target",
        priority: "Resolution priority: Employee > Team > Department > Company Default",
      },
      assignmentScope: {
        EMPLOYEE: "Specific Employee",
        TEAM: "Sales Team",
        DEPARTMENT: "Department",
        COMPANY: "Company Default",
      },
      toasts: {
        saved: "Commission plan saved",
        assigned: "Assignment added",
        unassigned: "Assignment removed",
      },
    },
    commissions: {
      title: "Commissions",
      description: "Monthly calculated commissions, resolved from each employee's assigned plan.",
      calculateNew: "Calculate Commission",
      fields: {
        employee: "Employee",
        period: "Period",
        plan: "Plan",
        basisAmount: "Basis Amount",
        targetAmount: "Target",
        achievementPercent: "Achievement %",
        amount: "Commission Amount",
        status: "Status",
      },
      status: {
        CALCULATED: "Calculated",
        APPROVED: "Approved",
        INCLUDED_IN_PAYROLL: "Included in Payroll",
        ADJUSTED: "Adjusted",
      },
      actions: {
        approve: "Approve",
        adjust: "Adjust",
        futureAdjustment: "Future-Period Adjustment",
      },
      adjust: {
        newAmount: "New Amount",
        reason: "Reason",
        targetPeriod: "Target Period",
      },
      toasts: {
        calculated: "Commission calculated",
        approved: "Commission approved",
        adjusted: "Adjustment saved",
      },
      errors: {
        enterReason: "Enter the adjustment reason",
      },
    },
    payroll: {
      title: "Payroll Runs",
      description:
        "The monthly payroll run — combines basic salary, allowances, KPI pay, and approved commission.",
      createNew: "New Run",
      fields: {
        period: "Period",
        status: "Status",
        grossEarnings: "Gross Earnings",
        totalDeductions: "Total Deductions",
        netPay: "Net Pay",
      },
      status: {
        DRAFT: "Draft",
        HR_REVIEWED: "HR Reviewed",
        FINANCE_APPROVED: "Finance Approved",
        POSTED: "Posted",
        PAID: "Paid",
      },
      lines: {
        title: "Employee Breakdown",
        employee: "Employee",
        basicSalary: "Basic Salary",
        kpiPay: "KPI Pay",
        commission: "Commission",
        allowances: "Allowances",
        otherEarnings: "Other Earnings",
        deductions: "Deductions",
        grossEarnings: "Gross",
        netPay: "Net",
        breakdown: "Breakdown",
        addComponent: "Add Component",
      },
      actions: {
        recalculate: "Recalculate",
        hrReview: "HR Review",
        financeApprove: "Finance Approve",
        post: "Post",
        pay: "Record Payment",
      },
      toasts: {
        created: "Payroll run created",
        recalculated: "Recalculated",
        hrReviewed: "Payroll reviewed",
        financeApproved: "Payroll approved by finance",
        posted: "Payroll posted",
        paid: "Payment recorded",
        componentAdded: "Component added",
      },
      errors: {
        alreadyExists: "A payroll run already exists for this period",
      },
    },
  },
  investors: {
    dashboard: {
      title: "Investor Dashboard",
      description: "Overview of investment opportunities and funding.",
      stats: {
        activeOpportunities: "Active Opportunities",
        targetCapital: "Total Target Capital",
        confirmedCapital: "Confirmed Capital",
        totalInvestors: "Total Investors",
      },
      endingSoon: {
        title: "Opportunities Ending Soon",
        within: "Within {days} days",
        none: "No opportunities ending soon.",
      },
      recentContributions: {
        title: "Recent Contributions",
        none: "No contributions recorded yet.",
      },
    },
    list: {
      title: "Investors",
      description: "Investor registry and basic details.",
      addNew: "Add Investor",
      fields: {
        name: "Name",
        entityType: "Type",
        investorType: "Investor Type",
        phone: "Phone",
        email: "Email",
        activeInvestmentsCount: "Active Investments",
        totalConfirmedFunding: "Confirmed Funding",
        status: "Status",
        commercialRegistration: "Commercial Registration",
        nationalId: "National ID",
        residencyId: "Residency ID",
        iban: "IBAN",
        notes: "Notes",
      },
      entityType: {
        PERSON: "Person",
        ORGANIZATION: "Organization",
      },
      status: {
        ACTIVE: "Active",
        INACTIVE: "Inactive",
      },
    },
    profile: {
      tabs: {
        overview: "Overview",
        investments: "Investments",
        funding: "Funding",
        profits: "Profits",
        statement: "Statement",
        activity: "Activity",
      },
      summary: {
        totalInvested: "Total Invested",
        activeOpportunities: "Active Opportunities",
        completedOpportunities: "Completed Opportunities",
        profitUnavailable: "Not available until the Profit Engine is implemented",
      },
    },
    opportunities: {
      title: "Investment Opportunities",
      description: "Manage investment opportunities, their products, and funding.",
      addNew: "Add Investment Opportunity",
      fields: {
        code: "Code",
        nameAr: "Name",
        nameEn: "English Name",
        description: "Description",
        currency: "Currency",
        startDate: "Start Date",
        endDate: "End Date",
        investorNetProfitSharePercent: "Investor Net Profit Share %",
        status: "Status",
        targetCapital: "Target Capital",
        committedCapital: "Committed Capital",
        confirmedFundedCapital: "Confirmed Funded Capital",
        fundingPercent: "Funding %",
        investorsCount: "Investors",
        productsCount: "Products",
      },
      status: {
        DRAFT: "Draft",
        OPEN: "Open",
        FUNDED: "Funded",
        ACTIVE: "Active",
        ENDED: "Ended",
        SETTLED: "Settled",
        CLOSED: "Closed",
        CANCELLED: "Cancelled",
      },
      create: {
        sectionDetails: "Opportunity Details",
        sectionProducts: "Products",
        productsEligibilityHint:
          'Only active products marked "Available for investment opportunities" are listed. Products disabled later stay on existing opportunities but cannot be selected again.',
        sectionSummary: "Summary",
        addProduct: "Add Product",
        fundedUnits: "Funded Units",
        fundedUnitCost: "Funded Unit Cost",
        lineCapital: "Capital",
        duration: "Duration",
        productsCount: "Products",
        saveAsDraft: "Save as Draft",
      },
      tabs: {
        overview: "Overview",
        products: "Products",
        investors: "Investors",
        funding: "Funding",
        sales: "Sales",
        expenses: "Expenses",
        profit: "Profit",
        settlement: "Settlement",
        distributions: "Distributions",
        activity: "Activity",
      },
      financialSummary: {
        approvedNetProfit: "Approved Net Profit",
        investorProfitPool: "Investor Profit Pool",
        distributedProfit: "Distributed Profit",
        paidProfit: "Paid Profit",
        outstandingInvestorProfit: "Outstanding Investor Profit",
        capitalReturned: "Capital Returned",
      },
      actions: {
        open: "Open for Investment",
        activate: "Activate",
        end: "End Opportunity",
        close: "Close Opportunity",
        cancel: "Cancel",
        archive: "Archive",
        addInvestor: "Add Investor",
        addContribution: "Add Contribution",
        confirm: "Confirm",
        reject: "Reject",
      },
      overview: {
        daysRemaining: "Days Remaining",
        ended: "Ended",
        totalFundedUnits: "Total Funded Units",
      },
    },
    subscriptions: {
      fields: {
        investor: "Investor",
        committedAmount: "Committed",
        fundedAmount: "Funded",
        participationPercent: "Participation %",
        status: "Status",
      },
      status: {
        PENDING: "Pending",
        COMMITTED: "Committed",
        PARTIALLY_FUNDED: "Partially Funded",
        FUNDED: "Funded",
        CANCELLED: "Cancelled",
      },
    },
    contributions: {
      fields: {
        investor: "Investor",
        date: "Date",
        amount: "Amount",
        status: "Status",
        paymentMethod: "Payment Method",
        reference: "Reference",
        confirmedBy: "Confirmed By",
      },
      status: {
        PENDING: "Pending",
        CONFIRMED: "Confirmed",
        REJECTED: "Rejected",
        CANCELLED: "Cancelled",
      },
      addNew: "Add Contribution",
    },
    sales: {
      fields: {
        product: "Product",
        order: "Order",
        customer: "Customer",
        date: "Date",
        lineQuantity: "Line Qty",
        allocatedQuantity: "Allocated Qty",
        allocatedRevenue: "Allocated Revenue",
        type: "Type",
        status: "Status",
      },
      type: {
        AUTO: "Auto",
        MANUAL: "Manual",
        SETTLEMENT: "Settlement",
        REALLOCATION: "Reallocation",
      },
      status: {
        ACTIVE: "Active",
        REVERSED: "Reversed",
      },
      metrics: {
        fundedUnits: "Funded Units",
        netSoldUnits: "Net Sold Units",
        returnedUnits: "Returned Units",
        remainingUnits: "Remaining Units",
        sellThroughPercent: "Sell-Through %",
        attributableRevenue: "Attributable Revenue",
        cogs: "COGS",
      },
      actions: {
        recalculate: "Recalculate",
        manualAllocate: "Manual Allocation",
        reverse: "Reverse",
        recordReturn: "Record Return",
        reallocate: "Reallocate",
      },
    },
    expenses: {
      title: "Opportunity Expenses",
      addNew: "Add Expense",
      fields: {
        date: "Date",
        category: "Category",
        description: "Description",
        amount: "Amount",
        status: "Status",
        approvedBy: "Approved By",
      },
      category: {
        ADVERTISING: "Advertising",
        SHIPPING: "Shipping",
        STORAGE: "Storage",
        PAYMENT_FEES: "Payment Fees",
        RETURNS: "Returns",
        OTHER: "Other",
      },
      status: {
        DRAFT: "Draft",
        APPROVED: "Approved",
        REJECTED: "Rejected",
        VOIDED: "Voided",
      },
      actions: {
        approve: "Approve",
        reject: "Reject",
        void: "Void",
      },
    },
    profit: {
      waterfall: {
        revenue: "Attributable Revenue",
        cogs: "COGS",
        expenses: "Approved Expenses",
        returnsAdjustment: "Returns / Adjustments",
        netProfit: "Net Profit",
        investorProfitPool: "Investor Profit Pool",
        companyProfitPortion: "Company Portion (not accounting profit)",
      },
      status: {
        ESTIMATED: "Estimated",
        APPROVED: "Approved",
      },
      shares: {
        investor: "Investor",
        participationPercent: "Participation %",
        profitShareAmount: "Profit Share",
      },
      actions: {
        calculate: "Calculate",
        approve: "Approve",
      },
      noCalculation: "No Profit Calculation yet — showing the live estimate.",
    },
    settlement: {
      fields: {
        status: "Status",
        startedAt: "Started",
        approvedAt: "Approved",
        completedAt: "Completed",
        unresolvedRemainingUnits: "Unresolved Units",
        unresolvedReason: "Unresolved Reason",
      },
      status: {
        DRAFT: "Draft",
        REVIEW: "In Review",
        APPROVED: "Approved",
        COMPLETED: "Completed",
        CANCELLED: "Cancelled",
      },
      suggestions: {
        title: "Settlement Suggestions",
        product: "Product",
        order: "Order",
        available: "Available",
        suggested: "Suggested",
        revenue: "Est. Revenue",
        none: "No eligible unallocated sales found.",
      },
      actions: {
        start: "Start Settlement",
        moveToReview: "Move to Review",
        approve: "Approve Settlement",
        complete: "Complete Settlement",
        cancel: "Cancel Settlement",
      },
      completeDialog: {
        title: "Complete Settlement",
        unresolvedWarning: "{count} unit(s) remain unresolved — accepting requires a reason.",
        reasonPlaceholder: "Reason the remaining units were not settled",
      },
      none: "No Settlement started yet.",
    },
    distributions: {
      empty: "No Profit Distribution has been created for this Opportunity yet.",
      fields: {
        investor: "Investor",
        total: "Total",
        entitled: "Entitled",
        paid: "Paid",
        outstanding: "Outstanding",
        amount: "Amount",
        date: "Date",
        financialAccount: "Financial Account",
        paymentMethod: "Payment Method",
        reference: "Reference",
        notes: "Notes",
      },
      status: {
        DRAFT: "Draft",
        APPROVED: "Approved",
        PARTIALLY_PAID: "Partially Paid",
        PAID: "Paid",
        CANCELLED: "Cancelled",
      },
      investorStatus: {
        PENDING: "Pending",
        PAYABLE: "Payable",
        PARTIALLY_PAID: "Partially Paid",
        PAID: "Paid",
        CANCELLED: "Cancelled",
      },
      paymentStatus: {
        PENDING: "Pending",
        CONFIRMED: "Confirmed",
        REJECTED: "Rejected",
        CANCELLED: "Cancelled",
      },
      actions: {
        create: "New Distribution",
        approve: "Approve",
        cancel: "Cancel",
        recordPayment: "Record Payment",
        confirmPayment: "Confirm",
      },
      createDialog: {
        title: "Create Profit Distribution",
        description:
          "Copies the Approved Profit Calculation's investor entitlements exactly — nothing is recalculated here.",
        pool: "Investor Profit Pool",
        shares: "Investor Entitlements",
      },
      paymentDialog: {
        title: "Record Payment",
        overpaymentBlocked: "Amount exceeds the outstanding entitlement.",
      },
      confirmCancel: {
        title: "Cancel Distribution",
        description:
          "Draft distributions cancel freely. An Approved-but-unpaid distribution reverses its accounting posting and ledger entitlement.",
      },
    },
    ledger: {
      tabs: { profits: "Profits", statement: "Statement" },
      summary: {
        totalConfirmedCapital: "Total Confirmed Capital",
        capitalReturned: "Capital Returned",
        remainingCapitalPosition: "Remaining Capital",
        totalApprovedProfit: "Approved Profit",
        totalProfitPaid: "Profit Paid",
        outstandingProfit: "Outstanding Profit",
      },
      statement: {
        date: "Date",
        opportunity: "Opportunity",
        type: "Type",
        description: "Description",
        debit: "Debit",
        credit: "Credit",
        empty: "No ledger entries yet.",
      },
      entryType: {
        CAPITAL_FUNDED: "Capital Funded",
        PROFIT_ENTITLEMENT: "Profit Entitlement",
        PROFIT_PAYMENT: "Profit Payment",
        CAPITAL_RETURN: "Capital Return",
        ADJUSTMENT: "Adjustment",
        REVERSAL: "Reversal",
      },
      profitsTab: {
        opportunity: "Opportunity",
        approvedProfit: "Approved Profit",
        paid: "Paid",
        outstanding: "Outstanding",
        status: "Status",
        empty: "No Profit Distributions involve this Investor yet.",
      },
    },
    capitalReturns: {
      title: "Capital Returns",
      fields: {
        code: "Code",
        opportunity: "Opportunity",
        amount: "Amount",
        date: "Date",
        financialAccount: "Financial Account",
        status: "Status",
      },
      status: {
        DRAFT: "Draft",
        APPROVED: "Approved",
        PAID: "Paid",
        CANCELLED: "Cancelled",
      },
      actions: {
        create: "Return Capital",
        approve: "Approve",
        pay: "Pay",
        cancel: "Cancel",
      },
      empty: "No Capital Returns recorded yet.",
    },
  },
  investorPortal: {
    nav: {
      dashboard: "Home",
      investments: "My Investments",
      profits: "Profits & Distributions",
      statement: "Statement",
      documents: "Documents",
      account: "My Account",
      logout: "Sign Out",
    },
    accountStatus: {
      INVITED: "Invited",
      ACTIVE: "Active",
      SUSPENDED: "Suspended",
      DISABLED: "Disabled",
    },
    auth: {
      loginTitle: "Investor Portal",
      loginSubtitle: "Sign in to view your investments, profits, and statement.",
      email: "Email",
      emailPlaceholder: "you@example.com",
      password: "Password",
      passwordPlaceholder: "••••••••",
      emailRequired: "Enter a valid email address.",
      passwordRequired: "Enter your password.",
      forgotPassword: "Forgot your password?",
      signIn: "Sign In",
      signingIn: "Signing in…",
      genericError: "We couldn't sign you in. Please try again.",
      forgotTitle: "Reset Your Password",
      forgotSubtitle: "Enter your email and we'll send you a reset link.",
      forgotSubmit: "Send Reset Link",
      forgotSubmitting: "Sending…",
      backToLogin: "Back to Sign In",
      activateTitle: "Set Your Password",
      activateSubtitle: "Choose a password for your Investor Portal account.",
      newPassword: "New Password",
      newPasswordPlaceholder: "At least 8 characters",
      newPasswordRequired: "Password must be at least 8 characters.",
      activateSubmit: "Activate Account",
      activateSubmitting: "Activating…",
      activateSuccess: "Your account is ready. You can now sign in.",
      activateInvalidToken: "This link is invalid or has expired. Request a new one below.",
      missingToken: "This link is missing its activation token.",
    },
    dashboard: {
      title: "Home",
      welcome: "Welcome, {name}",
      recentInvestments: "Recent Investments",
      viewAllInvestments: "View all investments",
      noRecentInvestments: "You have no investments yet.",
    },
    investments: {
      title: "My Investments",
      empty: "You have no investments yet.",
      emptyDescription: "Investments you fund will appear here.",
      fields: {
        opportunity: "Opportunity",
        status: "Status",
        startDate: "Start Date",
        endDate: "End Date",
        committedAmount: "Committed",
        confirmedFunding: "Confirmed Funding",
        participationPercent: "Participation %",
        approvedProfit: "Approved Profit",
        paidProfit: "Paid",
        outstandingProfit: "Outstanding",
      },
      detail: {
        notFoundTitle: "Investment Not Found",
        notFoundDescription: "This investment doesn't exist or isn't linked to your account.",
        backToList: "Back to My Investments",
        sectionOverview: "Overview",
        sectionFunding: "My Funding",
        sectionPerformance: "Performance",
        sectionProfit: "My Profit",
        sectionPayments: "Payments",
        fundedUnits: "Funded Units",
        soldUnits: "Sold Units",
        remainingUnits: "Remaining Units",
        sellThroughPercent: "Sell-Through %",
        contributionsEmpty: "No funding contributions recorded yet.",
        profitEmpty: "No profit entitlements recorded yet.",
        paymentsEmpty: "No payments recorded yet.",
        contributionDate: "Date",
        contributionAmount: "Amount",
        contributionReference: "Reference",
        contributionStatus: "Status",
        distributionCode: "Distribution",
        entitledAmount: "Entitled",
        paidAmount: "Paid",
        outstandingAmount: "Outstanding",
        paymentDate: "Payment Date",
        paymentAmount: "Amount",
        paymentReference: "Reference",
      },
    },
    profits: {
      title: "Profits & Distributions",
      empty: "No profit distributions yet.",
      emptyDescription: "Approved profit entitlements will appear here.",
      fields: {
        opportunity: "Opportunity",
        distribution: "Distribution",
        approvedProfit: "Approved Profit",
        paid: "Paid",
        outstanding: "Outstanding",
        status: "Status",
        lastPaymentDate: "Last Payment Date",
      },
    },
    statement: {
      title: "Statement",
      empty: "No statement entries yet.",
      emptyDescription: "Financial events for your account will appear here.",
      fields: {
        date: "Date",
        opportunity: "Opportunity",
        type: "Type",
        description: "Description",
        debit: "Debit",
        credit: "Credit",
      },
      filters: {
        opportunity: "Opportunity",
        type: "Type",
        dateFrom: "From",
        dateTo: "To",
        allOpportunities: "All Opportunities",
        allTypes: "All Types",
      },
    },
    documents: {
      title: "Documents",
      empty: "No documents yet.",
      emptyDescription: "Funding and distribution receipts will appear here.",
      fields: {
        name: "Document",
        type: "Type",
        opportunity: "Opportunity",
        date: "Date",
      },
      type: {
        FUNDING_RECEIPT: "Funding Receipt",
        DISTRIBUTION_RECEIPT: "Distribution Receipt",
      },
      view: "View",
      downloadFailed: "Couldn't open this document. Please try again.",
    },
    account: {
      title: "My Account",
      profileSection: "Profile",
      portalAccessSection: "Portal Access",
      fields: {
        name: "Name",
        investorType: "Investor Type",
        phone: "Phone",
        email: "Email",
        nationalId: "National ID",
        residencyId: "Residency ID",
        iban: "IBAN",
        portalEmail: "Portal Login Email",
        portalStatus: "Status",
        lastLoginAt: "Last Sign-In",
      },
      readOnlyNote: "These fields are read-only. Contact your account manager to update them.",
    },
    common: {
      loading: "Loading…",
      loadFailed: "Couldn't load this page. Please try again.",
      retry: "Retry",
      page: "Page {page} of {pageCount}",
      previous: "Previous",
      next: "Next",
      notAvailable: "—",
    },
    admin: {
      sectionTitle: "Portal Access",
      noAccount: "This Investor has no Portal account yet.",
      fields: {
        email: "Login Email",
        status: "Status",
        invitedAt: "Invited",
        activatedAt: "Activated",
        lastLoginAt: "Last Sign-In",
      },
      actions: {
        invite: "Invite to Portal",
        resendInvite: "Resend Invite",
        suspend: "Suspend Access",
        reactivate: "Reactivate",
        disable: "Disable Access",
      },
      confirm: {
        suspendTitle: "Suspend Portal Access",
        suspendDescription:
          "The Investor will not be able to sign in to the Portal until reactivated.",
        disableTitle: "Disable Portal Access",
        disableDescription:
          "The Investor will lose Portal access. This can be reversed by an Admin later.",
      },
      toasts: {
        invited: "Portal invitation sent.",
        resent: "Portal invitation resent.",
        suspended: "Portal access suspended.",
        reactivated: "Portal access reactivated.",
        disabled: "Portal access disabled.",
      },
    },
  },
  // docFlow:begin
  docFlow: {
    products: {
      browse: "Browse products",
      browseTitle: "Add products",
      selectedCount: "{count} selected",
      addSelected: "Add {count} lines",
      allCategories: "All categories",
      allBrands: "All brands",
      allTypes: "All types",
      selectPage: "Select all on this page",
      clearSelection: "Clear selection",
      pageOf: "Page {page} of {pages} · {total} products",
      previousPage: "Previous page",
      nextPage: "Next page",
      createNew: "New product",
      activateFailed:
        "The product was created but couldn't be activated. Complete its details in Products, then select it.",
      noInvestmentEligible:
        'No products are marked available for investment opportunities. Enable "Available for investment opportunities" on a product first.',
    },
    lines: {
      options: "Line options",
      treatment: "Recognize as",
      treatments: {
        STANDARD: "Standard purchase",
        FIXED_ASSET: "Fixed asset",
        PREPAID_EXPENSE: "Prepaid expense",
      },
      usefulLife: "Useful life (months)",
      method: "Depreciation method",
      methods: {
        STRAIGHT_LINE: "Straight line",
        DECLINING_BALANCE: "Declining balance",
      },
      prepaidMonths: "Months covered",
      expenseAccount: "Expense account",
      startDate: "First month",
      assetHint:
        "Posting the invoice capitalizes this line and creates the asset with its depreciation schedule. Due months post automatically.",
      prepaidHint:
        "Posting the invoice defers this line to Prepayments and recognizes it monthly into the expense account.",
      priceRequired: "Enter a price greater than 0 for every product.",
      amountRequired: "Enter the agreed amount (greater than 0) for every product.",
    },
    actions: {
      more: "More",
    },
    flow: {
      quotationApproveTitle: "Approve this quotation?",
      quotationApproveDescription:
        "The quotation becomes Approved and can be converted into an order in one step. Nothing is posted or reserved.",
      orderTitle: "Confirm this sales order?",
      orderDescription:
        "The order becomes Confirmed and reserves stock for its stocked lines. No accounting entry is created. Next step: create the invoice.",
      salesInvoiceTitle: "Post this invoice?",
      salesInvoiceDescription:
        "Posting records the revenue and the customer receivable, delivers stocked lines from the warehouse with their cost (COGS), and creates one journal entry. A posted invoice is corrected only with a sales return.",
      purchaseOrderTitle: "Approve this purchase order?",
      purchaseOrderDescription:
        "Approving commits the order to the supplier. No stock, cost or accounting entry is created until the goods are invoiced.",
      purchaseInvoiceTitle: "Post this purchase invoice?",
      purchaseInvoiceDescription:
        "Posting records the supplier payable, receives stocked lines into inventory at moving-average cost, capitalizes fixed-asset lines, defers prepaid lines, and creates one journal entry. A posted invoice is corrected only with a purchase return.",
    },
    lifecycle: {
      duplicate: "Duplicate",
      duplicateTitle: "Create a draft copy?",
      duplicateDescription:
        "A new draft with a new number is created from {document} — same party, currency and lines. Payments, postings, stock movements and approvals are never copied; the original is unchanged.",
      duplicated: "Draft {number} created.",
      returnToDraft: "Return to draft",
      returnToDraftTitle: "Return to draft?",
      returnToDraftDescription:
        "{document} goes back to Draft so it can be edited; it must be approved again. Reservations are released where applicable. Posted documents cannot return to draft — correct them with a return or a reversal.",
      returnedToDraft: "Returned to draft.",
    },
    import: {
      menu: "Import",
    },
    payments: {
      verifiedPosted: "Payment verified — receipt {receipts} posted to the ledger.",
      verifiedPendingInvoice:
        "Payment verified. The customer receipt posts automatically once the order's invoice is generated.",
      verifiedPostingFailed:
        "Payment verified, but the receipt could not be posted: {message} Fix the cause, then use “Sync receipt”.",
      syncReceipt: "Sync receipt",
    },
    preview: {
      openFull: "Open full record",
      loadFailed: "Couldn't load the preview.",
      status: "Status",
      party: "Party",
      date: "Date",
      total: "Total",
      paid: "Paid",
      outstanding: "Outstanding",
      lines: "Lines",
      reference: "Reference",
      amount: "Amount",
      allocatedTo: "Allocated to",
      paymentSource: "Payment source",
      journal: "Journal",
      currency: "Currency",
      description: "Description",
      account: "Account",
      debit: "Debit",
      credit: "Credit",
      totals: "Totals",
      balanced: "Balanced",
      unbalanced: "Unbalanced",
    },
    return: {
      backTo: "Back to {label}",
      trail: "Navigation trail",
      previous: "previous page",
    },
    trace: {
      view: "Related",
      state: {
        AWAITING_PAYMENT: "No payments yet",
        PENDING: "Pending — created when the document is posted",
        FAILED: "Missing — this should exist but doesn't. Retry the posting or contact finance.",
        UNAUTHORIZED: "Restricted — you don't have permission to view these records",
        NONE: "None recorded",
        FAILED_PARTIAL:
          "Incomplete — some expected records are missing. Retry the posting or contact finance.",
        AWAITING_DOCUMENT: "Not created yet",
        AWAITING_SHIPMENT: "Not shipped yet",
      },
      showingOf: "Showing {shown} of {total}",
      viewAll: "View all",
      title: "Related records",
      none: "No related records yet.",
      loadFailed: "Couldn't load related records.",
      groups: {
        SOURCE: "Source",
        DOCUMENTS: "Documents",
        PAYMENTS: "Payments",
        JOURNAL_ENTRIES: "Journal entries",
        STOCK_MOVEMENTS: "Stock movements",
        ASSETS: "Assets & prepayments",
        SHIPMENTS: "Deliveries & shipments",
        RETURNS: "Returns",
      },
    },
    reports: {
      unbalancedBy: "Unbalanced by",
      debitsEqualCredits: "Debits = Credits",
      assetsEqualLiabilitiesEquity: "Assets = Liabilities + Equity",
    },
    fx: {
      title: "Exchange rate needed: {from} → {to}",
      description:
        "Posting this {from} document needs the {from} → {to} rate on or before {date}. Your document is unchanged — record the rate and the posting continues.",
      rateLabel: "1 {from} = ? {to}",
      effectiveDate: "Effective date",
      noGuess: "OMS never guesses a rate or treats a foreign currency as the base currency.",
      noPermission:
        "You don't have permission to record exchange rates. Ask a finance user to add this rate, then try again.",
      openRates: "Open exchange rates",
      saveAndContinue: "Save rate and continue",
      saved: "Rate {from} → {to} saved.",
    },
    kinds: {
      SALES_QUOTATION: "Quotation",
      SALES_ORDER: "Sales order",
      SALES_INVOICE: "Sales invoice",
      SALES_RETURN: "Sales return",
      CUSTOMER_RECEIPT: "Receipt",
      CUSTOMER_REFUND: "Refund",
      CUSTOMER: "Customer",
      PURCHASE_QUOTATION: "Purchase quotation",
      PURCHASE_ORDER: "Purchase order",
      PURCHASE_INVOICE: "Purchase invoice",
      PURCHASE_RETURN: "Purchase return",
      SUPPLIER_PAYMENT: "Payment voucher",
      EXPENSE_PAYMENT: "Expense payment",
      LANDED_COST: "Landed cost",
      STORE_ORDER: "Store order",
      PAYMENT: "Payment",
      JOURNAL_ENTRY: "Journal entry",
      INVENTORY_MOVEMENT: "Stock movement",
      FIXED_ASSET: "Fixed asset",
      PREPAID_EXPENSE: "Prepaid expense",
      SHIPMENT: "Shipment",
    },
    status: {
      DRAFT: "Draft",
      PENDING_APPROVAL: "Pending approval",
      APPROVED: "Approved",
      CONFIRMED: "Posted",
      POSTED: "Posted",
      REVERSED: "Reversed",
      CANCELLED: "Cancelled",
      CLOSED: "Closed",
      DELIVERED: "Delivered",
      PARTIALLY_DELIVERED: "Partially delivered",
      PENDING: "Pending",
      MATCHED: "Matched",
      VERIFIED: "Verified",
      REJECTED: "Rejected",
      CAPITALIZED: "Capitalized",
      DISPOSED: "Disposed",
      ACTIVE: "Active",
      COMPLETED: "Completed",
      RESERVATION: "Reservation",
      RESERVATION_RELEASE: "Release",
      SALES_DELIVERY: "Delivery",
      PURCHASE_RECEIPT: "Receipt",
      ADJUSTMENT: "Adjustment",
      FULLY_PAID_RECONCILED: "Paid",
      PARTIALLY_PAID: "Partially paid",
      PAYMENT_PENDING: "Unpaid",
      PAYMENT_REVIEW: "Payment review",
      LABEL_CREATED: "Label created",
      SHIPPED: "Shipped",
      OUT_FOR_DELIVERY: "Out for delivery",
      DELIVERY_FAILED: "Delivery failed",
      NEEDS_RESHIPMENT: "Needs reshipment",
      RETURN_BEFORE_DELIVERY: "Returned before delivery",
      RETURN_AFTER_DELIVERY: "Returned after delivery",
    },
  },
  // docFlow:end
} as const;

export default en;
